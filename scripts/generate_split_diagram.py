...
STEPS: List[Step] = [
    Step(CHARTER, "flow.authorize()", "authorization URL, PKCE verifier + challenge, `state` generation"),
...
def render(palette: Palette, out: Path) -> None:
    ...
    wrapped = [
        ...
        if step.owner != SERVER else [step.phrase.replace("`", "")]
        for step in STEPS
    ]
    ...
    for owner, label in LANES.items():
        ax.text(
            LANE_X[owner], HEADER_Y, label.upper(),
            fontproperties=f_lane, color=palette.muted, va="bottom", ha="left",
        )
        ax.plot(
            [LANE_X[owner], LANE_X[owner] + LANE_W], [HEADER_Y + 10, HEADER_Y + 10],
            color=palette.muted_edge, linewidth=1.0, solid_capstyle="butt",
        )
    ...
    for index, ((step, y, h), lines) in enumerate(zip(placed, wrapped, strict=True), start=1):
        if step.owner == SERVER:
            ...
            ax.text(
                mid, y + h / 2 - 13, lines[0],
                fontproperties=f_band, color=palette.muted, va="center", ha="center",
            )
            ax.text(
                mid, y + h / 2 + 14,
                "on the authorization server — where neither of you is running",
                fontproperties=f_band_sub, color=palette.muted, va="center", ha="center", alpha=0.75,
            )
        ...
        # never beside it.
        ax.text(
            x + PAD_X, y + 19, str(index),
            fontproperties=f_num, color=keyed, va="center", ha="left", alpha=0.55,
        )
        cursor = y + CONTENT_TOP
        if step.symbol:
            ax.text(
                x + PAD_X, cursor + 12, step.symbol,
                fontproperties=fitted(step.symbol), color=keyed, va="center", ha="left",
            )
            cursor += SYMBOL_H
        for line in lines:
            ax.text(
                x + PAD_X, cursor + LINE_H / 2, line,
    ...
    for (x0, _, bottom), (x1, top, _) in zip(anchors, anchors[1:], strict=False):
        ...
        ax.plot(
            [x0, x0, x1], [bottom + 3, turn, turn],
            color=palette.arrow, linewidth=1.2,
            solid_capstyle="round", solid_joinstyle="round",
        )
        ax.add_patch(FancyArrowPatch(
            (x1, turn), (x1, top - 2),
            arrowstyle="-|>,head_length=4.5,head_width=2.8",
            color=palette.arrow, linewidth=1.2, shrinkA=0, shrinkB=0,
        ))
    ...
    ax.plot(
        [bx, bx + 9, bx + 9, bx], [top_edge, top_edge + 9, bottom_edge - 9, bottom_edge],
        color=palette.ink, linewidth=1.3, alpha=0.45,
        solid_capstyle="round", solid_joinstyle="round",
    )
    label_x = bx + 22
    ...
    for offset, line in enumerate(label_lines):
        ax.text(
            label_x, start + offset * 21, line,
            fontproperties=f_span, color=palette.ink, alpha=0.8, va="center", ha="left",
        )
