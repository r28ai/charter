        ...
        try:
            ...
            if status in allow_status:
                self.outcomes.append(
                    Outcome(tool_name, "expected", f"HTTP {status} — {note or 'documented for this repo'}")
                )
                return None
            self.outcomes.append(Outcome(tool_name, "fail", f"HTTP {status}: {str(exc)[:200]}"))
            return None
        except CharterError as exc:
            self.outcomes.append(Outcome(tool_name, "fail", f"{type(exc).__name__}: {str(exc)[:200]}"))
            return None
...
async def phase_reads(run: Run) -> None:
    ...
    await run.call("repos_get", **repo_args(), expect=lambda r: r["full_name"].lower() == REPO.lower())
    await run.call("repos_list_for_authenticated_user", per_page=5, expect=lambda r: isinstance(r, list))
    await run.call(
        "repos_list_for_org", org=OWNER, per_page=5,
        allow_status=(404,), note=f"{OWNER} is a user account, not an organization",
    )
    await run.call("repos_get_readme", **repo_args(), expect=lambda r: "content" in r)
    await run.call(
        "repos_get_content", **repo_args(), path="README.md",
    ...
    await run.call("repos_list_contributors", **repo_args(), expect=lambda r: isinstance(r, (list, dict)))
    await run.call("repos_list_tags", **repo_args(), expect=lambda r: isinstance(r, list))
    await run.call("branches_list", **repo_args(), expect=lambda r: any(b["name"] == "main" for b in r))
    await run.call("repos_get_branch", **repo_args(), branch="main", expect=lambda r: r["name"] == "main")
    await run.call(
        "repos_get_branch_protection", **repo_args(), branch="main",
        allow_status=(403, 404), note="the sandbox's main branch is unprotected",
    )
    ...
    if commits:
        ...
        await run.call(
            "repos_get_commit", **repo_args(), ref=commits[0]["sha"],
            expect=lambda r: r.get("sha") == commits[0]["sha"],
        )
        await run.call(
            "repos_list_branches_for_head_commit", **repo_args(), commit_sha=commits[0]["sha"],
            expect=lambda r: isinstance(r, list),
        )
        await run.call(
            "repos_get_combined_status", **repo_args(), ref=commits[0]["sha"],
            expect=lambda r: r.get("state") in {"success", "pending", "failure", "error"},
        )
        await run.call(
            "checks_list_for_ref", **repo_args(), ref=commits[0]["sha"],
            expect=lambda r: "check_runs" in r,
        )
        await run.call(
            "checks_list_suites_for_ref", **repo_args(), ref=commits[0]["sha"],
...
async def phase_git(run: Run) -> None:
    ...
    """
    head = await run.call(
        "git_refs_get", **repo_args(), ref="heads/main",
    ...
    commit = await run.call(
        "git_commits_get", **repo_args(), commit_sha=base_sha,
    ...
    await run.call(
        "git_trees_get", **repo_args(), tree_sha=base_tree, recursive="1",
        expect=lambda r: "truncated" in r and isinstance(r.get("tree"), list),
    )

    blob = await run.call(
        "git_blobs_create", **repo_args(), content="written by the live check\n",
        expect=lambda r: len(r.get("sha", "")) == 40,
    )
    if blob:
        await run.call(
            "git_blobs_get", **repo_args(), file_sha=blob["sha"],
            expect=lambda r: r.get("content") == "written by the live check\n",
        )

    tree = await run.call(
        "git_trees_create", **repo_args(), base_tree=base_tree,
        tree=[
            {"path": f"{RUN}/one.txt", "mode": "100644", "type": "blob", "content": "one\n"},
            {"path": f"{RUN}/two.txt", "mode": "100644", "type": "blob",
    ...
    made = await run.call(
        "git_commits_create", **repo_args(),
        message=f"{RUN}: two files, one commit", tree=tree["sha"], parents=[base_sha],
        expect=lambda r: len(r.get("sha", "")) == 40,
    )
    if not made:
        return
    ...
    await run.call(
        "git_refs_create", **repo_args(), ref=f"refs/heads/{branch}", sha=made["sha"],
        note=f"created {branch}",
    )
    await run.call(
        "git_refs_list_matching", **repo_args(), ref=f"heads/{RUN}",
    ...
    second_tree = await run.call(
        "git_trees_create", **repo_args(), base_tree=tree["sha"],
        tree=[{"path": f"{RUN}/one.txt", "mode": "100644", "type": "blob", "content": "one, edited\n"}],
        expect=lambda r: bool(r.get("sha")),
    )
    if second_tree:
        second = await run.call(
            "git_commits_create", **repo_args(), message=f"{RUN}: edit one.txt",
            tree=second_tree["sha"], parents=[made["sha"]],
            expect=lambda r: bool(r.get("sha")),
        )
        if second:
            await run.call(
                "git_refs_update", **repo_args(), ref=f"heads/{branch}", sha=second["sha"],
                expect=lambda r: r.get("sha") == second["sha"],
            )
            run.state["commit_sha"] = second["sha"]

    await run.call(
        "repos_compare_commits", **repo_args(), basehead=f"main...{branch}",
        expect=lambda r: r.get("status") in {"ahead", "diverged"} and r.get("ahead_by", 0) >= 1,
    )

    tag_object = await run.call(
        "git_tags_create", **repo_args(), tag=f"{RUN}-v1", message="live check tag",
        object=run.state["commit_sha"], type="commit",
        expect=lambda r: bool(r.get("sha")),
    )
    if tag_object:
        await run.call(
            "git_refs_create", **repo_args(),
            ref=f"refs/tags/{RUN}-v1", sha=tag_object["sha"],
    ...
    written = await run.call(
        "repos_create_or_update_file", **repo_args(),
        path=f"{RUN}/scratch.txt", message=f"{RUN}: add scratch.txt",
        content="delete me\n", branch=branch,
        expect=lambda r: bool(r.get("content", {}).get("sha")),
    )
    if written:
        await run.call(
            "repos_delete_file", **repo_args(),
            path=f"{RUN}/scratch.txt", message=f"{RUN}: remove scratch.txt",
            sha=written["content"]["sha"], branch=branch,
    ...
    spare = f"{RUN}-spare"
    if await run.call(
        "git_refs_create", **repo_args(), ref=f"refs/heads/{spare}", sha=base_sha,
        note=f"created {spare}",
    ) is not None:
        renamed = f"{RUN}-renamed"
        if await run.call(
            "repos_rename_branch", **repo_args(), branch=spare, new_name=renamed,
            expect=lambda r: r.get("name") == renamed,
        ) is not None:
            run.state["spare_branch"] = renamed

    await run.call(
        "repos_merge_branches", **repo_args(), base=branch, head="main",
        commit_message=f"{RUN}: merge main into the work branch",
        allow_status=(409,), note="already up to date, or a conflict",
    )
...
async def phase_issues(run: Run) -> None:
    ...
    created_label = await run.call(
        "issues_create_label", **repo_args(), name=label, color="ededed",
        description="Created by the live check.",
        expect=lambda r: r.get("name") == label,
    )
    await run.call(
        "issues_list_labels_for_repo", **repo_args(), per_page=100,
        expect=lambda r: any(item["name"] == label for item in r) if created_label else True,
    )
    if created_label:
        await run.call(
            "issues_update_label", **repo_args(), name=label, description="Edited by the live check.",
            expect=lambda r: r.get("description") == "Edited by the live check.",
        )

    milestone = await run.call(
        "issues_create_milestone", **repo_args(), title=f"{RUN} milestone",
        description="Created by the live check.",
    ...
    if milestone:
        await run.call(
            "issues_update_milestone", **repo_args(), milestone_number=milestone["number"],
            state="closed", expect=lambda r: r.get("state") == "closed",
        )

    issue = await run.call(
        "issues_create", **repo_args(),
    ...
    await run.call("issues_get", **repo_args(), issue_number=number,
                   expect=lambda r: r["number"] == number)
    await run.call("issues_list_for_repo", **repo_args(), per_page=5,
                   expect=lambda r: isinstance(r, list))
    await run.call("issues_list_for_authenticated_user", filter="created", per_page=5,
                   expect=lambda r: isinstance(r, list))
    await run.call(
        "issues_update", **repo_args(), issue_number=number,
        body={"body": "Edited by the live check."},
        expect=lambda r: r.get("body") == "Edited by the live check.",
    )

    if created_label:
        await run.call("issues_add_labels", **repo_args(), issue_number=number, labels=[label],
                       expect=lambda r: any(item["name"] == label for item in r))
        await run.call("issues_set_labels", **repo_args(), issue_number=number, labels=[label],
                       expect=lambda r: [item["name"] for item in r] == [label])
        await run.call("issues_remove_label", **repo_args(), issue_number=number, name=label,
                       expect=lambda r: isinstance(r, list))

    login = run.state.get("login")
    if login:
        await run.call("issues_add_assignees", **repo_args(), issue_number=number,
                       assignees=[login], expect=lambda r: login in (r.get("assignees") or []))
        await run.call("issues_remove_assignees", **repo_args(), issue_number=number,
                       assignees=[login], expect=lambda r: login not in (r.get("assignees") or []))

    comment = await run.call(
        "issues_create_comment", **repo_args(), issue_number=number,
        body={"body": "A comment from the live check."},
        expect=lambda r: bool(r.get("id")),
    )
    await run.call("issues_list_comments", **repo_args(), issue_number=number,
                   expect=lambda r: isinstance(r, list))
    if comment:
        await run.call("issues_get_comment", **repo_args(), comment_id=comment["id"],
                       expect=lambda r: r["id"] == comment["id"])
        await run.call("issues_update_comment", **repo_args(), comment_id=comment["id"],
    ...
    await run.call("issues_list_events", **repo_args(), issue_number=number,
                   expect=lambda r: isinstance(r, list))
    await run.call("issues_list_timeline", **repo_args(), issue_number=number,
                   expect=lambda r: isinstance(r, list))
    ...
    if child:
        ...
        if raw:
            await run.call(
                "issues_add_sub_issue", **repo_args(), issue_number=number,
                sub_issue_id=raw["id"], expect=lambda r: bool(r.get("number")),
            )
        await run.call("issues_list_sub_issues", **repo_args(), issue_number=number,
                       expect=lambda r: isinstance(r, list))
...
async def phase_pulls(run: Run) -> None:
    ...
    pull = await run.call(
        "pulls_create", **repo_args(),
        body={"title": f"{RUN}: live check", "head": branch, "base": "main",
    ...
    await run.call("pulls_get", **repo_args(), pull_number=number,
                   expect=lambda r: r["number"] == number)
    await run.call("pulls_list", **repo_args(), per_page=5, expect=lambda r: isinstance(r, list))
    await run.call(
        "pulls_get_diff", **repo_args(), pull_number=number,
        expect=lambda r: r.get("files_changed", 0) >= 1 and "diff --git" in r.get("diff", ""),
    )
    files = await run.call("pulls_list_files", **repo_args(), pull_number=number,
                           expect=lambda r: len(r) >= 1)
    commits = await run.call("pulls_list_commits", **repo_args(), pull_number=number,
                             expect=lambda r: len(r) >= 1)
    await run.call("pulls_update", **repo_args(), pull_number=number,
                   body="Edited by the live check.",
                   expect=lambda r: r.get("body") == "Edited by the live check.")
    await run.call(
        "pulls_check_merged", **repo_args(), pull_number=number,
        allow_status=(404,), note="not merged yet, which this endpoint reports as a 404",
    )
    await run.call("pulls_update_branch", **repo_args(), pull_number=number,
                   allow_status=(422,), note="already up to date with base")
    ...
    if head_sha:
        line_comment = await run.call(
            "pulls_create_review_comment", **repo_args(), pull_number=number,
            body="A remark on this line, from the live check.",
            commit_id=head_sha, path=path, line=1, side="RIGHT",
            expect=lambda r: r.get("path") == path and r.get("line") == 1,
        )
    await run.call("pulls_list_review_comments", **repo_args(), pull_number=number,
                   expect=lambda r: isinstance(r, list))
    await run.call("pulls_list_review_comments_for_repo", **repo_args(), per_page=5,
                   expect=lambda r: isinstance(r, list))
    if line_comment:
        await run.call(
            "pulls_reply_to_review_comment", **repo_args(), pull_number=number,
            comment_id=line_comment["id"], body="And a reply to it.",
            expect=lambda r: bool(r.get("id")),
        )
        await run.call(
            "pulls_update_review_comment", **repo_args(), comment_id=line_comment["id"],
            body="Edited by the live check.",
            expect=lambda r: r.get("body") == "Edited by the live check.",
        )
    ...
    review = await run.call(
        "pulls_create_review", **repo_args(), pull_number=number,
        event="COMMENT", body="A review from the live check.",
        expect=lambda r: bool(r.get("id")),
    )
    await run.call("pulls_list_reviews", **repo_args(), pull_number=number,
                   expect=lambda r: isinstance(r, list))
    if review:
        rid = review["id"]
        await run.call("pulls_get_review", **repo_args(), pull_number=number, review_id=rid,
                       expect=lambda r: r["id"] == rid)
        await run.call("pulls_list_comments_for_review", **repo_args(), pull_number=number,
                       review_id=rid, expect=lambda r: isinstance(r, list))
        await run.call("pulls_update_review", **repo_args(), pull_number=number, review_id=rid,
                       body="Edited by the live check.",
                       expect=lambda r: r.get("body") == "Edited by the live check.")
        await run.call(
            "pulls_dismiss_review", **repo_args(), pull_number=number, review_id=rid,
            message="Dismissed by the live check.",
            allow_status=(422,), note="only a review that blocks the pull request can be dismissed",
        )
    ...
    if pending:
        await run.call(
            "pulls_submit_review", **repo_args(), pull_number=number, review_id=pending,
            event="COMMENT", body="Submitted by the live check.",
            expect=lambda r: r.get("state") == "COMMENTED",
        )
    second_pending = await _raw_pending_review(number)
    if second_pending:
        await run.call("pulls_delete_pending_review", **repo_args(), pull_number=number,
                       review_id=second_pending, expect=lambda r: bool(r.get("id")))

    # You cannot request a review from yourself, so this is the documented 422.
    login = run.state.get("login")
    if login:
        await run.call(
            "pulls_request_reviewers", **repo_args(), pull_number=number, reviewers=[login],
            allow_status=(422,), note="GitHub refuses a review request to the pull request's author",
        )
    await run.call("pulls_list_requested_reviewers", **repo_args(), pull_number=number,
                   expect=lambda r: isinstance(r, dict))
    await run.call(
        "pulls_remove_requested_reviewers", **repo_args(), pull_number=number,
        reviewers=[login] if login else [],
        allow_status=(422,), note="nobody was successfully requested, so there is nobody to remove",
    )

    merged = await run.call(
        "pulls_merge", **repo_args(), pull_number=number, merge_method="squash",
        allow_status=(405, 409), note="the branch was not mergeable at this point",
    )
    if merged is not None:
        run.state["merged_pull"] = True
        await run.call("pulls_check_merged", **repo_args(), pull_number=number,
...
_CI_TOOLS = {
    "actions_get_workflow", "actions_create_workflow_dispatch",
    "actions_list_runs_for_workflow", "actions_get_workflow_run",
    "actions_list_jobs_for_run", "actions_get_job", "actions_download_job_logs",
    "actions_download_run_logs", "actions_list_run_artifacts", "actions_get_artifact",
    "actions_download_artifact", "actions_get_run_usage", "actions_rerun_failed_jobs",
    "actions_rerun_workflow", "actions_cancel_workflow_run",
    "actions_list_pending_deployments", "actions_list_repo_secrets",
    "actions_list_workflows", "actions_list_workflow_runs",
    "checks_get_run", "checks_list_annotations",
}
...
async def phase_actions(run: Run, *, wait: int) -> None:
    ...
    seeded = await run.call(
        "repos_create_or_update_file", **repo_args(),
        path=path, message=f"{RUN}: seed a workflow", content=WORKFLOW, branch="main",
        expect=lambda r: bool(r.get("content", {}).get("sha")),
    ...
    for attempt in range(12):
        ...
        if any(w.get("path") == path for w in listed.get("workflows", [])):
            run.outcomes.append(
                Outcome("actions_list_workflows", "pass",
        ...
        run.outcomes.append(
            Outcome("actions_list_workflows", "fail",
                    "the seeded workflow never appeared in the listing")
        )
    registered = await run.call(
        "actions_get_workflow", **repo_args(), workflow_id=f"{RUN}.yml",
    ...
    for attempt in range(12):
        ...
        run.outcomes.append(
            Outcome(
                "actions_create_workflow_dispatch", "pass",
    ...
    await run.call("actions_get_workflow_run", **repo_args(), run_id=run_id,
                   expect=lambda r: r.get("id") == run_id)
    await run.call("actions_list_workflow_runs", **repo_args(), per_page=5,
                   expect=lambda r: "workflow_runs" in r)
    await run.call("actions_list_pending_deployments", **repo_args(), run_id=run_id,
                   expect=lambda r: isinstance(r, list))
    await run.call(
        "actions_get_run_usage", **repo_args(), run_id=run_id,
        allow_status=(403, 404, 410), note="GitHub has this endpoint closing down",
    )

    jobs = await run.call(
        "actions_list_jobs_for_run", **repo_args(), run_id=run_id,
    ...
    await run.call("actions_get_job", **repo_args(), job_id=subject["id"],
                   expect=lambda r: r.get("id") == subject["id"])

    # The one that matters: a real failing job's real log, reduced.
    await run.call(
        "actions_download_job_logs", **repo_args(), job_id=subject["id"],
        expect=lambda r: r.get("total_lines", 0) > 0 and isinstance(r.get("tail"), str),
        note=(
            "read the failing job's log"
            if failed else "read a job log"
        ),
    )
    await run.call(
        "actions_download_run_logs", **repo_args(), run_id=run_id,
    ...
    head = (await run.call(
        "actions_get_workflow_run", **repo_args(), run_id=run_id,
    ...
    if head:
        ...
        while time.monotonic() < deadline:
            listed = await run.call(
                "checks_list_for_ref", **repo_args(), ref=head,
                expect=lambda r: "check_runs" in r,
            )
    ...
    if check:
        ...
        subject_check = next(
            (c for c in check if c.get("conclusion") == "failure"), check[0]
        )
        await run.call(
            "checks_get_run", **repo_args(), check_run_id=subject_check["id"],
            expect=lambda r: bool(r.get("id")),
        )
        await run.call(
            "checks_list_annotations", **repo_args(), check_run_id=subject_check["id"],
            expect=lambda r: isinstance(r, list),
    ...
    if failed:
        await run.call(
            "actions_rerun_failed_jobs", **repo_args(), run_id=run_id,
    ...
    await run.call(
        "actions_rerun_workflow", **repo_args(), run_id=run_id,
        allow_status=(403,), note="re-ran every job, including the ones that passed",
    )
    await _dispatch_and_cancel(run)

    artifacts = await run.call(
        "actions_list_run_artifacts", **repo_args(), run_id=run_id,
        expect=lambda r: "artifacts" in r,
    )
    items = (artifacts or {}).get("artifacts", [])
    if items:
        await run.call("actions_get_artifact", **repo_args(), artifact_id=items[0]["id"],
                       expect=lambda r: bool(r.get("name")))
        await run.call(
            "actions_download_artifact", **repo_args(), artifact_id=items[0]["id"],
    ...
    await run.call("actions_list_repo_secrets", **repo_args(),
                   expect=lambda r: "secrets" in r)
...
async def _dispatch_and_cancel(run: Run) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if fresh:
            await run.call(
                "actions_cancel_workflow_run", **repo_args(), run_id=fresh[0]["id"],
                allow_status=(409,), note=f"cancelled run {fresh[0]['id']}",
            )
            return
        await asyncio.sleep(5)
...
async def _await_run(run: Run, wait: int) -> Optional[int]:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            run.outcomes.append(
                Outcome("actions_list_runs_for_workflow", "fail", str(exc)[:200])
            )
            return None
        ...
        if runs:
            ...
            if runs[0].get("status") == "completed":
                run.outcomes.append(
                    Outcome("actions_list_runs_for_workflow", "pass",
...
async def phase_releases(run: Run) -> None:
    await run.call(
        "releases_generate_notes", **repo_args(), tag_name=f"{RUN}-notes",
    ...
    release = await run.call(
        "releases_create", **repo_args(), tag_name=f"{RUN}-rel", name=f"{RUN} release",
        body="Created by the live check.", draft=True,
        expect=lambda r: bool(r.get("id")),
    )
    await run.call("releases_list", **repo_args(), per_page=5, expect=lambda r: isinstance(r, list))
    await run.call(
        "releases_get_latest", **repo_args(),
        allow_status=(404,), note="the sandbox has no published release",
    )
    if not release:
        return
    rid = release["id"]
    ...
    await run.call(
        "releases_update", **repo_args(), release_id=rid, body="Edited by the live check.",
        expect=lambda r: r.get("body") == "Edited by the live check.",
    )
    await run.call(
        "releases_upload_asset", **repo_args(), release_id=rid,
        name=f"{RUN}-checksums.txt", content="d41d8cd98f00b204e9800998ecf8427e  empty\n",
        expect=lambda r: r.get("state") == "uploaded",
        note="posted raw bytes to uploads.github.com",
    )
    await run.call("releases_list_assets", **repo_args(), release_id=rid,
                   expect=lambda r: len(r) >= 1)
    ...
    published = await run.call(
        "releases_update", **repo_args(), release_id=rid,
        draft=False, tag_name=f"{RUN}-rel",
        expect=lambda r: r.get("draft") is not True and r.get("tag_name") == f"{RUN}-rel",
    )
    if published:
        await run.call(
            "releases_get_by_tag", **repo_args(), tag=f"{RUN}-rel",
...
async def phase_discovery(run: Run) -> None:
    await run.call("search_issues", q=f"repo:{REPO} {RUN}", per_page=5,
                   expect=lambda r: "items" in r)
    await run.call("search_repositories", q="charter language:python", per_page=5,
                   expect=lambda r: "items" in r)
    await run.call("search_code", q=f"repo:{REPO} charter", per_page=5,
                   allow_status=(403, 422), note="code search is indexed separately and rate limited")
    await run.call("search_commits", q=f"repo:{REPO} {RUN}", per_page=5,
                   expect=lambda r: "items" in r)
    await run.call("search_users", q="type:org charter", per_page=5,
                   expect=lambda r: "items" in r)

    threads = await run.call("notifications_list", per_page=5, all=True,
                             expect=lambda r: isinstance(r, list))
    if threads:
        await run.call("notifications_get_thread", thread_id=int(threads[0]["id"]),
                       expect=lambda r: bool(r.get("reason")))
    else:
        run.skip("notifications_get_thread", "the account has no notifications to read")

    await run.call(
        "dependabot_list_alerts", **repo_args(), per_page=5,
        allow_status=(403, 404), note="Dependabot is not enabled on the sandbox",
    )
    await run.call(
        "dependabot_get_alert", **repo_args(), alert_number=1,
        allow_status=(403, 404), note="no alert 1 on the sandbox",
    )
    await run.call(
        "code_scanning_list_alerts", **repo_args(), per_page=5,
        allow_status=(403, 404), note="code scanning is not configured on the sandbox",
    )
    await run.call(
        "code_scanning_get_alert", **repo_args(), alert_number=1,
        allow_status=(403, 404), note="no alert 1 on the sandbox",
    )
    await run.call(
        "secret_scanning_list_alerts", **repo_args(), per_page=5, hide_secret=True,
        allow_status=(403, 404), note="secret scanning is not enabled on the sandbox",
    )
...
async def teardown(run: Run) -> List[str]:
    ...
    if (rid := run.state.get("release")) is not None:
        ...
        await attempt(
            "git_refs_delete", f"tag {RUN}-rel", **repo_args(), ref=f"tags/{RUN}-rel"
        )

    for number_key in ("child_issue", "issue"):
        if (number := run.state.get(number_key)) is not None:
            await attempt(
                "issues_update", f"issue #{number}", **repo_args(), issue_number=number,
    ...
    if (path := run.state.get("workflow_path")) is not None:
        ...
        if sha:
            await attempt(
                "repos_delete_file", f"workflow {path}", **repo_args(), path=path,
                message=f"{RUN}: remove the seeded workflow", sha=sha, branch="main",
            )
    ...
    if run.state.get("tag"):
        await attempt(
            "git_refs_delete", f"tag {run.state['tag']}",
            **repo_args(), ref=f"tags/{run.state['tag']}",
        )

    await attempt(
        "issues_delete_label", f"label {RUN}-label", **repo_args(), name=f"{RUN}-label"
    )
