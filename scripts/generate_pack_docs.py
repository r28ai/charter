...
def _dominant(tools: Sequence[Tool], read: Callable[[Tool], Any]) -> Tuple[Any, List[str]]:
...
def _fence(title: str, lines: Sequence[str], marked: Sequence[str]) -> List[str]:
    ...
    for needle in marked:
        ...
        if len(found) != 1:
            raise SystemExit(f"{title!r}: {needle!r} matches {len(found)} lines, expected 1")
...
def _render_pagination_style(pagination: Pagination) -> str:
    if pagination.page_param:
        style = (
            f"page number, {_code(pagination.page_param)} with {_code(pagination.per_page_param)}"
        )
    ...
    style = (
        f"cursor at {_code(pagination.cursor_field)}, sent back as {_code(pagination.cursor_param)}"
    )
...
def _py_seq(values: Sequence[Any], open_: str, close: str, indent: str, prefix: int = 0) -> str:
...
def wire_declarations(pack: str, module: ModuleType) -> str:
    ...
    if oauth:
        ...
        kwargs.append(
            (
                None,
                f"api_key_headers={_py_api_key_headers(first, '    ', 16, key_name or '...')}",
            )
        )

    # Required on both factories, and the reason this block is a runnable
    # equivalent of the pack rather than an illustration of one: without it the
    # call raises before any tool is built.
    kwargs.append((None, f"pack={_py(module.__name__.rsplit('.', 1)[-1])}"))
...
def render_pagination(pack: str, module: ModuleType) -> str:
    ...
    # names long.
    others = f" The other {quiet} take no cursor." if quiet else ""
