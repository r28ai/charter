...
**Set `body_case` explicitly, every time, even when you mean the default.** The
default is Google-shaped, and most APIs are not: Stripe, Slack, OpenAI, Anthropic,
Linear's variables and nearly every API documented with `snake_case` keys want
`body_case="snake"`. Leave it unset against one of those and the first call comes
back `400 Extra inputs are not permitted, field: 'maxTokens'` — the vendor naming
the field you did not send. Copy a request body out of the vendor's own reference
page and read the keys; that is the whole check, and it takes a second.
