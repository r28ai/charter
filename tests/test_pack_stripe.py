...
def test_every_amount_in_the_pack_tells_the_model_it_is_cents():
    ...
    assert not naked, (
        f"Stripe money fields with no Gloss telling the model they are cents: {sorted(naked)}"
    )
...
def test_every_timestamp_in_the_pack_tells_the_model_it_is_seconds():
    ...
    assert not naked, (
        f"Stripe timestamp fields with no Gloss telling the model they are seconds: {sorted(naked)}"
    )
...
async def test_a_paid_checkout_session_needs_line_items():
    with pytest.raises(ToolValidationError, match="line_items"):
        await stripe.checkout_sessions_create.ainvoke(mode="payment", success_url="https://x")


async def test_a_setup_session_needs_a_currency():
    with pytest.raises(ToolValidationError, match="currency"):
        await stripe.checkout_sessions_create.ainvoke(mode="setup", success_url="https://x")
...
def test_pending_invoice_items_behavior_has_two_values():
    ...
    branch = next(b for b in schema["pendingInvoiceItemsBehavior"]["anyOf"] if "enum" in b)
...
def test_connect_is_read_only():
    for tool in stripe.TOOLS:
        if tool.url_template.startswith(
            (
                "v1/accounts",
                "v1/transfers",
                "v1/payouts",
...
def test_cancellation_reason_offers_only_what_may_be_sent():
    ...
    assert set(branch["enum"]) == {"duplicate", "fraudulent", "requested_by_customer", "abandoned"}
...
async def test_clearing_a_scheduled_cancellation_puts_an_empty_value_on_the_wire():
    ...
    await stripe.subscriptions_update.ainvoke({"subscription": "sub_1", "cancel_at": ""})
...
def test_a_subscription_cannot_be_created_with_an_empty_cancel_at():
    ...
    with pytest.raises(ValidationError):
        SubscriptionsCreateRequest(customer="cus_1", items=[{"price": "price_1"}], cancel_at="")
