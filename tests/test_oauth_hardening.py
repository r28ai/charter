...
def _tool(provider, **kw):
    factory = oauth_tool_factory(
        pack="oauth", base_url=API, provider="example", credential_provider=provider, **kw
    )
    return factory(
...
async def test_a_transport_failure_leaves_no_poisoned_state():
    route = respx.post(TOKEN_URL).mock(side_effect=[httpx.ConnectTimeout("down"), _ok()])
    client = _client()
...
async def test_one_clients_cache_is_never_handed_to_another_subject():
    ...
    respx.post(TOKEN_URL).mock(
        side_effect=lambda request: _ok(
            access_token="at-"
            + dict(p.split("=", 1) for p in request.content.decode().split("&"))["refresh_token"]
        )
    )
...
async def test_an_unserialisable_result_does_not_break_the_call():
    ...
    calls = CallCollector()
    factory = oauth_tool_factory(
        pack="oauth", base_url=API, provider="example", credential_provider=_client(), on_call=calls
    )
    tool = factory(
        name="things_get",
        args_schema=Args,
