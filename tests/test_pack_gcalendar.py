...
async def test_all_day_events_keep_their_date():
    ...
    respx.get(EVENTS).mock(
        return_value=httpx.Response(200, json=events_payload(0, items=[all_day]))
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            dict(date_time="2026-09-14T08:45:00+02:00", time_zone="Europe/Paris"),
            id="offset agrees",
        ),
        pytest.param(
            dict(date_time="2026-09-14T08:45:00", time_zone="Europe/Paris"),
            id="naive time plus its zone",
        ),
        pytest.param(dict(date_time="2026-09-14T08:45:00Z"), id="offset with no zone"),
        pytest.param(
            dict(date_time="2026-09-14T08:45:00Z", time_zone="Not/AZone"),
            id="zone we cannot resolve",
        ),
...
def _list_schema(**kwargs):
    return gcalendar.events_list.llm_schema().model_validate({"calendarId": "primary", **kwargs})
...
async def test_a_patch_body_mandates_nothing_while_the_put_beside_it_does():
    ...
    assert sorted(n for n, f in put_body.model_fields.items() if f.is_required()) == [
        "end",
        "start",
    ]
...
async def test_ical_uid_reaches_the_wire_under_the_name_google_documents():
    ...
    await gcalendar.events_list.ainvoke({"calendarId": "primary", "iCalUID": "abc@example.com"})
...
def test_the_rule_covers_every_parameter_the_documentation_names():
    ...
    declared = {
        name for name, f in fields.items() if any(isinstance(m, ConflictsWith) for m in f.metadata)
    }
...
async def test_the_join_link_survives_the_cap():
    ...
    respx.get(f"{API}calendar/v3/calendars/primary/events").mock(
        return_value=httpx.Response(200, json={"items": [_event_with_long_description(1)]})
    )
    result = await gcalendar.events_list.ainvoke(calendar_id="primary")
