...
def _formatted() -> Tool:
    return Tool(
        name="send",
        method="POST",
        url_template="v1/{user_id}/send",
        args_schema=Formatted,
        base_url=BASE,
        api_key_headers={"k": "v"},
    )
...
def _unwrapping() -> Tool:
    return Tool(
        name="create",
        method="POST",
        url_template="v1/{parent}/docs",
        args_schema=Unwrapping,
        base_url=BASE,
        api_key_headers={"k": "v"},
    )
...
def test_pruning_one_branch_does_not_reach_an_identical_type_in_another():
    ...
    t = gdocs.documents_batch_update.derived(name="t", keep={"insert_text", "insert_page_break"})
...
def test_a_negative_price_is_a_drop_that_makes_the_tool_larger():
    ...
    total = schema_tokens(t)
    priced = {
        pc.path: pc.tokens for pc in t.paths("body.requests.insert_text", depth=None, by_cost=True)
    }
    ...
    assert (
        len([d for d in defs if d.endswith("Location_LLM__1") or d.endswith("Location_LLM__2")])
        == 2
    )
...
def test_prices_are_measured_against_the_projection_doing_the_asking():
    ...
    assert (
        next(pc for pc in lean.paths(by_cost=True) if pc.path == "body").tokens
        < next(pc for pc in t.paths(by_cost=True) if pc.path == "body").tokens
    )
...
def test_keep_at_two_levels_does_not_let_one_level_delete_the_other():
    ...
    t = gdocs.documents_batch_update.derived(name="x", keep={"insert_text", "write_control"})
...
def test_keep_still_prunes_below_a_field_that_is_its_own_ancestor():
    ...
    for narrowed in (plain, cyclic):
        assert (
            len(json.dumps(narrowed.to_json_schema())) < len(json.dumps(tool.to_json_schema()))
            or narrowed._prune
        ), "the projection removed nothing"
        with pytest.raises(ToolValidationError):
...
# -----------------------------------------------------
# A projection and a mode are different levers
# -----------------------------------------------------


def test_a_projection_is_the_only_lever_that_can_only_remove():
    """Which is why the mode is a separate method and not a `derived()` keyword.

    `derived()` promises one thing: a projection can only ever remove, and that
    is the property that makes it safe to hand to whoever owns a deployment
    rather than the pack. Resolving a mode rebuilds the view from the
    declarations and can widen. Folding the second into the first would have
    bought one signature and cost the sentence that justifies the first.
    """
    shipped = gcalendar.events_insert
    assert shipped.mode == "write", "this pack no longer declares that mode; pick another"

    def visible(tool):
        return set(egress_map([tool])[report_key(tool)]["visible"])

    narrow = shipped.derived(name="events_insert", drop={"send_updates"})
    assert visible(narrow) < visible(shipped)

    # The mode, by contrast, is not ordered against the tool it came from. The
    # pack withholds `event.i_cal_uid` at `write` and offers it at `import`, and
    # both are the author's own declaration read at a different label.
    widened = shipped.with_mode("import")
    assert "event.i_cal_uid" not in visible(shipped)
    assert "event.i_cal_uid" in visible(widened)


def test_a_mode_keeps_both_halves_of_a_projection_that_keeps_and_pins():
    """Composition in the order a deployment actually writes it.

    A projection is a startup decision and a mode is a per-conversation one, so
    the projection is built once and resolved at a mode per session. A mode that
    rebuilt the view from the pack's declarations and forgot the narrowing would
    come off on exactly the sessions that asked for a tier.
    """
    projected = gdocs.documents_batch_update.derived(
        name="documents_edit_text",
        keep=TEXT,
        pin={"document_id": "1AbC"},
    )
    resolved = projected.with_mode("write")

    assert resolved.name == "documents_edit_text"
    assert resolved._prune == projected._prune
    assert resolved._pins == {"document_id": "1AbC"}
    assert "document_id" not in resolved.llm_schema().model_fields
    assert "document_id" in resolved._exec_schema.model_fields

    members = set(resolved.paths("body.requests"))
    assert members == TEXT, f"the keep came off under a mode: {sorted(members)}"

    # And in the schema the model is actually handed, not only in what `paths`
    # reports: the 30 dropped edits are absent from the published document.
    published = json.dumps(resolved.to_json_schema())
    assert "insertText" in published
    for gone in ("insertTable", "deleteParagraphBullets", "createFooter"):
        assert gone not in published, f"{gone} survived the mode into the published schema"


def test_deriving_from_a_moded_variant_narrows_what_is_left():
    """The two compose in either order, and the second one still only removes."""
    resolved = gdocs.documents_batch_update.with_mode("write")
    narrowed = resolved.derived(name="documents_insert_text", keep={"insert_text"})

    assert narrowed.session_mode == "write", "the deployment's label came off"
    assert narrowed.modes == frozenset({"write"})
    assert set(narrowed.paths("body.requests")) == {"insert_text"}


def test_a_variant_carries_everything_the_copy_path_carries():
    """`derived()` and `with_mode()` are one copy, so neither can drift.

    A projection that quietly lost the pack's response handler returned a
    different shape from the tool it narrowed, which is the bug that put this
    assertion here in the first place; a mode variant is the same copy with a
    different label, and gets the same check.
    """
    source = gdrive.files_list
    variant = source.with_mode("audit")
    ...
    for attribute in carried:
        assert getattr(variant, attribute) == getattr(source, attribute), attribute
    assert variant.args_schema is source.args_schema
    assert variant._response_handler is source._response_handler
    assert variant._build_request is source._build_request
    assert variant._llm_cache is source._llm_cache
    assert variant.credential_provider is source.credential_provider
    # The observability sink especially: a variant that lost it would stop
    # reporting calls, and nothing about the call would look wrong.
    assert variant.on_call is source.on_call
    # The deployment's label lands on its own axis, and the author's `mode`
    # is carried across untouched — a variant of a create is still a create.
    assert variant.session_mode == "audit" and variant.session_mode != source.session_mode
    assert variant.mode == source.mode


def test_a_mode_variant_prepares_and_prices_on_its_own():
    """`prepare()` per mode, or the startup cost moves back into a request.

    Each variant is a separate tool with its own memo, so preparing one says
    nothing about the others and the session has to prepare what it holds.
    """
    source = gdrive.files_list
    variant = source.with_mode("audit")
    assert variant._llm_schema_built is None

    assert variant.prepare() is variant
    assert variant._views_ready and variant._json_schema is not None

    second = source.with_mode("other")
