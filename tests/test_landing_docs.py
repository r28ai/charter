...
def test_the_prose_counts_the_steps_the_blocks_actually_take():
    ...
    assert claim in INDEX.read_text(), (
        f"index.mdx no longer says {claim!r}; the tabs now take {declared} steps "
        f"and {handwritten}"
    )
...
def test_the_caption_counts_the_installs_each_side_needs():
    ...
    assert claim in INDEX.read_text(), (
        f"index.mdx no longer says {claim!r}; the hand-written tab now installs "
        f"{handwritten}"
    )
...
def test_the_pack_counts_in_prose_match_the_package():
    ...
    assert f"one of {_word(len(names))}" in text, f"index.mdx miscounts the packs (now {len(names)})"
...
def test_the_field_split_in_prose_matches_the_egress_map():
    ...
    entry = egress_map([gmail.drafts_create])["drafts_create"]
...
def _quickstart_declaration() -> list[str]:
    ...
    return [
        textwrap.dedent(body) for lang, body in indented.findall(section) if lang == "python"
    ]
...
def test_the_quickstart_steps_build_a_tool_that_sends_what_it_claims():
    ...
    (payload_node,) = [
        node for node in ast.walk(_parse(blocks[-1])) if isinstance(node, ast.Dict)
    ]
    ...
    assert f"?{query}" in QUICKSTART.read_text() or query.replace("%3A", ":") in QUICKSTART.read_text(), (
...
def test_the_prompt_points_at_a_file_that_exists():
    ...
    assert target.is_file(), (
        f"the setup prompt fetches {url}, which no file backs "
