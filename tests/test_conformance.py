...
def _rules(
    ...
    found: Dict[Tuple[str, str], Tuple[str, ...]] = {
        (name, rule): () for rule in set(decorators.model_validators) - _INHERITED
    }
...
def _visible_fields(model: Type[BaseModel], seen: Set[Any] | None = None) -> Dict[str, Set[str]]:
...
def test_no_optional_field_is_secretly_required(pack: str, tool: Tool):
    ...
    for name in required:
        ...
        assert "Optional" not in annotation and "NoneType" not in annotation, (
            f"{pack}.{tool.name}.{name} is Optional yet required — it is missing a `= None` default"
        )
...
def test_no_field_the_vendor_calls_server_owned_reaches_the_model(pack_name):
    ...
    assert not offered, (
        f"{pack_name} offers the model fields the vendor calls server-owned: {sorted(offered)}"
    )
...
def _glosses(model: Type[BaseModel], seen: Set[Any] | None = None) -> Set[str]:
    ...
    for field in model.model_fields.values():
        found.update(
            marker.text for marker in getattr(field, "metadata", []) if isinstance(marker, Gloss)
        )
    for nested in _nested_models(model):
...
def test_every_gloss_a_pack_declares_reaches_the_model(pack_name):
    ...
    for tool in module.TOOLS:
        ...
        lost.extend(
            f"{tool.name}: {text}" for text in declared if json.dumps(text)[1:-1] not in published
        )
