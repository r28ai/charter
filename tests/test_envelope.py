...
def test_api_key_factory_propagates_the_envelope_to_every_tool():
    factory = api_key_tool_factory(
        base_url=BASE, api_key_headers={"x-api-key": "k"}, envelope=SLACKISH
    )
    tools = [
...
def test_factories_default_to_no_envelope():
    ...
    assert factory(name="t", args_schema=Args, method="GET", url_template="v1/{thing}").envelope is None
...
async def test_a_graphql_style_api_needs_one_line():
    ...
    factory = api_key_tool_factory(
        base_url=BASE, api_key_headers={"x-api-key": "k"}, envelope=GRAPHQL_ENVELOPE
    )
...
def test_several_error_paths_can_be_declared_at_once():
    ...
    assert (
        NESTED_ERRORS.failed(
            {"data": {"productCreate": {"userErrors": [{"message": "taken"}]}}}
        )
        is True
    )
