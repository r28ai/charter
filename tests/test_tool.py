...
async def test_401_propagates_as_credential_error_from_ainvoke():
    ...
    with pytest.raises(CredentialError) as excinfo:
        await _gmail_tool().ainvoke(raw={"to": "a@b.com", "subject": "S", "body": "B"})
...
def test_api_key_factory_returns_a_builder():
    factory = api_key_tool_factory(pack="tool", base_url=BASE, api_key_headers={"x-api-key": "k"})
...
def test_api_key_factory_rejects_an_empty_header_value():
    with pytest.raises(ValueError, match="empty value"):
        api_key_tool_factory(pack="tool", base_url=BASE, api_key_headers={"x-api-key": ""})


def test_api_key_factory_requires_headers_somewhere():
    factory = api_key_tool_factory(pack="tool", base_url=BASE)
...
def test_api_key_factory_per_tool_header_override():
    factory = api_key_tool_factory(
        pack="tool", base_url=BASE, api_key_headers={"x-api-key": "default"}
    )
    tool = factory(
        name="t",
        args_schema=GetWeather,
...
def test_factory_case_defaults_and_overrides():
    factory = api_key_tool_factory(
        pack="tool", base_url=BASE, api_key_headers={"x-api-key": "k"}, body_case="pascal"
    )
...
async def test_oauth_factory_tools_share_the_credential_provider():
    ...
    factory = oauth_tool_factory(
        pack="tool", base_url=BASE, provider="google", credential_provider=Counting()
    )
...
def test_factory_accepts_a_resolver_and_skips_build_time_validation():
    factory = api_key_tool_factory(
        pack="tool", base_url=BASE, api_key_headers=lambda: {"x-api-key": "k"}
    )
    tool = factory(name="t", args_schema=GetWeather, method="GET", url_template="v1/{city}")
...
def test_a_factory_builds_a_shared_type_once_for_every_tool_it_makes():
    ...
    elsewhere = api_key_tool_factory("https://api.other.com", {"X-Key": "{api_key}"}, pack="other")
...
def test_a_store_never_holds_a_view_that_cannot_validate():
    ...
    assert all(model.__pydantic_complete__ for _source, model in store.values()), (
...
def test_a_walk_that_raises_leaves_the_store_as_it_found_it():
    ...
    assert store == {}, f"a failed walk left {len(store)} model(s) in the caller's store"
...
def test_a_variant_carries_every_argument_the_constructor_takes():
    """The mechanical guard on the bug that let `llm_cache` go missing.

    `_copy` rebuilds a `Tool` by listing its arguments, so a parameter added to
    `__init__` and forgotten here is silently dropped from every variant. That is
    how projections lost the factory's schema store: nothing failed, they just
    paid for the graph again. A test that reads the signature notices the next
    one.

    One copy path is what keeps this one test enough. `derived` and `with_mode`
    both go through `_copy`, so a second list of arguments cannot fall behind the
    first.
    """
    import inspect

    constructor = set(inspect.signature(Tool.__init__).parameters) - {"self"}
    source = inspect.getsource(Tool._copy)
    passed = {name for name in constructor if f"{name}=" in source}

    missing = constructor - passed
    assert not missing, f"_copy() does not carry: {sorted(missing)}"

    for method in (Tool.derived, Tool.with_mode):
        assert "self._copy(" in inspect.getsource(method), (
            f"{method.__name__} builds a Tool of its own instead of using the copy path"
        )
...
def test_every_recursive_walk_carries_the_whole_walk_state():
    ...
    The walk used to recurse from four places - a nested model, a list item, a
    forward-referenced list item, and a model reached as a dict *value* - each
    having to hand the whole walk down. An internal argument missing from one of
    them failed silently: the subtree below it simply stopped being cached, and
    nothing about the schema it produced was wrong. `_pending` went missing from
    ...
    There is one call site now. `_llm_type` walks the containers and calls back
    through a single closure, which is what made `List[List[Model]]` reachable
    at all, and it retires this whole bug class rather than guarding it: there is
    no longer a second branch to fall out of step with the first. The assertion
    stays because the closure could still drop an argument, and it still reads
    off the source, so a branch added back is covered the day it is written.
    """
    import ast
    import inspect
    import textwrap
    ...
    assert calls, "the walk no longer recurses by name; this guard reads nothing"
...
def test_two_walks_sharing_a_store_do_not_splice_two_graphs_together():
    ...
    slow = threading.Thread(
        target=lambda: built.__setitem__("slow", create_llm_schema(Args, cache=store))
    )
    store.slow = slow
    slow.start()
    ...
    fast = threading.Thread(
        target=lambda: built.__setitem__("fast", create_llm_schema(Args, cache=store))
    )
    fast.start()
...
def test_prepare_pays_the_build_up_front_and_is_idempotent():
    ...
    assert calls[0].schema_ms == 0, "a prepared tool still charged a schema build to its first call"
...
def test_the_resolvability_check_does_not_refuse_a_working_schema():
    ...
    class Args(BaseModel):
        ...
        cells: Annotated[Optional[TypingDict[str, Cell]], Field(None, description="Cells."), Body()]
    ...
    deep = Optional[
        List[
            Optional[
                List[
                    Optional[
                        List[Optional[List[Optional[List[Optional[List[Optional[List[Cell]]]]]]]]]
                    ]
                ]
            ]
        ]
...
def test_the_resolvability_check_sees_every_model_the_walk_builds():
    ...
    assert missed == [], (
        f"the walk built a view for {len(missed)} model(s) the check never visits: {missed[:5]}"
    )
