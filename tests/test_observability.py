...
async def test_the_record_carries_the_template_not_the_resolved_url():
    ...
    respx.get(f"{BASE}v1/things/cus_secret_42").mock(return_value=httpx.Response(200, json={}))
...
async def test_invalid_input_is_recorded_and_never_sent():
    route = respx.get(f"{BASE}v1/things/t1").mock(return_value=httpx.Response(200, json={}))
...
async def test_a_missing_credential_is_recorded_without_a_status_code():
    calls = CallCollector()
    factory = api_key_tool_factory(
        pack="obs", base_url=BASE, api_key_headers=lambda: {"x-api-key": ""}, on_call=calls
    )
    tool = factory(
...
async def test_a_failed_transform_is_recorded_without_a_request():
    ...
    calls = CallCollector()
    factory = api_key_tool_factory(
        pack="obs", base_url=BASE, api_key_headers={"x-api-key": "k"}, on_call=calls
    )
...
async def test_an_http_error_is_recorded_with_its_status():
    respx.get(f"{BASE}v1/things/t1").mock(return_value=httpx.Response(500, json={"error": "boom"}))
...
async def test_a_raising_sink_never_breaks_the_call():
    respx.get(f"{BASE}v1/things/t1").mock(return_value=httpx.Response(200, json={"ok": 1}))
...
async def test_a_shipped_pack_records_without_being_mutated():
    ...
    respx.get(url__regex=r".*labels.*").mock(return_value=httpx.Response(200, json={"labels": []}))
