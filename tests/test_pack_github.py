...
async def test_constant_headers_survive_a_body_carrying_request():
    ...
    await github.issues_create.ainvoke(owner="octocat", repo="hello", body={"title": "Flaky test"})
...
def test_search_pagination_walks_the_items_array():
    ...
    assert (
        github.SEARCH_PAGINATION.next_page_args({"total_count": 9, "items": [_issue(3)]}, previous)
        is None
    )
...
async def test_file_contents_come_back_decoded():
    ...
    out = await github.repos_get_content.ainvoke(owner="octocat", repo="hello", path="app.py")
...
async def test_a_text_file_still_decodes_to_its_text():
    ...
    result = await github.repos_get_content.ainvoke(owner="octocat", repo="hello", path="README.md")
...
async def test_a_binary_file_is_named_as_one_rather_than_decoded_to_mojibake():
    ...
    result = await github.repos_get_content.ainvoke(owner="octocat", repo="hello", path="logo.png")
...
async def test_a_file_over_a_megabyte_says_why_there_is_no_content():
    ...
    respx.get(f"{API}repos/octocat/hello/contents/dump.csv").mock(
        return_value=httpx.Response(
            200,
            json=_content(
                name="dump.csv", path="dump.csv", size=5 * 1024 * 1024, encoding="none", content=""
            ),
        )
    )

    result = await github.repos_get_content.ainvoke(owner="octocat", repo="hello", path="dump.csv")
...
async def test_a_directory_listing_is_unaffected_by_either_branch():
    ...
    result = await github.repos_get_content.ainvoke(owner="octocat", repo="hello", path="src")
...
async def test_a_file_path_keeps_its_slashes():
    ...
    await github.repos_create_or_update_file.ainvoke(
        {"owner": "octocat", "repo": "hello", "path": "a/b/c.txt", "message": "m", "content": "x"}
    )
...
async def test_a_review_request_must_ask_somebody():
    with pytest.raises(ToolValidationError):
        await github.pulls_request_reviewers.ainvoke({"owner": "o", "repo": "r", "pull_number": 1})
...
def test_a_commit_with_no_parents_is_refused_rather_than_written_as_a_root():
    ...
    """
    with pytest.raises(ValidationError) as caught:
        github.git_commits_create.args_schema(owner="o", repo="r", message="m", tree="t")
...
def test_a_label_colour_is_refused_with_the_hash_github_rejects():
    ...
    assert (
        github.issues_create_label.args_schema(
...
_ENUMS: Dict[str, set] = {
    ...
    "CodeScanningSeverity": {"critical", "high", "medium", "low", "warning", "note", "error"},
...
def test_no_tool_offers_the_model_a_field_github_does_not_accept():
    ...
    wrong = []
    for entry in egress_map(github.TOOLS).values():
        name = entry["name"]
        tool = getattr(github, name)
