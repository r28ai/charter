...
def _tool(schema):
    factory = api_key_tool_factory(
        base_url="https://x.test/", pack="t", api_key_headers={"k": "v"}
    )
    return factory(
...
def test_the_map_reports_each_rule_with_both_halves_named_as_the_api_names_them():
    ...
    rules = {r["field"]: r for r in report["listing"]}
...
def test_a_rule_that_can_never_fire_is_reported():
    ...
    assert report["listing"][0]["excludes"][0]["declared"] is False
...
def test_the_shipped_calendar_rules_read_back_whole():
    ...
    assert set(report) == {"events_list", "calendar_list_list"}
    assert len(report["events_list"]) == 8
    assert len(report["calendar_list_list"]) == 2
    assert all(
