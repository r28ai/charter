...
def test_a_fixed_entropy_source_yields_a_known_verifier_and_challenge(monkeypatch):
    ...
    expected_verifier = base64.urlsafe_b64encode(bytes(range(32))).rstrip(b"=").decode("ascii")
    expected_challenge = (
        base64.urlsafe_b64encode(hashlib.sha256(expected_verifier.encode("ascii")).digest())
        .rstrip(b"=")
        .decode("ascii")
    )
...
async def test_the_happy_path_returns_a_grant_with_parsed_scopes():
    route = respx.post(TOKEN_URL).mock(return_value=_exchange_response(scope="scope-a scope-b"))
...
async def test_the_first_call_after_connecting_makes_zero_http_calls():
    client = OAuth2Client.from_grant(SERVER, _grant(), client_id="cid", client_secret="csec")
...
async def test_after_expiry_the_seeded_client_refreshes_normally():
    route = respx.post(TOKEN_URL).mock(return_value=_exchange_response(access_token="at-2"))
    expired = _grant(expires_at=datetime.now(timezone.utc) - timedelta(seconds=1))

    client = OAuth2Client.from_grant(SERVER, expired, client_id="cid", client_secret="csec")
    credentials = await client.get_credentials("example")
...
async def test_a_non_string_refresh_token_is_not_mistaken_for_one(refresh_token):
    ...
    respx.post(TOKEN_URL).mock(return_value=_exchange_response(refresh_token=refresh_token))
