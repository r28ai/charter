...
def _must_fail(check, *args, because: str):
    ...
    try:
        ...
        return
    pytest.fail(f"{check.__name__} passed on broken input — the guard against {because} is vacuous")
...
def test_a_field_rule_whose_every_field_is_withheld_is_not_reported():
    ...
    assert conformance._visible_fields(create_llm_schema(HalfHidden)) == {"HalfHidden": {"shown"}}
