...
class OneOfInput(BaseModel):
    ...
    def _exactly_one_location(self):
        if (self.location is None) == (self.end_of_segment_location is None):
            raise ValueError("Exactly one of `location` or `end_of_segment_location` must be set.")
        return self
...
def test_a_gloss_is_appended_to_the_description_the_model_reads():
    ...
    assert described == (
        "A positive integer in the smallest currency unit. Cents, not dollars: $15.00 is 1500."
    )
...
def test_a_gloss_stands_alone_when_the_field_has_no_description():
    ...
    assert create_llm_schema(Undocumented).model_fields["x"].description == ("Cents, not dollars.")
...
#
# Pydantic titles every field and every model. The runtime drops those and keeps
# anyone else's, which is what makes the pruning safe to run against a pack it
# has never seen.


class _TitleLeaf(BaseModel):
    inner_value: Annotated[str, Body()]


class _Titled(BaseModel):
    max_results: Annotated[int, Body()] = 10
    leaf: Annotated[_TitleLeaf, Body()]
    labelled: Annotated[str, Field(title="RFC 3339 timestamp"), Body()] = ""


def test_pydantic_s_own_titles_are_dropped():
    published = llm_json_schema(create_llm_schema(_Titled))
    assert "title" not in published, "the root kept its class name"
    assert "title" not in published["properties"]["maxResults"]
    for name, definition in published.get("$defs", {}).items():
        assert "title" not in definition, f"{name} kept its class name"


def test_a_title_someone_wrote_survives():
    published = llm_json_schema(create_llm_schema(_Titled))
    assert published["properties"]["labelled"]["title"] == "RFC 3339 timestamp"


def test_dropping_titles_changes_nothing_else():
    """Same schema, one key less. Anything beyond `title` is a regression."""
    view = create_llm_schema(_Titled)
    ...
    assert strip(llm_json_schema(view)) == strip(view.model_json_schema())


def test_a_title_is_matched_against_its_own_key_not_any_key():
    """`labelled` keeps a title that some *other* field would have generated."""

    class _Confusable(BaseModel):
        labelled: Annotated[str, Field(title="Maxresults"), Body()] = ""
        max_results: Annotated[int, Body()] = 0

    published = llm_json_schema(create_llm_schema(_Confusable))
    assert published["properties"]["labelled"]["title"] == "Maxresults"
    assert "title" not in published["properties"]["maxResults"]


def test_no_shipped_tool_sends_a_title():
    """The 8,476 strings this removed, held at zero.

    The walk has to know where it is: Linear declares a field *named* `title`,
    so `"title" in node` is true for its `properties` map without any schema
    carrying a title at all.
    """
    from charter.packs import gmail, linear

    def titles(node, path="$"):
        if isinstance(node, list):
            return [n for i, item in enumerate(node) for n in titles(item, f"{path}[{i}]")]
        if not isinstance(node, dict):
            return []
        found = [path] if "title" in node else []
        for key, value in node.items():
            if key in ("properties", "$defs") and isinstance(value, dict):
                # A map of names to schemas: the names are data, the values are
                # nodes. Never test this level for a `title` key.
                for name, sub in value.items():
                    found += titles(sub, f"{path}.{key}.{name}")
            elif key != "title":
                found += titles(value, f"{path}.{key}")
        return found

    for tool in (*gmail.TOOLS, *linear.TOOLS):
        assert not titles(tool.to_json_schema()["parameters"]), tool.name
