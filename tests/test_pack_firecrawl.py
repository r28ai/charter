...
NOISY_METADATA = {
    ...
    "language": "en",
    **{k: "x" * 60 for k in (
        "og:image", "ogImage", "twitter:image", "og:description", "ogDescription",
        "twitter:description", "og:image:alt", "apple-itunes-app", "go-import",
        "visitor-payload", "octolytics-dimension-user_id", "hovercard-subject-tag",
        "request-id", "html-safe-nonce", "theme-color", "browser-stats-url",
    )},
}
...
async def test_the_markdown_is_never_touched():
    ...
    respx.post(f"{API}scrape").mock(return_value=httpx.Response(200, json={
        "success": True, "data": {"markdown": markdown, "metadata": NOISY_METADATA}}))
...
async def test_the_envelope_stays_where_callers_read_it():
    respx.post(f"{API}scrape").mock(return_value=httpx.Response(200, json={
        "success": True, "data": {"markdown": "# Hi", "metadata": NOISY_METADATA}}))
...
async def test_every_requested_format_survives():
    formats = {"markdown": "# Hi", "html": "<h1>Hi</h1>", "rawHtml": "<html>",
               "links": ["https://a.example"], "summary": "a page",
               "json": {"owner": "sam"}}
    respx.post(f"{API}scrape").mock(return_value=httpx.Response(200, json={
        "success": True, "data": {**formats, "metadata": NOISY_METADATA}}))
...
async def test_search_keeps_its_grouped_shape():
    respx.post(f"{API}search").mock(return_value=httpx.Response(200, json={
        "success": True,
        "data": {"web": [{"markdown": "a", "metadata": NOISY_METADATA}],
...
async def test_map_returns_the_urls_it_found():
    respx.post(f"{API}map").mock(return_value=httpx.Response(200, json={
        "success": True,
        "links": [{"url": "https://a.example", "title": "A", "description": "d",
                   "og:image": "x" * 100}]}))
...
async def test_scrape_keeps_every_documented_content_field():
    ...
    data = (await firecrawl.scrape.ainvoke(url="https://example.com"))["data"]
    for key in (
        "markdown", "answer", "highlights", "branding", "rawBase64",
        "audio", "video", "pages", "actions", "warning",
    ):
...
async def test_nested_scrape_options_on_search_keep_acronyms_and_camel_case():
    ...
    await firecrawl.search.ainvoke(
        ...
        scrape_options={
            ...
            "redact_pii": True,
            "formats": ["markdown", {"type": "json", "prompt": "name", "check_prompt_injection": True}],
        },
    )
...
async def test_pdf_parser_object_camel_cases_page_fields():
    ...
    await firecrawl.scrape.ainvoke(
        ...
        parsers=[{"type": "pdf", "mode": "ocr", "max_pages": 3, "page_markers": True, "pages": True}],
        actions=[
...
async def test_search_legacy_flat_list_is_still_trimmed():
    respx.post(f"{API}search").mock(return_value=httpx.Response(200, json={
        "success": True,
        "data": [{"url": "https://a.example", "title": "A", "markdown": "# A",
                  "metadata": NOISY_METADATA}],
    }))
...
async def test_scrape_status_trims_the_same_way_as_scrape():
    respx.get(f"{API}scrape/job-1").mock(return_value=httpx.Response(200, json={
        "success": True,
...
async def test_crawl_status_trims_each_page_and_keeps_paging():
    respx.get(f"{API}crawl/job-1").mock(return_value=httpx.Response(200, json={
        "status": "completed",
        "total": 2,
        "completed": 2,
...
async def test_batch_scrape_status_trims_each_page():
    respx.get(f"{API}batch/scrape/job-1").mock(return_value=httpx.Response(200, json={
        "status": "completed",
...
async def test_http_error_becomes_an_api_error():
    respx.post(f"{API}scrape").mock(
        return_value=httpx.Response(402, json={"error": "Payment required to access this resource."})
    )
...
async def test_screenshot_format_keeps_viewport_shape():
    route = respx.post(f"{API}scrape").mock(
        return_value=httpx.Response(200, json={"success": True, "data": {"screenshot": "https://cdn.example/s.png"}})
    )

    await firecrawl.scrape.ainvoke(
        url="https://example.com",
        formats=[{"type": "screenshot", "full_page": True, "quality": 80,
...
async def test_extract_status_is_not_trimmed_as_a_scrape_document():
    ...
    respx.get(f"{API}extract/ext-1").mock(return_value=httpx.Response(200, json={
        "success": True,
        "status": "completed",
