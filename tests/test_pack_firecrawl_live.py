...
Reads ``FIRECRAWL_API_KEY`` from the environment, or from a ``.env`` at the repository
...
def _configure_firecrawl() -> None:
    ...
    if not key:
        pytest.skip("FIRECRAWL_API_KEY is not set (environment or .env at the repo root)")
...
async def test_map_search_orders_matching_paths():
    result = await firecrawl.map.ainvoke(
        url="https://docs.firecrawl.dev", search="scrape", limit=10
    )
