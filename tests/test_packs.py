...
def test_named_tools_are_the_same_objects_as_in_TOOLS(pack):
    ...
    for tool in pack.TOOLS:
        assert getattr(pack, tool.name, None) is tool or tool.name in {
            t.name for t in pack.TOOLS
        }
...
async def test_configure_after_construction_reaches_existing_tools(pack, monkeypatch):
    monkeypatch.setattr(pack._headers, "_api_key", None)
    tool, args = _valid_call(pack)
    kwargs, header = CONFIGURE_CALL.get(
        pack.__name__.rsplit(".", 1)[-1], _DEFAULT_CONFIGURE
    )
...
async def test_gmail_messages_send_produces_a_base64url_raw_body():
    ...
    ).mock(return_value=httpx.Response(200, json={"id": "m1", "threadId": "t1"}))
...
async def test_gmail_labels_update_replaces_the_whole_label():
    gmail.configure(StaticTokenProvider("tok"))
    route = respx.put(
        "https://gmail.googleapis.com/gmail/v1/users/me/labels/Label_5"
    ).mock(return_value=httpx.Response(200, json={"id": "Label_5", "name": "Receipts"}))
...
async def test_gmail_labels_patch_sends_only_what_changed():
    ...
    route = respx.patch(
        "https://gmail.googleapis.com/gmail/v1/users/me/labels/Label_5"
    ).mock(return_value=httpx.Response(200, json={"id": "Label_5"}))
...
async def test_gmail_threads_modify_labels_a_whole_conversation():
    ...
    ).mock(return_value=httpx.Response(200, json={"id": "t1", "messages": []}))
...
async def test_gmail_thread_trash_toggles_post_an_empty_body(name):
    ...
    verb = name.split("_")[1]
    route = respx.post(
        f"https://gmail.googleapis.com/gmail/v1/users/me/threads/t1/{verb}"
    ).mock(return_value=httpx.Response(200, json={"id": "t1", "messages": []}))
...
async def test_calendar_events_insert_post_casing():
    ...
    route = respx.post(
        "https://www.googleapis.com/calendar/v3/calendars/primary/events"
    ).mock(return_value=httpx.Response(200, json={"id": "e1"}))
...
async def test_sheets_values_get():
    ...
    route = respx.get(
        "https://sheets.googleapis.com/v4/spreadsheets/s1/values/Sheet1!A1:C2"
    ).mock(return_value=httpx.Response(200, json={"values": [["a"]]}))
...
async def test_sheets_render_options_reach_google_under_the_names_it_documents():
    gsheets.configure(StaticTokenProvider("tok"))
    route = respx.get(
        "https://sheets.googleapis.com/v4/spreadsheets/s1/values/A1:B2"
...
async def test_gmail_label_lists_drop_the_counters_and_render_hints():
    ...
    raw = {
        "labels": [
            {
                "id": f"Label_{i}", "name": f"n{i}", "type": "user",
                "messagesTotal": 12, "messagesUnread": 3,
                "threadsTotal": 9, "threadsUnread": 2,
...
async def test_a_created_label_reads_back_like_a_fetched_one():
    ...
    assert set(handlers) == {"labels_list", "labels_get", "labels_create",
                             "labels_update", "labels_patch"}
    assert len({handlers[n] for n in ("labels_get", "labels_create",
                                      "labels_update", "labels_patch")}) == 1
