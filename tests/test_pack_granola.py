...
async def test_the_no_argument_call_sends_nothing_it_was_not_asked_to(tool):
    ...
    assert json.loads(body or "{}") == ({k: v for k, v in args.items() if k in ("url", "scopes")})
...
@pytest.mark.parametrize(
    "tool",
    [granola.webhook_endpoints_create, granola.webhook_endpoints_update],
    ids=lambda t: t.name,
)
...
def test_the_egress_map_is_exactly_what_granola_accepts():
    ...
    visible = {entry["name"]: set(entry["visible"]) for entry in egress_map(granola.TOOLS).values()}
    assert visible == {
        "notes_list": {
            "created_before",
            "created_after",
...
async def test_the_paging_signal_survives_the_trim():
    ...
    respx.get(f"{API}v1/notes/not_1d3tmYTlCICgjy/transcript").mock(
        return_value=httpx.Response(
            200,
            json={
                "transcript": [MACOS_ITEM],
                "hasMore": True,
                "cursor": "eyJyb3dfb2Zmc2V0Ijo1MH0=",
            },
        )
    )
    page = await granola.notes_transcript_get.ainvoke(note_id="not_1d3tmYTlCICgjy")
...
async def test_an_empty_transcript_page_still_reports_whether_more_follows():
    respx.get(f"{API}v1/notes/not_1d3tmYTlCICgjy/transcript").mock(
        return_value=httpx.Response(200, json={"transcript": [], "hasMore": False, "cursor": None})
    )
    page = await granola.notes_transcript_get.ainvoke(note_id="not_1d3tmYTlCICgjy")
...
async def test_a_note_returns_its_summary_once_rather_than_twice():
    respx.get(f"{API}v1/notes/not_1d3tmYTlCICgjy").mock(return_value=httpx.Response(200, json=NOTE))
...
async def test_a_note_with_no_markdown_keeps_the_text_it_does_have():
    ...
    respx.get(f"{API}v1/notes/not_1d3tmYTlCICgjy").mock(
        return_value=httpx.Response(
            200, json={**NOTE, "summary_markdown": None, "private_notes_markdown": None}
        )
    )
...
async def test_an_inline_transcript_is_trimmed_the_same_way():
    ...
    note = await granola.notes_get.ainvoke(note_id="not_1d3tmYTlCICgjy", include="transcript")
...
async def test_an_audit_page_comes_back_untrimmed():
    ...
    route = respx.get(url__startswith=f"{API}v1/audit").mock(
        return_value=httpx.Response(
            200, json={"events": [AUDIT_EVENT], "hasMore": False, "cursor": None}
        )
    )
...
async def test_an_audit_walk_does_not_stop_on_a_short_page():
    ...
    """
    pages = [
        httpx.Response(200, json={"events": [AUDIT_EVENT], "hasMore": True, "cursor": "page-two"}),
...
async def test_a_two_page_walk_advances_and_then_stops():
    pages = [
        httpx.Response(
            200,
            json={"notes": [{"id": "not_1d3tmYTlCICgjy"}], "hasMore": True, "cursor": "page-two"},
        ),
        httpx.Response(
            200, json={"notes": [{"id": "not_2d3tmYTlCICgjz"}], "hasMore": False, "cursor": "stale"}
        ),
    ]
