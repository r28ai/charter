...
def test_the_covenant_is_present_verbatim():
    ...
    covenant = (
        "Charter runs where you run it. No telemetry, no phone-home, no training on your "
        "traffic —\nthe library makes no network calls except the API calls you define. "
        "MIT, forever."
    )
...
def test_readme_leads_with_the_three_controls():
    """The frame is load-bearing positioning, not decoration. If it drifts back
    to mechanics-only, that should be a deliberate edit, not an accident."""
    text = README.read_text()
    for control in ("Egress —", "Action —", "Change —"):
        assert control in text, f"missing control: {control}"
    # And the claim underneath each one.
    assert "data minimisation" in text
    assert "chokepoint every action" in text
