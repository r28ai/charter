...
def _check_pin_values(exec_schema: Type[BaseModel], pins: Dict[str, Any]) -> None:
    ...
    for path, value in pins.items():
        field = exec_schema.model_fields.get(path)
        if field is None:
            continue
        try:
...
class Tool:
    ...
    def __init__(
        ...
        *,
        ...
        _check_schema_resolvable(
            args_schema, name, getattr(llm_cache, "verified", None)
        )
    ...
    def paths(
        ...
        *,
        ...
        eight names was not enough on its own. ``search_issues`` costs 47,026
        ...
        # difference is not a micro-optimisation: `search_issues` has 2,271,553
        ...
        costs = path_costs(self.args_schema, absolute, mode=self.mode, prune=pruned)
    ...
    def derived(
        ...
        *,
        ...
        Raises:
            DeclarationError: a selector names nothing, names several things, or
                would drop a field the API requires.
        """
        ...
        return Tool(
            ...
            # The projection reads the same type graph as the tool it narrows,
            # so it reads the same store. Only subtrees no prune path reaches
            # are shared, which is exactly the set the extra prune cannot
            # change - the condition `path_costs` already relies on to price a
            # level. Dropped here, every projection rebuilt the graph alone:
            # Linear ships 17 of them, and the pack's own curated list tools are
            # all projections, so the tools most likely to be exposed were the
            # ones paying full price.
            ...
            with self._schema_lock:
                ...
                if cached is None:
                    cached = {
                        ...
                        "parameters": self._llm_schema.model_json_schema(),
                    }
        ...
        *,
        ...
        try:
            call = ToolCall.from_probe(
                ...
                context_bytes=(
                    _measure(result) if probe.observed and error is None else None
                ),
            )
            ...
            for sink in self._sinks():
                try:
                    ...
                    logger.debug(
                        "charter call sink failed for %s", self.name, exc_info=True
                    )
    ...
    def __repr__(self) -> str:
        return f"Tool(name={self.name!r}, method={self.method!r}, url_template={self.url_template!r})"
