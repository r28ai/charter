...
async def extract_messages(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    out: Dict[str, Any] = {
        "messages": [_trim_message(m) for m in response.get("messages") or []]
    }
...
async def extract_channels(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        "channels": [_trim_channel(c) for c in response.get("channels") or []]
    }
    cursor = _cursor(response)
    if cursor:
        out["next_cursor"] = cursor
...
async def extract_users(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        "members": [_trim_user(m) for m in response.get("members") or []]
    }
    cursor = _cursor(response)
    if cursor:
        out["next_cursor"] = cursor
