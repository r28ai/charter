...
class DecryptionMetadata(BaseModel):
    ...
    wrapped_key: Optional[str] = Field(
        None,
        description=(
            "The URL-safe Base64 encoded wrapped key used to encrypt the contents "
            "of the file."
        ),
    )
...
class FileCapabilities(BaseModel):
    ...
    can_share: Optional[bool] = Field(
        None,
        description=(
            "Output only. Whether the current user can modify the sharing settings "
            "for this file."
        ),
    )
...
class File(BaseModel):
    ...
    sharing_user: Annotated[
        ...
        Field(
            None,
            description=(
                "Output only. The user who shared the file with the requesting "
                "user, if applicable."
            ),
        ),
        Mode("response_only"),
    ...
    link_share_metadata: Annotated[
        ...
        Field(
            None,
            description=(
                "Contains details about the link URLs that clients are using to "
                "refer to this item."
            ),
        ),
        Mode("response_only"),
