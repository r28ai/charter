...
def _trim_permission(permission: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in ("emailAddress", "domain", "displayName", "expirationTime", "deleted", "pendingOwner"):
...
async def extract_comments(response: Dict[str, Any]) -> Dict[str, Any]:
    out: Dict[str, Any] = {
        "comments": [_trim_comment(c) for c in response.get("comments") or [] if isinstance(c, dict)]
    }
    cursor = _cursor(response)
    if cursor:
        out["nextPageToken"] = cursor
...
async def extract_about(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(quota, dict):
        kept = {k: quota[k] for k in ("limit", "usage", "usageInDrive", "usageInDriveTrash") if quota.get(k)}
        if kept:
