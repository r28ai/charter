...
class ProductCreateInput(BaseModel):
    ...
    description_html: Optional[str] = Field(
        None, description="The product description, as HTML."
    )
    ...
    status: Optional[ProductStatus] = Field(
        None,
        description=(
            "The product's status. Defaults to ACTIVE. DRAFT keeps it out of the "
            "storefront."
        ),
    )
    handle: Optional[str] = Field(
        None,
        description=(
            "The URL slug for the product. Derived from the title when omitted."
        ),
    )
...
class ProductUpdateInput(BaseModel):
    ...
    description_html: Optional[str] = Field(
        None, description="A new description, as HTML."
    )
...
class OrdersListVariables(ConnectionVariables):
    ...
    query: Optional[str] = Field(
        ...
        description=(
            ...
            "`financial_status:paid fulfillment_status:unshipped`. ""Use only the qualifiers listed: Shopify ignores one it does not recognise instead of rejecting it, so a typo like `vendorr:` returns the whole unfiltered set rather than an error. "
...
class CustomerInput(BaseModel):
    ...
    phone: Optional[str] = Field(
        None, description="The customer's phone number, in E.164 format."
    )
...
class CustomerUpdateInput(BaseModel):
    ...
    id: str = Field(
        ..., description="The customer's global ID, `gid://shopify/Customer/...`."
    )
    ...
    tags: Optional[List[str]] = Field(
        None,
        description=(
            "Tags for the customer. This replaces the existing tags rather than "
            "adding to them."
        ),
    )
...
class OrderFulfillmentOrdersVariables(BaseModel):
    ...
    first: int = Field(
        10, ge=1, le=50, description="How many fulfillment orders to read."
    )
...
class FulfillmentCreateVariables(BaseModel):
    ...
    message: Optional[str] = Field(
        None, description="A message recorded against the fulfillment."
    )
...
class InventoryChange(BaseModel):
    ...
    delta: int = Field(
        ..., description="How much to add or remove. Negative reduces the count."
    )
...
class InventoryAdjustInput(BaseModel):
    ...
    name: InventoryStateName = Field(
        ..., description="Which stock state to move."
    )
    ...
    changes: List[InventoryChange] = Field(
        ..., min_length=1, description="The movements to apply."
    )
...
class VariantInventoryLevelVariables(BaseModel):
    ...
    id: str = Field(
        ..., description="The variant's global ID, `gid://shopify/ProductVariant/...`."
    )
    location_id: str = Field(
        ..., description="The location's global ID, `gid://shopify/Location/...`."
    )
...
class OrderCancelVariables(BaseModel):
    ...
    restock: bool = Field(
        ..., description="Whether to return the items to inventory."
    )
    notify_customer: Optional[bool] = Field(
        None, description="Email the customer. Absent, Shopify does not."
    )
...
class DraftOrderCompleteVariables(BaseModel):
    ...
    source_name: Optional[str] = Field(
        None, description="Where the resulting order came from."
    )
