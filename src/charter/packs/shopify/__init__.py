...
customer_update = _mutation(
    "customer_update", CustomerUpdateRequest, queries.CUSTOMER_UPDATE, "customerUpdate",
...
order_fulfillment_orders = _query(
    "order_fulfillment_orders", OrderFulfillmentOrdersRequest,
    queries.ORDER_FULFILLMENT_ORDERS, "order",
    (
...
fulfillment_create = _mutation(
    "fulfillment_create", FulfillmentCreateRequest, queries.FULFILLMENT_CREATE,
    "fulfillmentCreate",
    (
...
inventory_adjust_quantities = _mutation(
    "inventory_adjust_quantities", InventoryAdjustRequest,
    queries.INVENTORY_ADJUST_QUANTITIES, "inventoryAdjustQuantities",
    (
...
variant_inventory_level = _query(
    "variant_inventory_level", VariantInventoryLevelRequest,
    queries.VARIANT_INVENTORY_LEVEL, "productVariant",
    (
...
locations_list = _query(
    "locations_list", LocationsRequest, queries.LOCATIONS, "locations",
...
order_close = _mutation(
    "order_close", OrderCloseRequest, queries.ORDER_CLOSE, "orderClose",
    "Close an order, marking it finished. Closing does not cancel or refund anything.",
    "Closes a Shopify order.",
)

order_open = _mutation(
    "order_open", OrderOpenRequest, queries.ORDER_OPEN, "orderOpen",
    "Reopen a closed order.",
    "Reopens a Shopify order.",
)

order_cancel = _mutation(
    "order_cancel", OrderCancelRequest, queries.ORDER_CANCEL, "orderCancel",
    (
...
draft_order_create = _mutation(
    "draft_order_create", DraftOrderCreateRequest, queries.DRAFT_ORDER_CREATE,
    "draftOrderCreate",
    (
...
draft_order_complete = _mutation(
    "draft_order_complete", DraftOrderCompleteRequest, queries.DRAFT_ORDER_COMPLETE,
    "draftOrderComplete",
    (
        "Turn a draft order into a real one. This reserves inventory, so check "
        "stock first."
    ),
...
product_variants_bulk_create = _mutation(
    "product_variants_bulk_create", ProductVariantsBulkCreateRequest,
    queries.PRODUCT_VARIANTS_BULK_CREATE, "productVariantsBulkCreate",
    (
