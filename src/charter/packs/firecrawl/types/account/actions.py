...
class ActivityListRequest(BaseModel):
    ...
    limit: Annotated[
        Optional[int],
        Field(None, ge=1, le=100, description="Maximum number of results per page. The server applies 50 when this is absent."),
        Query(),
    ]
    cursor: Annotated[
        Optional[str],
        Field(None, description="Cursor for pagination. Use the cursor value from the previous response."),
        Query(),
    ]
...
class ThreatProtectionUpdateRequest(BaseModel):
    ...
    risk_score_threshold: Annotated[
        Optional[int],
        Field(None, ge=0, le=100, description="Normalized score (0-100) at or above which a classifier verdict is blocked."),
        Body(),
    ]
    blacklist: Annotated[
        Optional[List[str]],
        Field(None, description="Exact domains or globs always blocked, without a classifier call."),
        Body(),
    ]
    whitelist: Annotated[
        Optional[List[str]],
        Field(None, description="Exact domains or globs always allowed. Wins over every other rule."),
        Body(),
    ]
    blocked_tlds: Annotated[
        Optional[List[str]],
        Field(None, description="Top-level domains to block outright, lowercase without a leading dot."),
        Body(),
    ]
    failure_policy: Annotated[
        Optional[ThreatProtectionFailurePolicy],
        Field(None, description="Behavior when the classifier is unreachable. The server applies 'closed' when this is absent."),
        Body(),
    ]
