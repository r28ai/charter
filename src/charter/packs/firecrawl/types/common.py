...
class Action(BaseModel):
    ...
    milliseconds: Optional[int] = Field(None, ge=1, description="Number of milliseconds to wait. Only applicable to 'wait'.")
    selector: Optional[str] = Field(None, description="CSS selector for the target element.")
    full_page: Optional[bool] = Field(None, description="Whether to capture a full-page screenshot.")
    quality: Optional[int] = Field(None, description="The quality of the screenshot, from 1 to 100.")
    ...
    ] = Field(None, description="The page size of the resulting PDF. The server applies 'Letter' when this is absent.")
    landscape: Optional[bool] = Field(None, description="Whether to generate the PDF in landscape orientation.")
...
class AuditMetadata(BaseModel):
    ...
    username: str = Field(..., max_length=1024, description="The username associated with the request.")
...
class RedactPIIOptions(BaseModel):
    ...
    entities: Optional[List[Literal["PERSON", "EMAIL", "PHONE", "LOCATION", "FINANCIAL", "SECRET"]]] = Field(
        None,
...
class Webhook(BaseModel):
    ...
    headers: Optional[Dict[str, str]] = Field(None, description="Headers to send to the webhook URL.")
...
class ScrapeOptionsNested(BaseModel):
    ...
    skip_tls_verification: Optional[bool] = Field(None, description="Skip TLS certificate verification when making requests.")
    ...
    parsers: Optional[List[ParserItem]] = Field(None, description="Controls how files are processed during scraping.")
    actions: Optional[List[Action]] = Field(None, description="Actions to perform on the page before grabbing the content.")
    location: Optional[ScrapeLocation] = Field(None, description="Location settings for the request.")
    remove_base64_images: Optional[bool] = Field(None, description="Removes all base64 images from the markdown output.")
    block_ads: Optional[bool] = Field(None, description="Enables ad-blocking and cookie popup blocking.")
    proxy: Optional[Literal["basic", "enhanced", "auto"]] = Field(None, description="Specifies the type of proxy to use.")
    store_in_cache: Optional[bool] = Field(None, description="If true, the page will be stored in the Firecrawl index and cache.")
...
class ScrapeOptionsMixin(BaseModel):
    ...
    formats: Annotated[
        ...
        Field(None, description="Output formats to include in the response. Strings or objects. The server applies markdown when this is absent."),
        Body(),
    ]
    only_main_content: Annotated[
        Optional[bool],
        Field(None, description="Only return the main content of the page excluding headers, navs, footers, etc. The server applies true when this is absent."),
        Body(),
    ]
    only_clean_content: Annotated[
        Optional[bool],
        Field(None, description="Beta. LLM pass over markdown to remove residual boilerplate that onlyMainContent can miss."),
        Body(),
    ]
    include_tags: Annotated[Optional[List[str]], Field(None, description="Tags to include in the output."), Body()]
    exclude_tags: Annotated[Optional[List[str]], Field(None, description="Tags to exclude from the output."), Body()]
    max_age: Annotated[
        Optional[int],
        Field(None, description="Returns a cached version of the page if it is younger than this age in milliseconds. The server applies 172800000 (2 days) when this is absent."),
        Body(),
    ]
    min_age: Annotated[
        Optional[int],
        Field(None, description="When set, the request only checks the cache and never triggers a fresh scrape."),
        Body(),
    ]
    headers: Annotated[
    ...
    wait_for: Annotated[
        Optional[int],
        Field(None, description="Specify a delay in milliseconds before fetching the content. The server applies 0 when this is absent."),
        Body(),
    ]
    mobile: Annotated[
        Optional[bool],
    ...
    timeout: Annotated[
        Optional[int],
        Field(None, ge=1000, le=300000, description="Timeout in milliseconds. The server applies 60000 when this is absent."),
        Body(),
    ]
    parsers: Annotated[
    ...
    store_in_cache: Annotated[
        Optional[bool],
        Field(None, description="If true, the page will be stored in the Firecrawl index and cache."),
        Body(),
    ]
    lockdown: Annotated[
        Optional[bool],
        Field(None, description="Serve from cache only and never make an outbound request. On miss, returns 404 SCRAPE_LOCKDOWN_CACHE_MISS."),
        Body(),
    ]
    redact_pii: Annotated[
        Optional[RedactPII],
        Field(None, description="Redact personally identifiable information from returned markdown. Pass true for defaults, or an object to tune it."),
        Body(),
        WireName("redactPII"),
    ]
    profile: Annotated[
...
class CrawlTargetOptions(BaseModel):
    ...
    include_paths: Optional[List[str]] = Field(None, description="URL pathname regex patterns to include.")
    exclude_paths: Optional[List[str]] = Field(None, description="URL pathname regex patterns to exclude.")
