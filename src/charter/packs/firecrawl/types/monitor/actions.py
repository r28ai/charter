...
class MonitorSchedule(BaseModel):
    ...
    cron: Optional[str] = Field(None, description="Five-field cron expression. Minimum interval is 5 minutes.")
...
class MonitorWebhook(BaseModel):
    ...
    metadata: Optional[dict] = Field(None, description="Custom metadata included in webhook payloads.")
...
class MonitorEmailNotification(BaseModel):
    ...
    recipients: Optional[List[str]] = Field(None, max_length=25, description="Email recipients for monitor alerts.")
    include_diffs: Optional[bool] = Field(None, description="Include changed page details in email summaries.")


class MonitorNotification(BaseModel):
    """Notification destinations for a monitor."""

    email: Optional[MonitorEmailNotification] = Field(None, description="Email notification settings.")


class MonitorScrapeTarget(BaseModel):
    """Monitor target that scrapes one or more URLs on each check."""

    id: Optional[str] = Field(None, description="Optional stable ID for this target. Generated if omitted.")
    ...
    scrape_options: Optional[ScrapeOptionsNested] = Field(None, description="Scrape options applied to each URL.")
...
class MonitorCrawlTarget(BaseModel):
    ...
    crawl_options: Optional[CrawlTargetOptions] = Field(None, description="Crawl options such as limit and includePaths.")
    scrape_options: Optional[ScrapeOptionsNested] = Field(None, description="Scrape options applied to each crawled page.")
...
class MonitorSearchTarget(BaseModel):
    ...
    include_domains: Optional[List[str]] = Field(None, max_length=50, description="Restrict results to these domains.")
    exclude_domains: Optional[List[str]] = Field(None, max_length=50, description="Drop results from these domains.")
...
class MonitorCreateRequest(BaseModel):
    ...
    schedule: Annotated[MonitorSchedule, Field(..., description="Schedule for monitor checks."), Body()]
    targets: Annotated[
        List[MonitorTarget],
        Field(..., min_length=1, max_length=50, description="Targets to run on each check."),
        Body(),
    ]
    webhook: Annotated[Optional[MonitorWebhook], Field(None, description="Webhook destination for monitor events."), Body()]
    notification: Annotated[Optional[MonitorNotification], Field(None, description="Notification destinations."), Body()]
    retention_days: Annotated[
        Optional[int],
        Field(None, ge=1, le=365, description="How long to retain monitor history. The server applies 30 when this is absent."),
        Body(),
    ]
    goal: Annotated[
        Optional[str],
...
class MonitorListRequest(BaseModel):
    ...
    limit: Annotated[
        Optional[int],
        Field(None, ge=1, le=100, description="Maximum monitors to return. The server applies 25 when this is absent."),
        Query(),
    ]
    offset: Annotated[
        Optional[int],
        Field(None, ge=0, description="Number of monitors to skip. The server applies 0 when this is absent."),
        Query(),
    ]
...
class MonitorUpdateRequest(BaseModel):
    ...
    schedule: Annotated[Optional[MonitorSchedule], Field(None, description="Schedule for monitor checks."), Body()]
    webhook: Annotated[Optional[MonitorWebhook], Field(None, description="Webhook destination for monitor events."), Body()]
    notification: Annotated[Optional[MonitorNotification], Field(None, description="Notification destinations."), Body()]
    targets: Annotated[
    ...
    retention_days: Annotated[Optional[int], Field(None, ge=1, le=365, description="How long to retain monitor history."), Body()]
    goal: Annotated[
        Optional[str],
        Field(None, max_length=2000, description="Plain-language goal used to judge whether changed pages are meaningful."),
        Body(),
    ]
    judge_enabled: Annotated[Optional[bool], Field(None, description="Whether to judge changed pages against goal."), Body()]
    status: Annotated[Optional[Literal["active", "paused"]], Field(None, description="Monitor status."), Body()]
...
class MonitorChecksListRequest(BaseModel):
    ...
    limit: Annotated[
        Optional[int],
        Field(None, ge=1, le=100, description="Maximum checks to return. The server applies 25 when this is absent."),
        Query(),
    ]
    offset: Annotated[
        Optional[int],
        Field(None, ge=0, description="Number of checks to skip. The server applies 0 when this is absent."),
        Query(),
    ]
    status: Annotated[
...
class MonitorCheckGetRequest(BaseModel):
    ...
    limit: Annotated[
        Optional[int],
        Field(None, ge=1, le=100, description="Maximum page results to return. The server applies 25 when this is absent."),
        Query(),
    ]
    skip: Annotated[
        Optional[int],
        Field(None, ge=0, description="Number of page results to skip. The server applies 0 when this is absent."),
        Query(),
    ]
    status: Annotated[
