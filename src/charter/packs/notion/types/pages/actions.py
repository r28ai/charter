...
class PageCreateBody(BaseModel):
    ...
    properties: Optional[Dict[str, PagePropertyValue]] = Field(None, description=PROPERTIES)
...
class PageUpdateBody(BaseModel):
    ...
    cover: Optional[Cover] = Field(None, description="The page's cover image. Null removes it.")
    ...
    template: Optional[Template] = Field(None, description="A template to apply to the page.")
...
class PagesUpdateRequest(BaseModel):
    ...
    page_id: Annotated[str, Field(..., description=PAGE_ID), Path()]
    body: Annotated[PageUpdateBody, Field(..., description="The fields to change."), Body()]
    filter_properties: Annotated[
        Optional[List[str]],
        Field(
            None,
...
class PagesRetrievePropertyItemRequest(BaseModel):
    ...
    start_cursor: Annotated[
        ...
        Field(
            None,
            description=(
                "A `next_cursor` from a previous response, for the properties that paginate."
            ),
        ),
        Query(),
    ]
...
class ReplaceContentOp(BaseModel):
    ...
    allow_deleting_content: Optional[bool] = Field(
        None,
        description=(
            "Whether the replacement may delete child pages or databases. Refused when absent."
        ),
    )
...
class PagesUpdateMarkdownRequest(BaseModel):
    ...
    page_id: Annotated[str, Field(..., description=PAGE_ID), Path()]
    body: Annotated[PageMarkdownUpdateBody, Field(..., description="The new content."), Body()]
