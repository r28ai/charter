...
class CommentAttachment(BaseModel):
    ...
    type: Optional[Literal["file_upload"]] = Field(
        None, description="Always `file_upload`."
    )
...
class CommentCreateBody(BaseModel):
    ...
    parent: Annotated[
        ...
        Field(
            None,
            description=(
                "The page or block to comment on, starting a new discussion there."
            ),
        ),
        ConflictsWith(
            "discussion_id",
    ...
    rich_text: Annotated[
        ...
        ConflictsWith(
            "markdown", reason="Content comes as rich text or as Markdown, not both."
        ),
    ]
...
class CommentsCreateRequest(BaseModel):
    ...
    body: Annotated[
        CommentCreateBody, Field(..., description="The comment to post."), Body()
    ]
...
class CommentUpdateBody(BaseModel):
    ...
    rich_text: Annotated[
        Optional[List[RichText]],
        Field(
            None, max_length=100, description="The comment's new content, as rich text."
        ),
        ConflictsWith(
            "markdown", reason="Content comes as rich text or as Markdown, not both."
        ),
...
class CommentsUpdateRequest(BaseModel):
    ...
    comment_id: Annotated[str, Field(..., description=COMMENT_ID), Path()]
    body: Annotated[
        CommentUpdateBody, Field(..., description="The new content."), Body()
    ]
