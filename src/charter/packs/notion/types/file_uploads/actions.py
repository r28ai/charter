...
class FileUploadCreateBody(BaseModel):
    ...
    * ``external_url`` — needs ``filename`` and ``external_url``, and Notion
    ...
    content_type: Optional[str] = Field(
        default=None,
        description=(
            "The MIME type of the file. It must match the bytes and any extension on `filename`."
        ),
    )
    ...
    external_url: Optional[str] = Field(
        default=None,
        description=(
            "A public HTTPS URL for Notion to import. Required for an `external_url` upload."
        ),
    )
    ...
    def _mode_is_satisfied(self):
        if self.mode == "external_url":
            missing = [n for n in ("filename", "external_url") if getattr(self, n) is None]
            if missing:
                raise ValueError(f"mode='external_url' needs {' and '.join(missing)}")
        if self.mode == "multi_part":
            missing = [n for n in ("filename", "number_of_parts") if getattr(self, n) is None]
            if missing:
