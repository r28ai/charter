...
def exactly_one_of(model: Any, discriminator: str, variants: tuple) -> Any:
    ...
    if present and declared is not None and declared != present[0]:
        raise ValueError(
            f"{discriminator}={declared!r} does not name the `{present[0]}` object given"
        )
    return model
...
class Annotations(BaseModel):
    ...
    strikethrough: Optional[bool] = Field(None, description="Whether the text is struck through.")
    underline: Optional[bool] = Field(None, description="Whether the text is underlined.")
    code: Optional[bool] = Field(None, description="Whether the text is code style.")
...
class DateValue(BaseModel):
    ...
    start: str = Field(
        ...
        None,
        description=(
            "An ISO 8601 formatted date, with optional time. Null if this is not a range."
        ),
    )
...
class PartialUser(BaseModel):
    ...
    object: Optional[Literal["user"]] = Field("user", description="Always `user`.")
...
class Mention(BaseModel):
    ...
    link_preview: Optional[LinkPreviewRef] = Field(None, description="The mentioned link preview.")
...
class Icon(BaseModel):
    ...
    type: Optional[Literal["emoji", "external", "file_upload", "custom_emoji", "icon"]] = Field(
        None, description="The type of this icon."
    )
    emoji: Optional[str] = Field(None, description="A standard emoji character, for example `🥬`.")
    external: Optional[ExternalFileData] = Field(
    ...
    icon: Optional[NotionIconData] = Field(None, description="One of Notion's own built-in icons.")
    file: Annotated[
        Optional[NotionHostedFileData],
        Field(
            None,
...
class Parent(BaseModel):
    ...
    workspace: Annotated[
        ...
        Field(
            None,
            description=("Always true, on an object that sits at the top level of the workspace."),
        ),
        Mode("response_only"),
    ]
    agent_id: Annotated[
...
class PaginatedQuery(BaseModel):
    ...
    start_cursor: Annotated[Optional[str], Field(None, description=START_CURSOR), Query()]
    page_size: Annotated[
        Optional[int],
