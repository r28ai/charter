...
_CONDITIONS = (
    ...
    "unique_id", "checkbox", "select", "status", "multi_select", "date",
    "created_time", "last_edited_time", "people", "created_by",
    "last_edited_by", "files", "relation", "formula", "rollup", "verification",
)
...
_PROPERTY_TYPES = (
    ...
    "last_edited_time", "last_edited_by", "last_visited_time", "unique_id",
    "verification", "button", "location", "place",
)
...
class NumberCondition(ExistenceCondition):
    ...
    greater_than_or_equal_to: Optional[float] = Field(
        None, description="An inclusive lower bound."
    )
    less_than_or_equal_to: Optional[float] = Field(
        None, description="An inclusive upper bound."
    )
...
class CheckboxCondition(BaseModel):
    ...
    equals: Optional[bool] = Field(
        None, description="Whether the checkbox is checked."
    )
...
class DateCondition(ExistenceCondition):
    ...
    equals: Optional[str] = Field(
        None, description=f"An ISO 8601 date to match. {RELATIVE_DATES}"
    )
...
class SelectCondition(ExistenceCondition):
    ...
    equals: Optional[Union[str, List[str]]] = Field(
        None,
        description=(
            "The option name to match. A list matches any of several options."
        ),
    )
...
class PeopleCondition(ExistenceCondition):
    ...
    does_not_contain: Optional[str] = Field(
        None,
        description=(
            "The ID of a user the value must not include. `me` is accepted here too."
        ),
    )
...
class RelationCondition(ExistenceCondition):
    ...
    contains: Optional[str] = Field(
        None, description="The ID of a page the relation must include."
    )
    does_not_contain: Optional[str] = Field(
...
class Filter(BaseModel):
    ...
    * a **timestamp filter** — ``timestamp`` naming ``created_time`` or
    ...
    type: Optional[str] = Field(
        None, description="The type of the property being filtered on."
    )

    title: Optional[TextCondition] = Field(None, description="A condition on the title.")
    rich_text: Optional[TextCondition] = Field(
        None, description="A condition on a text property."
    )
    url: Optional[TextCondition] = Field(None, description="A condition on a URL property.")
    email: Optional[TextCondition] = Field(
        None, description="A condition on an email property."
    )
    phone_number: Optional[TextCondition] = Field(
        None, description="A condition on a phone number property."
    )
    number: Optional[NumberCondition] = Field(
        None, description="A condition on a number property."
    )
    ...
    select: Optional[SelectCondition] = Field(
        None, description="A condition on a select property."
    )
    status: Optional[SelectCondition] = Field(
        None, description="A condition on a status property."
    )
    multi_select: Optional[MultiSelectCondition] = Field(
        None, description="A condition on a multi-select property."
    )
    date: Optional[DateCondition] = Field(
        None, description="A condition on a date property."
    )
    ...
    people: Optional[PeopleCondition] = Field(
        None, description="A condition on a people property."
    )
    ...
    rollup: Optional[RollupCondition] = Field(
        None, description="A condition on a rollup's value."
    )
    ...
    def _one_shape(self):
        ...
        if self.timestamp is None and self.property is None:
            raise ValueError(
                f"a `{conditions[0]}` condition needs `property` naming the column it "
                f"applies to"
            )
        return self
...
class Sort(BaseModel):
    ...
    property: Optional[str] = Field(
        None, description="The name or ID of the property to sort by."
    )
    timestamp: Optional[Literal["created_time", "last_edited_time"]] = Field(
...
class PropertyConfiguration(BaseModel):
    ...
    multi_select: Optional[SelectConfig] = Field(
        None, description="A multiple-choice column."
    )
    status: Optional[StatusConfig] = Field(
        None, description="A status column, whose options belong to three groups."
    )
    date: Optional[EmptyConfig] = Field(
        None, description="A date column. Send an empty object."
    )
    people: Optional[EmptyConfig] = Field(
        None, description="A people column. Send an empty object."
    )
    files: Optional[EmptyConfig] = Field(
        None, description="A files column. Send an empty object."
    )
    checkbox: Optional[EmptyConfig] = Field(
        None, description="A checkbox column. Send an empty object."
    )
    url: Optional[EmptyConfig] = Field(
        None, description="A URL column. Send an empty object."
    )
    email: Optional[EmptyConfig] = Field(
        None, description="An email column. Send an empty object."
    )
    ...
    place: Optional[EmptyConfig] = Field(
        None, description="A place column. Send an empty object."
    )
...
class PropertyUpdate(PropertyConfiguration):
    ...
    def _one_type(self):
        ...
        if len(present) > 1:
            raise ValueError(
                f"a property has one type, got {', '.join(present)}"
            )
