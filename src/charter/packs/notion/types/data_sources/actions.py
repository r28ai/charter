...
class DataSourceParent(BaseModel):
    ...
    type: Optional[Literal["database_id"]] = Field(None, description="Always `database_id`.")
...
class DataSourceUpdateBody(BaseModel):
    ...
    parent: Optional[DataSourceParent] = Field(
        None,
        description=(
            "A different database to move this data source into. Left where it is when absent."
        ),
    )
    title: Optional[List[RichText]] = Field(None, max_length=100, description=TITLE)
    icon: Optional[Icon] = Field(None, description="The data source's icon. Null removes it.")
...
class DataSourceQueryBody(BaseModel):
    ...
    filter: Optional[Filter] = Field(
        default=None,
        description=("Which rows to return. Omit to return all of them."),
    )
    sorts: Optional[List[Sort]] = Field(
        default=None,
        description=("How to order the rows. Earlier entries in the list take precedence."),
    )
    ...
    is_archived: Optional[bool] = Field(
        default=None,
        description=(
            "Whether to return archived pages instead of live ones. Live pages when absent."
        ),
    )
