...
class MediaContent(BaseModel):
    ...
    file: Annotated[
        ...
        Field(
            None, description="Notion's hosted copy, with a URL that expires. Returned, never sent."
        ),
        Mode("response_only"),
    ]
...
class CodeContent(BaseModel):
    ...
    rich_text: List[RichText] = Field(..., max_length=100, description="The code, as rich text.")
    language: CodeLanguage = Field(..., description="The language the code is highlighted as.")
    caption: Optional[List[RichText]] = Field(
...
class EmbedContent(BaseModel):
    ...
    file_upload: Optional[FileUploadRef] = Field(
        None,
        description=(
            "A completed file upload to embed. An uploaded `.html` file becomes an HTML embed."
        ),
    )
    caption: Optional[List[RichText]] = Field(
...
class _BlockCommon(BaseModel):
    ...
    embed: Optional[EmbedContent] = Field(None, description="Third-party content rendered inline.")
    ...
    table_row: Optional[TableRowContent] = Field(None, description="One row of a table.")
    ...
    @model_validator(mode="after")
    def _one_kind(self):
        variants = tuple(name for name in type(self).model_fields if name not in _NOT_CONTENT)
...
class NestedParagraphContent(ParagraphContent):
    ...
    children: Optional[List[LeafBlock]] = Field(None, max_length=100, description=CHILDREN)
...
class NestedTableContent(TableContent):
    ...
    children: List[LeafBlock] = Field(
        ...
        max_length=100,
        description=(
            "The table's rows, each a `table_row` block whose `cells` count matches `table_width`."
        ),
    )
...
class NestedBlock(_BlockCommon):
    ...
    paragraph: Optional[NestedParagraphContent] = Field(None, description="A paragraph of text.")
    heading_1: Optional[NestedHeadingContent] = Field(None, description="A top-level heading.")
    heading_2: Optional[NestedHeadingContent] = Field(None, description="A second-level heading.")
    heading_3: Optional[NestedHeadingContent] = Field(None, description="A third-level heading.")
    heading_4: Optional[NestedHeadingContent] = Field(None, description="A fourth-level heading.")
...
class BlockParagraphContent(ParagraphContent):
    ...
    children: Optional[List[NestedBlock]] = Field(None, max_length=100, description=CHILDREN)
...
class Block(_BlockCommon):
    ...
    paragraph: Optional[BlockParagraphContent] = Field(None, description="A paragraph of text.")
    heading_1: Optional[BlockHeadingContent] = Field(None, description="A top-level heading.")
    heading_2: Optional[BlockHeadingContent] = Field(None, description="A second-level heading.")
    heading_3: Optional[BlockHeadingContent] = Field(None, description="A third-level heading.")
    heading_4: Optional[BlockHeadingContent] = Field(None, description="A fourth-level heading.")
    ...
    column: Optional[BlockColumnContent] = Field(None, description="One column of a column list.")
