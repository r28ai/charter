    ...
    async def handler(response: Any) -> Any:
        ...
        if isinstance(response, dict):
            for envelope in _ENVELOPE_KEYS:
                ...
                out: Dict[str, Any] = {
                    envelope: [
                        project(item) for item in response[envelope] if isinstance(item, dict)
                    ]
                }
...
async def trim_content(response: Any) -> Any:
    ...
    if encoding == "base64" and isinstance(response.get("content"), str):
        try:
            ...
            out["content"] = _UNREADABLE.format(why="it is a binary file, not decodable as text")
...
def _review(obj: Dict[str, Any]) -> Dict[str, Any]:
    out: Dict[str, Any] = {
        k: obj.get(k)
        for k in ("id", "state", "body", "submitted_at", "commit_id")
...
def _job(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(steps, list):
        failed = [
            ...
            for step in steps
            if isinstance(step, dict) and step.get("conclusion") not in (None, "success", "skipped")
        ]
        if failed:
...
async def trim_combined_status(response: Any) -> Any:
    ...
    if isinstance(statuses, list):
        out["statuses"] = [_commit_status(item) for item in statuses if isinstance(item, dict)]
    return out
...
async def read_artifact_archive(response: Any) -> Any:
    ...
    if skipped:
        ...
        out["not_inlined_note"] = (
            "Too large to inline, or not UTF-8 text. Their names and sizes are in `entries`."
        )
    return out
...
async def trim_requested_reviewers(response: Any) -> Any:
    ...
    teams = [t.get("slug") for t in response.get("teams") or [] if isinstance(t, dict)]
    if any(teams):
...
async def passthrough_diff(response: Any) -> Any:
    ...
    if "raw_base64" in response:
        return {
            "error": (f"The diff did not arrive as text. Got {response.get('content_type')}."),
            "size_bytes": response.get("size_bytes"),
        }
...
def _git_commit(obj: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(tree, dict) and tree.get("sha"):
        out["tree"] = tree["sha"]
    parents = [p["sha"] for p in obj.get("parents") or [] if isinstance(p, dict) and p.get("sha")]
    if parents:
        out["parents"] = parents
...
async def trim_blob(response: Any) -> Any:
    ...
    if encoding == "base64" and isinstance(response.get("content"), str):
        try:
            ...
            out["content"] = _UNREADABLE.format(why="it is a binary blob, not decodable as text")
    elif encoding == "none":
        out["content"] = _UNREADABLE.format(
