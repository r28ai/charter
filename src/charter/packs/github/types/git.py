...
class TreeEntry(BaseModel):
    ...
    * ``sha: null`` deletes the path — and "Returns an error if you try to
    ...
    type: Optional[TreeEntryType] = Field(None, description="Either `blob`, `tree`, or `commit`.")
...
class GitTagsCreateRequest(RepoRequest):
    ...
    tagger: Annotated[
        Optional[GitCommitAuthor],
        Field(
            None, description="An object with information about the individual creating the tag."
        ),
        Body(),
    ]
