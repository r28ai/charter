...
class NotificationsListRequest(BaseModel):
    ...
    all: Annotated[
        ...
        Field(
            None,
            description=(
                "If `true`, show notifications marked as read. Absent, only unread ones come back."
            ),
        ),
        Query(),
    ]
...
DismissedReason = Literal["fix_started", "inaccurate", "no_bandwidth", "not_used", "tolerable_risk"]
...
class DependabotListAlertsRequest(RepoRequest):
    ...
    severity: Annotated[
        ...
        Field(
            None,
            description=(
                "A comma-separated list of severities. Can be: `low`, `medium`, `high`, `critical`."
            ),
        ),
        Query(),
    ]
...
class DependabotUpdateAlertRequest(_AlertNumber):
    ...
    def _dismissal_states_its_reason(self) -> DependabotUpdateAlertRequest:
        ...
        if self.dismissed_reason is not None and self.state != "dismissed":
            raise ValueError("`dismissed_reason` only applies when `state` is 'dismissed'.")
        return self
...
CodeScanningSeverity = Literal["critical", "high", "medium", "low", "warning", "note", "error"]
...
class CodeScanningListAlertsRequest(RepoRequest, GitHubListRequest):
    ...
    def _one_tool_selector(self) -> CodeScanningListAlertsRequest:
        ...
        if self.tool_name is not None and self.tool_guid is not None:
            raise ValueError("Specify the tool with either `tool_name` or `tool_guid`, not both.")
        return self
