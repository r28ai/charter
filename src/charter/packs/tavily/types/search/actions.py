...
class SearchRequest(BaseModel):
    ...
    include_domains_mode: Annotated[
        ...
        Field(
            None,
            description=(
                "How `include_domains` is applied. Requires `include_domains` "
                "to be set."
            ),
        ),
        Body(),
    ...
    filter_by_language: Annotated[
        ...
        Field(
            None,
            description=(
                "Strictly filter out non-matching languages. Requires `language`."
            ),
        ),
        Body(),
    ]
    ...
    def _cross_field_rules(self) -> SearchRequest:
        ...
        if self.chunks_per_source is not None and self.search_depth in ("ultra-fast",):
            raise ValueError(
                "chunks_per_source is unavailable when search_depth is ultra-fast."
            )
