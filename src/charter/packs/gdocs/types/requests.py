...
class Range(BaseModel):
    ...
    start_index: Optional[int] = Field(
        default=None,
        description="The start index of the range, in UTF-16 code units."
    )
    end_index: Optional[int] = Field(
        default=None,
        description="The end index of the range, in UTF-16 code units."
    )
    segment_id: Optional[str] = Field(
        default=None,
...
class TableCellStyle(BaseModel):
    ...
    background_color: Optional[str] = Field(default=None, description="Background color as a hex string.")
...
class TableRowStyle(BaseModel):
    ...
    min_row_height: Optional[float] = Field(default=None, description="Minimum row height in points.")
...
class InsertTextRequest(BaseModel):
    ...
    location: Optional[Location] = Field(None, description="Inserts the text at a specific index in the document.")
    ...
    def _oneof(self) -> InsertTextRequest:
        if (self.location is None) == (self.end_of_segment_location is None):
            raise ValueError("Exactly one of `location` or `end_of_segment_location` must be provided.")
        return self
...
class CreateParagraphBulletsRequest(BaseModel):
    ...
    bullet_preset: BulletGlyphPreset = Field(..., description="The kinds of bullet glyphs to be used.")
...
class InsertInlineImageRequest(BaseModel):
    ...
    object_size: Optional[Size] = Field(default=None, description="The size that the image should appear as in the document.")
    location: Optional[Location] = Field(None, description="Inserts the image at a specific index in the document.")
    end_of_segment_location: Optional[EndOfSegmentLocation] = Field(
        None,
...
class InsertTableRequest(BaseModel):
    ...
    location: Optional[Location] = Field(None, description="Inserts the table at a specific index in the document.")
    end_of_segment_location: Optional[EndOfSegmentLocation] = Field(
        None,
...
class InsertTableRowRequest(BaseModel):
    ...
    insert_below: bool = Field(..., description="Whether to insert new row below the reference cell location.")
...
class InsertTableColumnRequest(BaseModel):
    ...
    insert_right: bool = Field(..., description="Whether to insert new column to the right of the reference cell location.")


class DeleteTableRowRequest(BaseModel):
    table_cell_location: TableCellLocation = Field(..., description="The reference table cell location from which the row will be deleted.")


class DeleteTableColumnRequest(BaseModel):
    table_cell_location: TableCellLocation = Field(..., description="The reference table cell location from which the column will be deleted.")


class InsertPageBreakRequest(BaseModel):
    location: Optional[Location] = Field(None, description="Inserts the page break at a specific index in the document.")
    end_of_segment_location: Optional[EndOfSegmentLocation] = Field(
        None,
...
class DeletePositionedObjectRequest(BaseModel):     
    ...
    tab_id: Optional[str] = Field(default=None, description="The tab that the positioned object to delete is in.")


class UpdateTableColumnPropertiesRequest(BaseModel):
    table_start_location: Location = Field(..., description="The location where the table starts in the document.")
    ...
    table_column_properties: TableColumnProperties = Field(..., description="The table column properties to update.")
    fields: str = Field(..., description="The fields that should be updated.")


class UpdateTableCellStyleRequest(BaseModel):
    table_cell_style: TableCellStyle = Field(..., description="The style to set on the table cells.")
    fields: str = Field(..., description="The fields that should be updated.")
    table_range: Optional[TableRange] = Field(None, description="The table range representing the subset of the table to which the updates are applied.")
    table_start_location: Optional[Location] = Field(None, description="The location where the table starts in the document.")

    @model_validator(mode="after")
    def _oneof(self) -> UpdateTableCellStyleRequest:
        if (self.table_range is None) == (self.table_start_location is None):
            raise ValueError("Exactly one of `table_range` or `table_start_location` must be provided.")
        return self
...
class ReplaceImageRequest(BaseModel):
    image_object_id: str = Field(..., description="The ID of the existing image that will be replaced.")
    uri: str = Field(..., description="The URI of the new image.")
    image_replace_method: Optional[ImageReplaceMethod] = Field(
        default=None,
        description="Optional. The replacement method."
    )
    tab_id: Optional[str] = Field(default=None, description="The tab that the image to be replaced is in.")
...
class UpdateDocumentStyleRequest(BaseModel):
    ...
    fields: str = Field(..., description="The fields that should be updated.")
    tab_id: Optional[str] = Field(default=None, description="The tab that contains the style to update.")


class MergeTableCellsRequest(BaseModel):
    table_range: TableRange = Field(..., description="The table range specifying which cells of the table to merge.")


class UnmergeTableCellsRequest(BaseModel):
    table_range: TableRange = Field(..., description="The table range specifying which cells of the table to unmerge.")
...
class CreateFootnoteRequest(BaseModel):
    location: Optional[Location] = Field(None, description="Inserts the footnote reference at a specific index in the document.")
    end_of_segment_location: Optional[EndOfSegmentLocation] = Field(
        None,
...
class ReplaceNamedRangeContentRequest(BaseModel):
    ...
    text: str = Field(..., description="Replaces the content of the specified named range(s) with the given text.")
    named_range_id: Optional[str] = Field(None, description="The ID of the named range whose content will be replaced.")
    named_range_name: Optional[str] = Field(None, description="The name of the NamedRanges whose content will be replaced.")

    @model_validator(mode="after")
    def _oneof(self) -> ReplaceNamedRangeContentRequest:
        if (self.named_range_id is None) == (self.named_range_name is None):
            raise ValueError("Exactly one of `named_range_id` or `named_range_name` must be provided.")
        return self
...
class InsertSectionBreakRequest(BaseModel):
    ...
    location: Optional[Location] = Field(None, description="Inserts a newline and a section break at a specific index in the document.")
    end_of_segment_location: Optional[EndOfSegmentLocation] = Field(
        None,
...
class DeleteHeaderRequest(BaseModel):
    ...
    tab_id: Optional[str] = Field(default=None, description="The tab containing the header to delete.")


class DeleteFooterRequest(BaseModel):
    footer_id: str = Field(..., description="The id of the footer to delete.")
    tab_id: Optional[str] = Field(default=None, description="The tab that contains the footer to delete.")


class PinTableHeaderRowsRequest(BaseModel):
    table_start_location: Location = Field(..., description="The location where the table starts in the document.")
    pinned_header_rows_count: int = Field(..., description="The number of table rows to pin, where 0 implies that all rows are unpinned.")
