...
class IssuesListVariables(PageVariables):
    ...
    order_by: Optional[PaginationOrderBy] = Field(
        default=None,
        description=("Sort by 'createdAt' or 'updatedAt'. Linear sorts by 'updatedAt' by default."),
    )
...
class IssueGetVariables(BaseModel):
    ...
    id: str = Field(
        ...,
        description=(
            "The issue's UUID, or its human identifier such as 'ENG-123'. Both are accepted here."
        ),
    )
...
class IssueCreateInput(BaseModel):
    ...
    assignee_id: Optional[str] = Field(
        None,
        description=(
            "The identifier of the user to assign the issue to. Use `users_list` to resolve a name."
        ),
    )
    ...
    project_id: Optional[str] = Field(None, description="The project associated with the issue.")
    project_milestone_id: Optional[str] = Field(
        None, description="The project milestone associated with the issue."
    )
    cycle_id: Optional[str] = Field(None, description="The cycle associated with the issue.")
    ...
    estimate: Optional[int] = Field(None, description="The estimated complexity of the issue.")
...
class IssueUpdateInput(BaseModel):
    ...
    cycle_id: Optional[str] = Field(None, description="The cycle associated with the issue.")
    parent_id: Optional[str] = Field(None, description="The identifier of the parent issue.")
    subscriber_ids: Optional[List[str]] = Field(
        None, description="The identifiers of the users subscribing to this ticket."
    )
    estimate: Optional[int] = Field(None, description="The estimated complexity of the issue.")
...
class IssueSubscriptionVariables(BaseModel):
    ...
    def _one_way_of_naming_the_user(self) -> IssueSubscriptionVariables:
        if self.user_id is not None and self.user_email is not None:
            raise ValueError("Name the user by `user_id` or by `user_email`, not both.")
        return self
...
class IssueRelationCreateInput(BaseModel):
    ...
    related_issue_id: str = Field(..., description="The identifier of the related issue.")
    type: IssueRelationType = Field(..., description="The type of relation between the two issues.")
    id: Optional[str] = Field(
        None,
        description=(
            "The identifier in UUID v4 format. If none is provided, the backend will generate one."
...
class IssueRelationUpdateInput(BaseModel):
    ...
    def _at_least_one_change(self) -> IssueRelationUpdateInput:
        if not self.model_dump(exclude_none=True):
            raise ValueError("issueRelationUpdate needs at least one field to change.")
        return self
