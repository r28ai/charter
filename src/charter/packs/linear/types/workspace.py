...
class TeamsListVariables(PageVariables):
    ...
    filter: Optional[TeamFilter] = Field(default=None, description="Narrow the teams returned.")
...
class UsersListVariables(PageVariables):
    ...
    filter: Optional[UserFilter] = Field(default=None, description="Narrow the users returned.")
    include_archived: Optional[bool] = Field(
...
class WorkflowStateCreateInput(BaseModel):
    ...
    team_id: str = Field(..., description="The team associated with the state.")
    type: str = Field(..., description=_STATE_TYPE_DESCRIPTION)
    color: str = Field(
        ..., description="The color of the state, as a hex string such as '#4ea7fc'."
    )
    description: Optional[str] = Field(None, description="The description of the state.")
