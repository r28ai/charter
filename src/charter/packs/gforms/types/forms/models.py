...
class Option(BaseModel):
    ...
    go_to_action: Annotated[
        ...
        ConflictsWith(
            "go_to_section_id",
            reason=(
                "An option navigates by action or by section id, never both. Drop "
                "one."
            ),
        ),
    ] = None
...
class TimeQuestion(BaseModel):
    ...
    duration: Optional[bool] = Field(
        default=None,
        description=(
            "`true` if the question is about an elapsed time. Otherwise it is about "
            "a time of day."
        ),
    )
...
class QuestionGroupItem(BaseModel):
    ...
    def _exactly_one_kind(self) -> QuestionGroupItem:
        if self.grid is None:
            raise ValueError(
                "QuestionGroupItem requires its `kind` union: set `grid`."
            )
        return self
