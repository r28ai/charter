...
class CheckoutSessionsCreateRequest(BaseModel):
    ...
    cancel_url: Annotated[
        ...
        Field(
            None,
            description=(
                "If set, Checkout displays a back button and sends customers here if they cancel."
            ),
        ),
        Body(),
    ]
    ...
    customer_email: Annotated[
        ...
        Field(
            ...
            max_length=800,
            description=(
                "Prefills the customer's email. If not provided, customers are asked to enter it."
            ),
        ),
        Body(),
    ]
    ...
    def _mode_and_ui_mode_agree(self) -> CheckoutSessionsCreateRequest:
        ...
        if self.ui_mode in _IN_PAGE_UI_MODES:
            offending = [
                name for name in ("success_url", "cancel_url") if getattr(self, name) is not None
            ]
            if offending:
                raise ValueError(
