...
PaymentBehaviorOnCreate = Literal[
    "allow_incomplete", "default_incomplete", "error_if_incomplete"
]
...
class SubscriptionDiscount(BaseModel):
    ...
    discount: Optional[str] = Field(
        None, description="The ID of an existing discount to reuse."
    )
    promotion_code: Optional[str] = Field(
        None, description="The ID of a promotion code to apply."
    )
...
class PriceRecurring(BaseModel):
    ...
    interval_count: Optional[int] = Field(
        ...
        ge=1,
        description=(
            "The number of intervals between billings. Absent, Stripe bills once "
            "per interval."
        ),
    )
...
class _SubscriptionItemCommon(BaseModel):
    ...
    tax_rates: Optional[List[str]] = Field(
        None,
        description=(
            "Tax rate IDs overriding the subscription's default_tax_rates for this "
            "item."
        ),
    )
...
class _SubscriptionWriteCommon(BaseModel):
    ...
    collection_method: Annotated[
        ...
        Field(
            None,
            description=(
                "How to collect payment. Absent, Stripe charges automatically."
            ),
        ),
        Body(),
    ]
    days_until_due: Annotated[
        ...
        Field(
            ...
            ge=0,
            description=(
                "Days until an invoice is due. Valid only when collection_method is "
                "'send_invoice'."
            ),
        ),
        Body(),
...
class SubscriptionsCreateRequest(_SubscriptionWriteCommon):
    ...
    trial_period_days: Annotated[
        ...
        Field(
            ...
            ge=0,
            description=(
                "Length of the trial in days. Overrides any trial the plan defines."
            ),
        ),
        Body(),
    ]
...
class SubscriptionsUpdateRequest(_SubscriptionWriteCommon):
    ...
    items: Annotated[
        ...
        Field(
            ...
            max_length=_MAX_ITEMS,
            description=(
                f"The items to change, up to {_MAX_ITEMS}. Absent, the items are left "
                "alone."
            ),
        ),
        Body(),
