...
class Border(BaseModel):
    ...
    color: Annotated[
        Optional[Color],
        Mode("disabled"),
        Field(default=None, description="The color of the border. Deprecated: Use color_style."),
    ] = None
...
class Chip(BaseModel):
    ...
    def _at_most_one(self) -> Chip:
        provided = [
            n for n in ("person_properties", "rich_link_properties") if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class ConditionValue(BaseModel):
    ...
    def _exactly_one(self) -> ConditionValue:
        provided = [
            n for n in ("relative_date", "user_entered_value") if getattr(self, n) is not None
        ]
        if len(provided) != 1:
            raise ValueError(
...
class DataFilter(BaseModel):
    ...
    def _exactly_one(self) -> DataFilter:
        provided = [
            n
            for n in ("developer_metadata_lookup", "a1_range", "grid_range")
            if getattr(self, n) is not None
        ]
        if len(provided) != 1:
            raise ValueError(
...
class DataSourceChartProperties(BaseModel):
    ...
    data_execution_status: Annotated[
        ...
        Mode("response_only"),
        Field(default=None, description="Output only. The data execution status."),
    ] = None
...
class DataSourceRefreshSchedule(BaseModel):
    ...
    next_run: Annotated[
        Optional[Interval],
        Mode("response_only"),
        Field(default=None, description="Output only. The time interval of the next run."),
    ] = None
    ...
    def _at_most_one(self) -> DataSourceRefreshSchedule:
        provided = [
            n
            for n in ("daily_schedule", "weekly_schedule", "monthly_schedule")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class DataSourceSpec(BaseModel):
    ...
    def _at_most_one(self) -> DataSourceSpec:
        ...
        if len(provided) > 1:
            raise ValueError(f"At most one of 'big_query', 'looker' may be set, found: {provided}")
        return self
...
class DeveloperMetadataLocation(BaseModel):
    ...
    def _at_most_one(self) -> DeveloperMetadataLocation:
        provided = [
            n
            for n in ("spreadsheet", "sheet_id", "dimension_range")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class DeveloperMetadataLookup(BaseModel):
    ...
    def _strategy_requires_location(self) -> DeveloperMetadataLookup:
        if self.location_matching_strategy is not None and self.metadata_location is None:
            raise ValueError(
                "If locationMatchingStrategy is specified, a metadataLocation must "
                "also be specified."
            )
        if self.location_matching_strategy == "EXACT_LOCATION" and self.location_type is not None:
            raise ValueError(
...
class EmbeddedObjectPosition(BaseModel):
    ...
    def _exactly_one(self) -> EmbeddedObjectPosition:
        provided = [
            n for n in ("sheet_id", "overlay_position", "new_sheet") if getattr(self, n) is not None
        ]
        if len(provided) != 1:
            raise ValueError(
...
class ExtendedValue(BaseModel):
    ...
    error_value: Annotated[
        Optional[ErrorValue],
        Mode("response_only"),
        Field(default=None, description="Represents an error. This field is read-only."),
    ] = None
    ...
    def _at_most_one(self) -> ExtendedValue:
        provided = [
            n
            for n in ("number_value", "string_value", "bool_value", "formula_value", "error_value")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class FilterSpec(BaseModel):
    ...
    def _at_most_one(self) -> FilterSpec:
        provided = [
            n
            for n in ("column_index", "data_source_column_reference")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class PivotFilterSpec(BaseModel):
    ...
    def _at_most_one(self) -> PivotFilterSpec:
        provided = [
            n
            for n in ("column_offset_index", "data_source_column_reference")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class PivotGroup(BaseModel):
    ...
    def _at_most_one(self) -> PivotGroup:
        provided = [
            n
            for n in ("source_column_offset", "data_source_column_reference")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class PivotGroupRule(BaseModel):
    ...
    def _at_most_one(self) -> PivotGroupRule:
        provided = [
            n
            for n in ("manual_rule", "histogram_rule", "date_time_rule")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class PivotValue(BaseModel):
    ...
    def _exactly_one(self) -> PivotValue:
        provided = [
            n
            for n in ("formula", "source_column_offset", "data_source_column_reference")
            if getattr(self, n) is not None
        ]
        if len(provided) != 1:
            raise ValueError(
...
class ProtectedRange(BaseModel):
    ...
    protected_range_id: Annotated[
        Optional[int],
        Mode("response_only"),
        Field(default=None, description="The ID of the protected range. This field is read-only."),
    ] = None
...
class Sheet(BaseModel):
    ...
    comment_anchors: Optional[List[CommentAnchor]] = Field(
        default=None,
        description="The comment anchors on this sheet. Developer Preview.",
    )
...
class SortSpec(BaseModel):
    ...
    def _at_most_one(self) -> SortSpec:
        provided = [
            n
            for n in ("dimension_index", "data_source_column_reference")
            if getattr(self, n) is not None
        ]
        if len(provided) > 1:
            raise ValueError(
...
class Spreadsheet(BaseModel):
    ...
    spreadsheet_url: Annotated[
        Optional[str],
        Mode("response_only"),
        Field(default=None, description="The url of the spreadsheet. This field is read-only."),
    ] = None
    ...
    data_source_schedules: Annotated[
        ...
        Mode("response_only"),
        Field(default=None, description="Output only. A list of data source refresh schedules."),
    ] = None
    ...
    spreadsheet_id: Annotated[
        Optional[str],
        Mode("response_only"),
        Field(default=None, description="The ID of the spreadsheet. This field is read-only."),
    ] = None
    ...
    comments_view_mode: Annotated[
        ...
        Field(
            default=None,
            description=(
                "Output only. The comments view mode applied to the spreadsheet. Developer Preview."
            ),
        ),
    ] = None
...
class TextRotation(BaseModel):
    ...
    def _at_most_one(self) -> TextRotation:
        ...
        if len(provided) > 1:
            raise ValueError(f"At most one of 'angle', 'vertical' may be set, found: {provided}")
        return self
...
class WaterfallChartColumnStyle(BaseModel):
    ...
    color: Annotated[
        Optional[Color],
        Mode("disabled"),
        Field(default=None, description="The color of the column. Deprecated: Use color_style."),
    ] = None
...
class CommentAnchor(BaseModel):
    ...
    range: Optional[GridRange] = Field(
        default=None,
        description=("The coordinate range inside the sheet where this comment is anchored."),
    )
