...
_INSTALL_HINT = "The MCP adapter needs the mcp package. Install it with: pip install 'charter[mcp]'"
...
def build_server(
    ...
    *,
    ...
    # and `charter-mcp` names its server `charter`, so the result reads
    ...
    session = (
        tools
        if isinstance(tools, ToolSession)
        else ToolSession(tools, progressive=progressive)
    )
            ...
            return [
                types.Tool(
                    ...
                    input_schema=(
                        entry["parameters"]
                        if isinstance(entry, dict)
                        else entry.llm_schema().model_json_schema()
                    ),
                )
    ...
    server = _CharterServer(name=name)
