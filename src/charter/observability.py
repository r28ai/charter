...
def _classify(
    error: Optional[BaseException], probe: CallProbe
...
def format_call_line(call: ToolCall) -> str:
    ...
    if call.payload_bytes is not None and call.context_bytes is not None:
        ...
        line += (
            f"  ↓ {_fmt_bytes(call.payload_bytes):>9}"
            f" → {_fmt_bytes(call.context_bytes):>9}{pct}"
        )
    return line + build
...
def format_call_summary(calls: Iterable[ToolCall]) -> str:
    ...
    lines.append(
        f"  {'tool'.ljust(26)} {'calls':>5}  {'upstream p50':>12}   payload → context"
    )
    for name, tool_calls in sorted(by_tool.items(), key=lambda kv: -len(kv[1])):
        ...
        if tool_sized:
            ...
            sizes = "—"
        lines.append(
            f"  {_clip(name, 26).ljust(26)} {len(tool_calls):>5}  {p50:>12}   {sizes}"
        )
