...
describes. Across the twelve shipped packs the schemas come to about 209,000
...
SEARCH_PARAMETERS: Dict[str, object] = {
    ...
    "properties": {
        "query": {
            "type": "string",
            "description": (
                'Use "select:<tool_name>,<tool_name>" for direct selection, '
...
def rank(
    deferrable: Dict[str, Tool], required: List[str], terms: List[str]
) -> List[str]:
    ...
    for name, tool in deferrable.items():
        ...
        score = sum(2 for t in terms if t in lowered) + sum(
            1 for t in terms if t in description
        )
