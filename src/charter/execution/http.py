...
def _mask_headers_for_log(headers: Dict[str, str], max_len: int = 50) -> Dict[str, str]:
...
def _extract_field_cases(model: BaseModel) -> Dict[str, str]:
    """Extract per-field ``Case`` marker overrides from a Pydantic model.

    Returns a dict mapping field_name -> case string for fields that have an
    explicit ``Case()`` marker in their ``Annotated`` metadata.
    """
    field_cases: Dict[str, str] = {}
...
def _extract_field_wire_names(model: BaseModel) -> Dict[str, str]:
    """Per-field ``WireName`` overrides: field_name -> the key the API documents.

    Separate from :func:`_extract_field_cases` because it is not a case. It wins
    over every convention in the cascade and applies at any casing, including the
    snake default.
    """
    wire_names: Dict[str, str] = {}
    for name, field in model.__class__.model_fields.items():
        for m in field.metadata:
            if isinstance(m, WireName):
                wire_names[name] = m.name
                break
    return wire_names


def _body_model_wire_names(model: BaseModel) -> Dict[str, str]:
    """Wire names for the model whose fields *are* the top-level body keys.

    A single un-enveloped ``Body()`` field is unwrapped to the root of the JSON,
    so the keys sent are that nested model's field names, not this one's. Marker
    lookups keyed against the request schema never see them — which would make
    ``WireName`` work on a query parameter and silently not on the body field
    beside it.
    """
    body_fields = [
        (name, field)
        for name, field in model.__class__.model_fields.items()
        if any(isinstance(m, Body) for m in field.metadata)
    ]
    if len(body_fields) != 1:
        return {}
    name, field = body_fields[0]
    marker = next((m for m in field.metadata if isinstance(m, Body)), None)
    if marker is None or marker.envelop:
        return {}
    value = getattr(model, name, None)
    if not isinstance(value, BaseModel):
        return {}
    return _extract_field_wire_names(value)
...
def _extract_http_params(
    ...
    1. **Single Body (standard unwrap)** — the default, ~90% of APIs.
       ...
       The count is taken from the *schema*, not from which fields happen to be
       populated on this call — otherwise a schema with several optional body
       fields would unwrap whenever only one of them was set, and the wire shape
       would depend on the arguments rather than the contract.
    ...
    Returns
    -------
    tuple[path_params, query_params, body]
    ...
    # How many fields the *schema* routes to the body. The unwrap decision below
    # is made from this, never from how many happen to be populated: otherwise
    # the same tool would send {"channel": "C1", "text": "hi"} on one call and
    # a bare "C1" on the next, and the wire shape would stop being a property of
    # the schema.
    declared_body_fields = sum(
        1
        for f in model.__class__.model_fields.values()
    ...
    for name, field in model.__class__.model_fields.items():
        ...
        if loc is None:
            ...
            declared = getattr(model, "charter_declared_name", "") or type(model).__name__
            raise DeclarationError(
                f"Field '{name}' on {declared} has no Path(), Query() "
    ...
    if len(body_fields) == 0:
        ...
        body_params = None
    elif unwraps_body(declared_body_fields, body_fields[0][2], body_fields[0][1]):
...
def _is_body_field(model: BaseModel, field_name: str) -> bool:
    """Check if a field is a Body field (or defaults to Body)."""
    field = model.__class__.model_fields.get(field_name)
    if not field:
        return False
    for m in field.metadata:
        if isinstance(m, (Path, Query)):
            return False
        if isinstance(m, Body):
            return True
    return True  # default is Body


def _is_query_field(model: BaseModel, field_name: str) -> bool:
    """Check if a field is a Query field."""
    field = model.__class__.model_fields.get(field_name)
    if not field:
        return False
    return any(isinstance(m, Query) for m in field.metadata)
...
def _escape_path_params(path: Dict[str, Any], tool_input: BaseModel) -> Dict[str, Any]:
    ...
    * ``"#"`` starts a fragment and truncates the rest of the path.
    ...
    is for the genuinely multi-segment case (a file path). ``..`` is refused
    outright, opt-in or not: no API has a path segment legitimately named that.
    """
    allow_slash = {
        name
    ...
    for name, value in path.items():
        ...
        if any(segment == ".." for segment in text.split("/")):
            ...
            raise ToolValidationError(
                f"Path parameter '{name}' on {declared} contains a '..' segment, which "
                f"would change which endpoint is called. Pass the value itself, not a "
                f"path relative to it.",
                errors=[{"loc": (name,), "msg": "'..' is not allowed in a path parameter"}],
            )
...
async def call_api(
    ...
    follow_redirects: Whether a 3xx is followed. ``False`` by default, which is
        ...
        way to ask again. httpx drops the ``Authorization`` header when a
        redirect leaves the origin, so the bearer token does not travel to the
        storage host the signed URL already authenticates against.
    ...
    path, params, body = _extract_http_params(tool_input)

    # Collect field-level Case overrides from the schema
    field_cases = _extract_field_cases(tool_input)
    wire_names = _extract_field_wire_names(tool_input)
    ...
    if path_case != "snake" or wire_names:
        path = {
            wire_names.get(k) or DictKeyParser.convert_key(k, path_case): v
    ...
    # Apply body_case conversion with field-level and schema-level overrides.
    # Cascade: Field Case > Schema __case__ > body_case (endpoint/factory default)
    if body_case != "snake" or field_cases or wire_names or _body_model_wire_names(tool_input):
        body_field_cases = {
            k: v for k, v in field_cases.items() if _is_body_field(tool_input, k)
        }
        body_wire_names = {
            **_body_model_wire_names(tool_input),
            **{k: v for k, v in wire_names.items() if _is_body_field(tool_input, k)},
        }
        schema_case = getattr(tool_input.__class__, "__case__", None)

        if isinstance(body, dict):
            json_payload = DictKeyParser.convert_keys_recursive(
                body,
                body_case,
                field_cases=body_field_cases or None,
                schema_case=schema_case,
                wire_names=body_wire_names or None,
            )
        elif isinstance(body, list):
            json_payload = [
                DictKeyParser.convert_keys_recursive(item, body_case)
    ...
    # Apply query_case conversion.
    #
    # The `or field_cases or wire_names` matters: gated on query_case alone, a
    # field-level override was dropped whenever the factory left query_case at
    # its snake default — which is six of the twelve shipped packs. `Case` is
    # documented as the highest priority in the cascade and was silently the
    # lowest here, while the body path beside it had the same clause and honoured
    # it. Nothing failed, because a query parameter under the wrong name is
    # usually ignored rather than rejected.
    query_field_cases = {
        k: v for k, v in field_cases.items() if _is_query_field(tool_input, k)
    }
    query_wire_names = {
        k: v for k, v in wire_names.items() if _is_query_field(tool_input, k)
    }
    if isinstance(params, dict) and (
        query_case != "snake" or query_field_cases or query_wire_names
    ):
        params_payload = DictKeyParser.convert_keys_recursive(
            params,
            query_case,
            field_cases=query_field_cases or None,
            wire_names=query_wire_names or None,
        )
    else:
