...
def _create_auto_transformer(
    ...
    def auto_transform(input_model: BaseModel) -> TransportOverride:
        ...
        # where every other field goes — so both now count through the same
        # function, over the same schema.
        body_field_count = declared_body_fields(original_schema)
        ...
        for field_name, transformed_value in transformed_fields.items():
            ...
            if isinstance(location, Path):
                ...
                if unwraps_body(body_field_count, location, transformed_value):
...
class ToolExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        # Counted once, off the declaration, and handed to every call. The view
        # the runtime executes may hold fewer fields than this — a projection or
        # a Mode can remove one — and the body shape must not move when it does.
        self._body_field_count = declared_body_fields(original_schema)
    ...
    async def _execute_api_key(
        ...
        if transport_override and "headers" in transport_override:
            # Through `httpx.Headers`, so a `build_request` override spelled with
            # different capitalisation than the credential header replaces it
            # rather than joining it. `call_api` merges the rest of the cascade
            # the same way, for the same reason.
            merged = httpx.Headers(auth_headers)
            merged.update(transport_override["headers"])
            auth_headers = dict(merged)
        ...
        return await call_api(
            ...
            credentials=None,
            # Which of the headers just assembled is the secret. Nothing
            # downstream can tell, and httpx's own redirect rule knows only
            # about `Authorization`, so the names travel with the request.
            credential_headers=tuple(auth_headers),
