...
def _gloss_text(field_info: Any) -> str:
    ...
    """
    return " ".join(
        marker.text for marker in getattr(field_info, "metadata", []) if isinstance(marker, Gloss)
    )
...
def _conflict_validator(fields: Dict[str, Any]) -> Optional[Any]:
    ...
    def _check(self: Any) -> Any:
        ...
        for other, (offenders, reason) in grouped.items():
            clause = (
                f"{', '.join(sorted(offenders))} cannot be combined with {names.get(other, other)}."
            )
...
# A label, several labels, or none. `Optional[str]` members are allowed inside
# the iterable because the commonest caller is a pair of axes that may each be
# unset — `in_force` drops them, which is what "contributes nothing" means.
ModeLabels = Union[str, Iterable[Optional[str]], None]


def in_force(mode: ModeLabels) -> FrozenSet[str]:
    """The labels a view is filtered against, as a set.

    A view is resolved against however many labels are in force at once, not one.
    There are two ends of ``Mode`` and they answer different questions: the pack
    author's ``mode=`` says *which operation this tool is* — ``create`` replaces
    and ``update`` merges, and no deployment changes that — while a deployment's
    label says *which surface this conversation is*, a tier or a region or a
    scope. Collapsing them onto one slot made the second silently destroy the
    first: a session at ``"pro"`` turned ``forms_create`` into a tool resolved at
    ``pro``, and the fields that make it a *create* went with it.

    So both are in force together, and this is the one place that says what
    "together" means. ``None`` contributes nothing, and an empty set means no
    partition is in force at all — the widest view, where every custom label is
    inert. That last rule is why a first label narrows (from everything to the
    unlabelled plus its own) while a second one only ever widens (it adds the
    fields carrying it). Both readings fall out of one sentence rather than
    being two rules to remember.

    Accepts a single label as well as a set of them, so every caller that holds
    one string keeps working and reads the same.
    """
    if mode is None:
        return frozenset()
    if isinstance(mode, str):
        return frozenset((mode,)) if mode else frozenset()
    return frozenset(label for label in mode if label)


def should_include_field(
    field_info: Any, mode: ModeLabels, parent_modes: Optional[set] = None
) -> bool:
    """Whether a field belongs in the LLM view, given the modes and any cascade.

    ``mode`` is the label in force, or the labels — see :func:`in_force`, which
    is what "or" means here. A field carrying a custom ``Mode`` is offered when
    any label in force names it, so a tool declared ``mode="create"`` and
    resolved by a deployment at ``"pro"`` keeps its ``create`` fields and gains
    its ``pro`` ones.

    Special modes, always enforced whatever is in force:

    - ``disabled``: always exclude
    - ``response_only``: always exclude (output-only fields)
    - ``request_only``: always include (input-only fields)
    """
    labels = in_force(mode)
    ...
    if mode_marker:
        ...
        # Otherwise: keep it when a label in force names it. With nothing in
        # force there is no partition, so every custom label is inert.
        if labels and mode_marker.modes:
            return bool(labels & mode_marker.modes)
    ...
    # If no explicit mode marker, check parent modes (cascading)
        ...
        if "request_only" in parent_modes:
            return True
        if labels:
            return bool(labels & parent_modes)
...
def _carry_validators(original_schema: Type[BaseModel], kept_fields: set) -> dict:
    ...
    for name, decorator in decorators.model_serializers.items():
        carried[name] = model_serializer(mode=decorator.info.mode)(_unbind(decorator.func))
    ...
    for name, decorator in decorators.field_validators.items():
        ...
        carried[name] = field_validator(*targets, mode=mode)(classmethod(_unbind(decorator.func)))

    return carried


def _resolve_annotation_ref(name: str, owner: Type[BaseModel], parked: str) -> Any:
    """A string annotation -> the model it names, or the parked name for a self-reference.

    Pydantic leaves an unresolved forward reference as a plain string on the
    field. A reference to the model currently being built cannot be resolved to
    a class yet — that class does not exist until this call returns — so it
    resolves to the name ``_rebuild_generated`` will bind later.
    """
    if name == owner.__name__:
        return parked
    module = sys.modules.get(owner.__module__)
    resolved = getattr(module, name, None) if module else None
    if isinstance(resolved, type) and issubclass(resolved, BaseModel):
        return resolved
    return None


def _llm_type(annotation: Any, build: Any, owner: Type[BaseModel], parked: str) -> Any:
    """The LLM view of one field's annotation, container layers and all.

    Returns ``None`` when the annotation is not something this walk rewrites — a
    scalar, an unresolvable forward reference, a container of either — and the
    caller then keeps the author's own annotation.

    **Recursive over the containers, not just the outermost one.** That is the
    whole of it: ``List[List[RichText]]`` is two layers above a model whose
    ``Mode`` markers still apply, and rewriting only the outer layer left the
    inner annotation pointing at the wire model. Notion's table cells are that
    shape, and `RichText.plain_text` and `.href` — both ``response_only`` — were
    handed to the model on three tools while :func:`~charter.egress.egress_map`,
    which unwraps containers all the way down, reported them withheld. A hole
    the audit artifact said was closed is worse than one it admits to, so the
    two walks peel containers the same way or neither can be trusted.
    """
    ...
    if origin is Union:
        ...
        non_none = [a for a in args if a is not type(None)]
        optional = len(args) > len(non_none)

        if len(non_none) == 1:
            inner = _llm_type(non_none[0], build, owner, parked)
            if inner is None:
                return None
            return Optional[inner] if optional else inner

        # Every member, each rewritten in its own right. Taking the first —
        # which is what this did, via `next(arg for arg in args ...)` — dropped
        # the rest from the type the model is offered: `Optional[Union[A, B]]`
        # came out as `Optional[A_LLM]` and B was simply gone. Keeping the union
        # whole and unrewritten is the other wrong answer, because then no
        # member's `Mode` markers filter anything. Members that rewrite to
        # nothing (a `bool` beside a model, which Firecrawl's `redact_pii` is)
        # keep the author's annotation, so a union of a scalar and a model comes
        # out as the scalar beside that model's view.
        rewritten = [_llm_type(m, build, owner, parked) for m in non_none]
        if all(r is None for r in rewritten):
            return None
        members = tuple(m if r is None else r for m, r in zip(non_none, rewritten, strict=True))
        union: Any = Union[members]
        return Optional[union] if optional else union

    if origin is list:
        args = get_args(annotation)
        if not args:
            return None
        inner = _llm_type(args[0], build, owner, parked)
        return None if inner is None else List[inner]  # type: ignore[valid-type]

    if origin is dict:
        args = get_args(annotation)
        if len(args) != 2:
            return None
        inner = _llm_type(args[1], build, owner, parked)
        if inner is None:
            return None
        # A cycle under the value type parks a forward reference to a generated
        # class, and this used to hand back the author's annotation rather than
        # emit one — those classes live in no module namespace, so the reference
        # resolved nowhere. `_rebuild_generated` resolves them now, against a
        # namespace of every model the walk touched, which is what the list
        # branch has always relied on. Keeping the bail-out meant the raw wire
        # model was embedded instead: Tavily's self-referential
        # `OutputSchemaProperty` reached the model unfiltered, and the map named
        # four fields fewer than it was offered, because the two walks closed the
        # same cycle at different depths. The protobuf `Struct`/`Value` pair is
        # not this case — it is held verbatim by `Format`, one branch up.
        return Dict[args[0], inner]  # type: ignore[valid-type]

    if isinstance(annotation, str):
        target = _resolve_annotation_ref(annotation, owner, parked)
        if target is None:
            return None
        return ForwardRef(target) if isinstance(target, str) else build(target)

    if isinstance(annotation, type) and issubclass(annotation, BaseModel):
        built = build(annotation)
        return ForwardRef(built) if isinstance(built, str) else built
...
def create_llm_schema(
    ...
    _root_mode: Optional[FrozenSet[str]] = None,
    _path: str = "",
...
def _auto_title(name: str) -> str:
    """Pydantic's own derivation of a title from a key.

    Mirrors ``GenerateJsonSchema.get_title_from_name``. Kept as a copy rather
    than imported because it is a method on a class that has to be instantiated,
    and because a change to it upstream should make titles survive here rather
    than make this raise.
    """
    return name.title().replace("_", " ").strip()


def llm_json_schema(model: Type[BaseModel]) -> Dict[str, Any]:
    """``model``'s JSON schema, without the titles Pydantic wrote itself.

    Pydantic titles every field and every model whether or not anyone asked:
    ``max_results`` gains ``"title": "Max Results"``, and a generated view class
    gains ``"title": "ListMessages_LLM"``. Nothing consumes them. OpenAI,
    Anthropic and MCP all ignore ``title`` in a parameter schema, and the model
    reading it already has the key itself one line above. Across the shipped
    packs they are 8,476 strings and 52,061 tokens — 510,250 down to 458,189,
    10.2% of the whole surface spent restating property names to a reader that
    can see them.

    So they are dropped, but only where Pydantic is demonstrably the author: a
    title is removed when it equals what Pydantic would have generated for the
    key it hangs on, and kept otherwise. A pack that writes
    ``Field(title="RFC 3339 timestamp")`` keeps it; the 8,476 that say "Max
    Results" above a field named ``maxResults`` do not. That test is what makes
    this safe to apply to a stranger's pack, which is the only reason it can run
    inside the runtime rather than being advice in a style guide.
    """
    # The root is titled with the class name verbatim, the same way a `$defs`
    # entry is, so it is compared against `__name__` rather than a derivation.
    return _prune_auto_titles(model.model_json_schema(), model.__name__)


def _prune_auto_titles(node: Any, auto: Optional[str] = None) -> Any:
    """Strip ``title`` from ``node`` and below wherever Pydantic generated it.

    ``auto`` is the title Pydantic would have written for this node, which only
    the parent knows: under ``properties`` it is derived from the field's key,
    under ``$defs`` it is the definition's name, and anywhere else — an
    ``items``, an ``anyOf`` member — there is no derivation to compare against,
    so a title found there is someone's own and survives.
    """
    if isinstance(node, list):
        return [_prune_auto_titles(item) for item in node]
    ...
    for key, value in node.items():
        if key == "title":
            if not (isinstance(value, str) and auto is not None and value == auto):
                out[key] = value
        elif key == "properties" and isinstance(value, dict):
            out[key] = {
                name: _prune_auto_titles(sub, _auto_title(name)) for name, sub in value.items()
            }
        elif key == "$defs" and isinstance(value, dict):
            # A definition's key is the class name, and Pydantic titles the
            # definition with it verbatim — no `_auto_title` pass, which would
            # turn `ListMessages_LLM` into `Listmessages Llm` and match nothing.
            out[key] = {name: _prune_auto_titles(sub, name) for name, sub in value.items()}
        else:
            out[key] = _prune_auto_titles(value)
    return out
...
def _create_llm_schema(
    ...
    Args:
        ...
        prune: Dotted paths to drop, relative to the root — ``"body.requests.
            insert_table"``. Where ``Mode`` filters by a label the author
            declared on the field, this filters by path, so it reaches a field
            nobody thought to label. The two are independent and compose.
    ...
    # Preserve the root mode for all nested types. Normalised to a set once,
    # here, so the cache key is hashable and every nested call filters against
    # the same labels rather than re-deriving them. `is None` rather than
    # falsiness: an empty set is "no partition in force", which is an answer.
    if _root_mode is None:
        _root_mode = in_force(mode)
    ...
    for field_name, field_info in original_schema.model_fields.items():
        ...
        if format_marker:
            ...
            if transform_spec and transform_spec.semantic_type is not BaseModel:
                ...
                is_optional = get_origin(field_info.annotation) is Union and type(None) in get_args(
                ...
                annotated_semantic_type = Annotated[(semantic_type, *preserved_metadata)]

                fields[field_name] = (
                    annotated_semantic_type,
                    Field(
                        default=default_value,
                        description=transform_spec.llm_description or field_info.description,
            ...
            # No Format marker. The annotation may hold a model at any depth
            # under its containers, and every one of them still carries the
            # `Mode` markers and prune paths this walk exists to apply.
            def build(
                nested: Type[BaseModel],
                _modes: Any = field_modes,
                _path_: str = field_path,
            ) -> Any:
                return _create_llm_schema(
                    nested,
                    mode=effective_mode,
                    ...
                    _seen_types=_seen_types,
                    _root_mode=_root_mode,
                    # Container layers do not deepen the path: a `List[Request]`
                    # at `body.requests` has its members at
                    # `body.requests.insert_text`, because the path is a name a
                    # person types and the index is never the thing being
                    # narrowed. See `_prune_reaches`.
                    _path=_path_,
                    _pending=_pending,
                )

            rewritten = _llm_type(field_info.annotation, build, original_schema, llm_model_name)
            if rewritten is None:
                fields[field_name] = (field_info.annotation, field_info)
            else:
                fields[field_name] = (rewritten, field_info)
...
def partial_of(
    ...
    **Nested models are left alone.** Relaxing a whole tree would quietly drop
    ...
    for field_name, field_info in model.model_fields.items():
        ...
        already_optional = get_origin(annotation) is Union and type(None) in get_args(annotation)
