...
from charter.egress import Surface
            ...
            out.append(
                {
                    ...
                    "excludes": [
                        {
                            "field": other,
                            "api_name": names.get(other, other),
                            "declared": other in names,
                        }
...
def conflict_map(tools: Surface) -> Dict[str, List[Dict[str, Any]]]:
    ...
    Takes the tools, or a map of name to tool, so ``session.tools`` maps
    directly. Only tools that declare a conflict appear. Reads the schema the
    tool was declared with, so a field withheld from the model by ``Mode`` is
    still reported: the rule binds the request, not the prompt.
...
def format_conflicts(tools: Surface) -> str:
