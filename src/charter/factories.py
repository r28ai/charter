...
from charter.naming import check_pack
    ...
    *,
    ...
    Args:
        ...
        follow_redirects: Whether a 3xx is followed, for the whole factory.
            ...
            short-lived signed URL. The API key does not travel with it: the
            headers named in ``api_key_headers`` are dropped when a redirect
            leaves the origin. (httpx does this for ``Authorization`` and only
            for it, which is why the runtime has to be told which header is the
            credential — an api-key factory's secret is whatever the API named
            it, and a library cannot guess that.)
    ...
    """
    check_pack(pack, "api_key_tool_factory")
    if api_key_headers:
...
def oauth_tool_factory(
    ...
    *,
    ...
    """
    check_pack(pack, "oauth_tool_factory")
