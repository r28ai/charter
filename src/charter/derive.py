...
``Mode`` is the pack author's lever: declared on a field, fixed when the pack is
written. This is the other half, and it belongs to the person holding the tool.
They did not write ``documents_batch_update`` and cannot edit it, but they are
the one who knows that *this* agent may insert text and must never delete a
range::
...
def walk_paths(
    ...
    *,
    ...
    enormous even though the graph is small: Linear's ``search_issues`` has 77
...
def path_costs(
    ...
    *,
    ...
    # instead of once per path: `search_issues` priced its `variables` level in
    ...
    base = json_tokens(
        create_llm_schema(
            schema, mode=mode, prune=prune or None, cache=shared
        ).model_json_schema()
    )
    costs: Dict[str, int] = {}
    for path in paths:
        without = json_tokens(
            create_llm_schema(
                schema, mode=mode, prune=(prune | {path}), cache=shared
            ).model_json_schema()
        )
...
def _class_paths(schema: Type[BaseModel], target: type, *, limit: int = 0) -> List[str]:
    ...
    return find_paths(
        schema, lambda _name, field: _unwrap(field.annotation) is target, limit=limit
    )
...
def resolve_path(schema: Type[BaseModel], selector: Any, *, where: str) -> str:
    ...
    if not isinstance(selector, str) or not selector.strip():
        raise DeclarationError(
            f"{where}: expected a dotted path, a field name, or a model class; "
            f"got {selector!r}.",
            docs="tools/projections",
        )
    ...
    tail = find_paths(
        schema, lambda name, _field: name == selector, limit=_AMBIGUITY_LIMIT + 1
    )
    if len(tail) == 1:
        return tail[0]
    if len(tail) > 1:
        ...
        raise DeclarationError(
            f"{where}: {selector!r} matches {_count(tail)} paths ({shown}). "
            "Write the full path.",
            docs="tools/projections",
        )
