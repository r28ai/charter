...
- **`--pack` takes a list, so one MCP server can hold every pack you want.**
  Repeat the flag or comma-separate the names — `--pack gmail --pack slack` and
  `--pack gmail,slack` are the same thing, order is the order given, and a pack
  named twice is loaded once. `main` and `resolve_packs` do the validation, and a
  bad name is still a usage error: exit 2, with the usage line, which is what
  `choices=PACKS` used to give.

  This is a fix and not only a convenience. An MCP host composes
  `mcp__<server>__<tool>` from the key the server was **configured under**, not
  from the name the server reports, and OpenAI caps a function name at 64
  characters. `docs/using/mcp.mdx` used to say to configure one server per pack as
  `charter-gmail`, so the real prefix on a gsheets server was
  `mcp__charter-gsheets__gsheets__` — the pack spelled twice, 31 characters before
  the tool name starts — and three shipped names went over 64:
  `spreadsheets_developer_metadata_search` landed at 69. Nobody found out because
  `tests/test_naming.py` asserted against `mcp__charter__`, a prefix that setup
  never produced, over 12 of the 15 shipped packs.

  Serving every pack from one server named `charter` takes the pack out of the
  prefix: `mcp__charter__gsheets__values_get` spells it once where
  `mcp__charter-gsheets__gsheets__values_get` spells it twice. The widest shipped
  name now composes to 61 of the 64 available, and the test measures that shape
  across all 15 packs. Nothing is given up — `ToolSession` already spanned packs,
  so three packs still cost exactly one `ToolSearch` up front, and a pack you hold
  no credential for is listed but inert, raising `CredentialError` when called.

  The server keeps the product's own name rather than an abbreviation; the pack
  de-duplication is what pays for it. Two tests in `tests/test_doc_figures.py`
  keep the setup page from drifting back, since no code can see the key a client
  was configured under.

  **Migration:** none required, but collapse your client config to one entry to
  get the shorter names — replace the per-pack servers with a single `charter`
  entry whose `--pack` lists them. Tool names change when you do
  (`mcp__charter-gmail__gmail__messages_send` becomes
  `mcp__charter__gmail__messages_send`), so update any allow-list or saved prompt
  that spells them out.

- **BREAKING: `pack` is now a required argument on both factories.** It was
  optional and defaulted to `None`, and nothing noticed until an adapter tried
  to publish the tool — `DeclarationError: Tool 'list_messages' declares no
  pack` — several steps and one import away from the line that has to change.
  The quickstart hit exactly that on its last step.

  A factory is, in this library's own words, "everything shared across the tools
  of one API". If there is a factory there is an API, and it has a name; there is
  no coherent factory whose tools share a base URL, a credential and a wire
  contract but not a namespace. Requiring it there puts the error on the line
  where the fix goes, at the moment the API is already being named, and a type
  checker catches it before the program runs.

  The label is also validated where it is written rather than where it is
  published: empty, spaces, dots, or a `__` that would stop a qualified name
  splitting back into halves are all refused by the factory with a
  `DeclarationError` naming the call.

  **`Tool(...)` is unchanged and still takes no pack.** That is the one-off a
  caller only ever `ainvoke`s — a test fixture, a private helper — and it is the
  same carve-out `report_key` already makes, for the same reason: nothing filters
  or dispatches on it. Calling such a tool works; only publishing it is refused,
  because a surface where `stripe__products_list` sits beside a bare
  `balance_retrieve` makes any `+stripe` filter or log grep silently miss half of
  it.

  **Migration:** add `pack="..."` to every `oauth_tool_factory` and
  `api_key_tool_factory` call. Every shipped pack already passed one, so nothing
  in `charter.packs` changes, and no published tool name moves.


- **Pydantic's auto-generated `title` keys are no longer sent to the model.**
  Every field and every model got one whether or not anyone asked: `max_results`
  arrived as `"title": "Max Results"`, and a generated view class as
  `"title": "ListMessages_LLM"`. Nothing reads them. OpenAI, Anthropic and MCP all
  ignore `title` in a parameter schema, and a model reading `"Max Results"` has
  the key `maxResults` on the line above it. Across the shipped packs they were
  8,476 strings and 52,061 tokens — 510,250 down to 458,189, 10.2% of the whole
  tool surface spent restating property names.

  A title is dropped only where Pydantic is demonstrably its author: it is
  compared against what Pydantic would have generated for the key it hangs on —
  the field's name under `properties`, the class's name under `$defs` — and kept
  when it differs. A pack that writes `Field(title="RFC 3339 timestamp")` keeps
  it; the 8,476 that restated a field name do not. That test is what makes the
  pruning safe to run over a pack this library has never seen, rather than advice
  in a style guide.

  Applied wherever a schema is generated, so `to_json_schema()`, the OpenAI
  adapter, the MCP server, `schema_tokens()` and `paths(by_cost=True)` all agree.
  The new `charter.execution.schema.llm_json_schema()` is the single place it
  happens.

  **This changes every schema size Charter reports.** If you have figures written
  down — a budget, a dashboard, a doc — they move by about 10%. Nothing about
  which fields are visible changes, and `tests/test_doc_figures.py` now holds this
  project's own quoted numbers against the package so they cannot drift again.

- **The MCP server reports its version.** `serverInfo.version` was the empty
  string on every `python -m charter.mcp` server; it is now Charter's version, so
...
- **Two pages drew opposite conclusions from the same two campaigns.**
  ...
  reversal — "the same two arms went in opposite directions". Those are the
  pre-`stage2-final` cuts. Against the campaigns the evidence page reports,
  Charter is 2.1x faster in breadth and 2.0x in depth, and that page says so:
  "Both campaigns put Charter at roughly half." The supporting figures were
  stale too — "6.79 against 6.32" tool calls, "three times the tool errors",
  "the turn limit six times against once" — against 7.22 vs 7.43, 0.29 vs 0.47,
  and 2 vs 4.

  The section keeps its point and loses the false premise. Wall time is still
  not the number to chase, but the reason is that it is the context ratio
  measured downstream through a provider whose inference time is not yours: the
  raw arm hands the model four to five times the tool output, and the seconds
  follow the tokens. `tests/test_measured_results.py` now checks any page
  quoting a campaign figure, not only the page that owns them.

- **The evidence page cited two files, one of which does not exist.**
  [Measured results](https://docs.r28.ai/charter/guarantees/measured-results)
  named `harness/results/FINAL-breadth/summary.json` and
  `FINAL-depth/summary.json` as its sources. Those are assembly paths in
  `harness/scripts/overnight.sh` for a rerun that never produced a reported
  campaign, and the first directory was never created. The figures were always
  right — `stage2-final` and `stage3-final-all`, 220 + 314 = 534 counted runs,
  chosen deliberately — but the provenance beside them pointed somewhere else,
  and `harness/results/` was gitignored, so nothing on the page could be
  checked and reconstructing which cut was canonical cost more than the page is
  worth. Those two summaries are now tracked (380 KB, the four files; the
  206 MB of `.eval` transcripts stay ignored), and
  `tests/test_measured_results.py` recomputes every figure on the page from
  them — the success table, the combined row, the run total, the z-test, the
  payload, context and token tables, tool errors, guard rejections,
  truncations, wall time, and the per-template scenarios including the
  `github_to_linear` result that goes against Charter. The campaign names are
  asserted too, so replacing the reported cut is an edit to the suite rather
  than a directory somebody picks out of an `ls`.


- **The quickstart's last step could not be followed.** Its factory declared no
  `pack`, so the tool it builds was refused by every adapter — `DeclarationError:
  Tool 'list_messages' declares no pack` — at the final step, after four that
  worked. The factory now sets `pack`, and [framework
  adapters](https://docs.r28.ai/charter/using/adapters) says up front that
  publishing needs one where calling does not.

- **Stale counts and sizes across the documentation.** Five pages still said
  eleven or twelve packs against fifteen shipped; the tool-discovery page priced
  the whole surface at 209,000 tokens against a real 458,189; the quickstart
  printed an egress map with four visible fields where the runtime prints
  fifteen. Every figure is now recomputed from the package by
  `tests/test_doc_figures.py` and the count checks in `tests/test_landing_docs.py`,
  which covered `index.mdx` alone and is why `index.mdx` was the one page that was
  right.

- **Monorepo leftovers in the shipped sdist.** `test_pack_tavily_live.py` and
  `test_pack_firecrawl_live.py` told the reader to `cd oss` and read `oss/.env`,
  a path that has not existed since Charter moved to its own repository. The code
...
- **`body_case` defaults to `"camel"`,** which is right for Google and wrong for
  ...
  produces the vendor's own `400 Extra inputs are not permitted, field:
  'maxTokens'` on the first call. The [casing
  cascade](https://docs.r28.ai/charter/tools/key-case-cascade), the API-key
  factory page and the pack-writing skill now all say to set it explicitly.

### Added

- **`ToolSession(tools, mode=...)` and `Tool.with_mode(mode)`.** `Mode` takes an
  arbitrary string, so one resource model already served a version, an experiment
  arm, a plan tier, a region or a read/write scope. Only the pack author could
  reach it. A deployer holding a pack that already ships had no mode parameter
  anywhere: not on `ToolSession`, not on `configure()`, not on `derived()`. The
  way to key a surface to a Clerk plan was to hand-build one tool set per tier
  and choose between them:

  ```python
  TOOLSETS = {"free": [t.derived(...) for t in gmail.TOOLS], "pro": gmail.TOOLS}
  ```

  That is glue code, in a library whose pitch is that you do not write glue, and
  it was asymmetric: credentials already resolved per end user and the schema did
  not. Now `ToolSession(reports.TOOLS, mode=plan_of(user))` resolves every tool it
  holds at that label, and everything downstream sees the resolved copies: the
  partition that decides what is deferred, the `ToolSearch` catalogue, what a load
  hands back, what `dispatch` validates against, and the `egress_map()` a reviewer
  runs over the session.

  Four decisions, each one pinned by a test.

  - **The published name does not change.** A `pro` variant of `messages_list` is
    still `messages_list`. `charter.naming` argues that a tool's published name is
    a function of that tool alone, and a mode suffix on half a surface breaks
    every allow-list and log grep over it in silence.
  - **The session mode joins the tool's own; it does not replace it.** The two
    ends of `Mode` answer different questions. The author's `mode=` says which
    *operation* a tool is — `create` replaces and `update` merges — and no
    deployment changes that; the session's label says which *surface* the
    conversation is. Both are in force at once (`Tool.modes` is the pair), so a
    session label only ever **adds** the fields carrying it. That is the property
    that makes the parameter usable against a pack you have not read.
  - **It resolves once, when the session is built**, not per call and not through
    a ContextVar. Credentials resolve per call because the subject can change
    between two calls of one conversation. A schema cannot: it was serialised into
    a prompt the model is still reading. A surface that has to change means a new
    session.
  - **A separate method, not `derived(mode=...)`.** `derived` promises that a
    projection can only ever remove, which is the whole reason one is safe to hand
    to a deployment without reading the pack. A mode rebuilds the view from the
    declarations and can widen against the tool it came from, so folding it in
    would have cost that sentence for one signature. Both go through one copy
    path, so neither can quietly drop a constructor argument the way projections
    once dropped the factory's schema store.

  No label widens past the declaration. `response_only` and `disabled` are refused
  without consulting the mode at all, so there is no string a deployment can pass
  that reaches a field the author withheld unconditionally. What moves is which of
  the author's *optional* surfaces this conversation gets.

  A projection and a mode compose in either order and keep both restrictions. A
  mode that would hide a field the tool pins raises `DeclarationError` where it is
  asked for, rather than failing every call: the pinned value is merged into the
  executed view and revalidated there, and that view forbids extras.

  `egress_map` reports both ends of the marker per row — `mode` (the operation the
  pack declared), `session_mode` (the surface this deployment resolved it at) and
  `modes` (what actually filtered the view) — because they are different claims and
  a reviewer needs each.

  `mode=None` is unchanged in every respect, and by identity rather than by
  equality. A session given no mode holds the very tool objects it was handed, so
  the views a pack built at import are reused and nothing is copied. Variants
  share the store their factory owns, so a surface's worth of tools at one mode
  build their shared types once between them rather than once each.

### Changed

- **`egress_map` and `conflict_map` are keyed by `<pack>__<tool>`, not the bare
  tool name.** Both are audit artifacts and both silently lost rows. Sixteen tool
  names collide across the fifteen shipped packs (`comments_list` alone is
  declared by gdrive, linear and notion), so the egress map of one assembled
  surface held **529 rows for 549 tools** and named none of the twenty it
  dropped. A report that loses rows is worse than no report: the reviewer cannot
  tell by reading it, which is the one failure mode an artifact whose whole job
  is to be read must not have. `conflict_map` had the same defect, latent only
  because three shipped tools declare a conflict and none of them collided yet.

  The new key is [`report_key`](https://docs.r28.ai/charter/reference/naming#report_key),
  now public: `qualified_name` where the tool declares a pack, the bare name
  where it does not. `qualified_name` refuses that fallback and is right to,
  because a half-qualified *tool surface* makes the pack a substring of some
  names and not others and any filter over it misses the rest in silence. A
  report is not a tool surface: nothing dispatches on these keys, each row now
  carries `name` and `pack` of its own, and refusing would have put the artifact
  out of reach of the hand-built tool the quickstart teaches. Both rules keep the
  part that matters, which is that the key is a function of the tool alone.

  Two tools that would still share a row now raise `DeclarationError` instead of
  one overwriting the other.

  **Migration.** `report["messages_send"]` becomes `report["gmail__messages_send"]`.
  Where you have the tool, `report[report_key(tool)]`; where you are iterating,
  `entry["name"]` and `entry["pack"]` are new fields on every row. The rendered
  `format_egress_map` and `format_conflicts` headings change the same way.

- **`egress_map` and `conflict_map` take a map of name to tool as well as an
  iterable.** `ToolSession.tools` is a dict, and the mode documentation sends a
  reviewer to audit exactly that — but iterating a dict yields its *keys*, so the
  documented call died on `'str' object has no attribute 'pack'`, which names
  neither the mistake nor the fix. `egress_map(session.tools)` now maps the
  surface one conversation resolved. The mapping's own keys are discarded, since
  a row is keyed by `report_key` either way.

### Fixed

- **A projection could change the shape of the request body.** The decision to
  unwrap a single `Body()` field was made twice from two different schemas:
  `_extract_http_params` counted the body fields of the view being *executed*,
  and `_create_auto_transformer` counted them on the *declared* schema. Those
  agree until something removes a field from the view, and two supported things
  do — `Tool.derived(drop=...)` and `Mode("response_only")` on a sibling. Then
  one said "one body field, so unwrap it" and the other said "two, so keep the
  names", and the request went out as neither. `github.pulls_request_reviewers`
  narrowed with `drop={"team_reviewers"}` sent a bare `["alice"]` where GitHub
  documents `{"reviewers": ["alice"]}`, rejecting every call; `stripe.disputes_update`
  narrowed the same way sent `uncategorized_text=x` instead of
  `evidence[uncategorized_text]=x`, which Stripe accepts with a 200 and ignores,
  so the evidence was never submitted. 55 shipped tools were exposed to this the
  moment anyone narrowed them, and a `Format` field additionally leaked its
  untransformed value into the body root. The count now comes from one function,
  `charter.types.markers.declared_body_fields`, read once off `args_schema` and
  carried to the wire — so the body shape is a property of the declaration, which
  is the rule the count already followed one level down.

- **An api-key credential followed a redirect off-origin.** `follow_redirects=True`
  relied on httpx dropping `Authorization` when a redirect leaves the origin,
  which it does — and that is the whole of its rule. An api-key tool's credential
  is `x-api-key`, `X-Shopify-Access-Token`, whatever the API named it, and httpx
  cannot know which of the headers it was handed is the secret, so the key was
  sent to whatever host the `Location` named. `api_key_tool_factory`'s own
  docstring promised the opposite, having inherited the sentence from the bearer
  factory where it is true. The runtime now walks the redirects itself — each hop
  built by httpx's own `Response.next_request`, so the 303 method downgrade, the
  body, relative `Location` resolution and its `Authorization` stripping are
  unchanged — and drops the headers it knows to be credentials whenever the next
  hop leaves the current origin. Running out of the budget raises `APIError`
  rather than `httpx.TooManyRedirects`, for the same reason as the `ToolSearch`
  entry below: an httpx exception escaping here would pass every adapter. The
  budget is 20 followed redirects, matching httpx. No shipped pack combined
  api-key auth with `follow_redirects`; the three GitHub tools that set it are
  bearer.

- **The key-case cascade stopped at the root.** Once a model is dumped to a dict
  every marker on it is gone, and the conversion walked dicts. So a nested
  model's `__case__` and a nested field's `Case()` were silently inert below the
  top level — including the "Schema-level override (nested model)" example in
  `docs/tools/key-case-cascade.md`, which documented a conversion that did not
  happen, and the reverse case, where a nested model pinned to `snake` under a
  camel factory was camel-cased anyway. `WireName` had been given a special case
  for the one shape that hurt most, the unwrapped body, which fixed a third of
  the problem and left the `Case()` on the field beside it still ignored. The
  cascade is now resolved from the model *graph* before anything is serialised
  (`charter.execution.casing.key_plan`) and applied at every depth. Path
  parameters are deliberately excluded: a path key names a hole in `url_template`
  rather than a key on the wire, so only `WireName` and `path_case` reach it. No
  shipped pack declares `__case__` or a nested `Case()`, and all 549 shipped
  tools send byte-identical requests across this change. A plan is derived from
  class declarations, so it is built once per model and endpoint case and kept
  behind a weak map rather than rebuilt per request — the walk is 1.4ms on a
  schema the size of `linear.issues_list`, which the wire layer would otherwise
  pay twice on every call.

- **Header precedence was case-sensitive, and HTTP is not.** The request's
  headers were assembled in a plain dict, so `{"Idempotency-Key": ...}` from
  `static_headers` and `{"idempotency-key": ...}` from the per-call `headers=`
  channel were two keys in the dict and one header on the wire: httpx joined them
  with a comma and the caller's value — documented here as the final word —
  neither won nor lost. The same shape sent an api-key credential twice when
  `static_headers` spelled it differently from `api_key_headers`, which is a
  malformed credential rather than a cosmetic duplicate. Assembled in an
  `httpx.Headers` now, which replaces by lowercased name, so the cascade means
  what it says whatever anyone spelled.

- **`ToolSearch`'s own arguments were never validated.** It is answered by the
  session rather than by a `Tool`, so it never passed through `validate_input`,
  and a bare `int()` was the only thing reading `max_results`. A model answering
  an integer parameter with a word — which they do — raised `ValueError`, and
  every adapter catches `CharterError` and nothing else, so it went straight past
  the OpenAI loop and the MCP server instead of coming back to the model as a
  correctable mistake. Now a `ToolValidationError` written for the model to read.
  A whole-number float is accepted, since JSON has one number type; a boolean is
  not, because `bool` is an `int` in Python and `max_results: true` would
  otherwise quietly mean one.

- **A model nested in a container never reached the wire.** `_extract_http_params`
  dumped a `BaseModel` field and the models one level inside a `list`, and
  stopped there. A `Dict[str, SomeModel]` field had no branch at all, and a
  `List[List[SomeModel]]` had one level too few, so both reached `json.dumps`
  still holding model instances and raised a bare `TypeError` out of the
  encoder — past every adapter, none of which catch it. The unwrapping is
  recursive now, so it covers the annotations nobody happened to try as well as
  the ones they did. Found while auditing the body-shape fix above, not by it.

- **Measuring a redirected call could fail it.** A request built for a redirect
  hop carries a stream that was never read, and `Request.content` raises
  `RequestNotRead` on one. `call_api` read it to size the request, so a call
  that followed a redirect died *after* a perfectly good response had
  arrived — and only when something was watching, since the sizing is skipped
  when no sink, collector or INFO handler is listening. Guarded now, under the
  rule `Tool._report` already follows: measurement must never be able to fail
  the thing it measures.

- **A `.` path segment deleted its own segment.** `..` was refused and `.` was
  not, but both are normalisation tokens rather than names: httpx resolves them
  before the request leaves, so `resource/{seg}/sub` with `seg="."` was sent as
  `resource/sub`. A weaker version of what `..` does and the same kind of thing —
  a value editing the endpoint instead of filling a hole in it. Refused now, with
  the same error.

- **An empty `mode` widened the surface instead of narrowing it.**
  `should_include_field` guards on `if mode`, so *any* falsy mode read as "no
  mode at all" — which is the **widest** view, where every custom label is inert
  and every tiered field is offered. `None` means that on purpose and
  `ToolSession` checks for it by identity before it copies anything. An empty
  string does not: it arrives from a runtime value, and
  `ToolSession(tools, mode=plan_of(user))` where `plan_of` returns `""` for the
  smallest tier served that user every tier's fields. It overrode the *pack's*
  label too — a tool built `mode="free"` resolved at `""` went from one field to
  every field its schema declares. That is the one direction a deployer's label
  must never move egress, and the only direction a wrong one can: a misspelt
  label fails closed, matching nothing and withholding everything optional.
  Refused now with `DeclarationError`, in `Tool.__init__` where `derived()`,
  `with_mode()` and a hand-built tool converge, and once more on `ToolSession`
  itself for the surface that holds no tools to reach it through.

- **A mode could delete a field the API requires, leaving a tool that could not
  be called.** `Mode` moves visibility and requiredness is a separate axis, so a
  label could take a *required* field out of the view and nothing put it back:
  the tool constructed, validated a call that never mentioned the field, and sent
  a body without it — a 400 on every call, naming a field the deployer never saw
  offered. `derived(drop=...)` already refused precisely that removal by path.
  Reaching the same end through a label went unchecked, which mattered little
  while only the pack author could write `mode=` and matters now that
  `with_mode` and `ToolSession` hand that lever to a deployer, who has no way to
  know which label makes which tool uncallable. Refused at declaration, naming
  the label and the field. Only the deployer's half: a required field marked
  `Mode("response_only")` or `Mode("disabled")` is withheld at every label, so no
  mode did that and none can fix it — the test is "present with no mode, absent
  at this one". No shipped pack is affected; across the fifteen of them no
  required field carries a custom `Mode`.

- **A pin on a field `Mode` withholds was accepted and then failed every call.**
  `derived(pin={...})` checks that the value satisfies the field it is pinned to,
  against the *executed* view, and it skipped a field it could not find there. A
  field marked `Mode("response_only")` or `Mode("disabled")` is not in that view,
  so pinning one built a tool that constructed without complaint and rejected
  every call with `Extra inputs are not permitted` naming a field the caller
  never set: the pin is merged into the executed view and revalidated, and that
  view forbids extras. Refused at declaration now, with the two cases told apart.
  A field withheld at every mode says so and says no label fixes it; a field
  carrying a custom mode names the mode that hid it, because the pin and the mode
  are each defensible alone and it is the pair that is not. Found while building
  `with_mode`, which reaches the same defect from the other side.
...
- **`Tool.derived()` — a projection of a tool, narrowed by whoever deploys it.**
  `keep`, `drop` and `pin` narrow a tool you did not declare and cannot edit, and
  they need nothing from the pack to do it: they work on fields and values rather
  than on labels somebody thought to write.
