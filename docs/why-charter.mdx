...
- **Field policy.** `drafts_create` offers the model five fields and withholds nine. `payload`, `snippet`, `labelIds` and the rest are absent from the type the model is handed, not filtered out afterwards. You can [print that map](/boundary/egress-control).
...
- [Measured results](/guarantees/measured-results) — 536 runs against live APIs, with the discarded column shown
