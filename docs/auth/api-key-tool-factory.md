...
<Note>
Most APIs that authenticate this way document `snake_case` bodies, and
`body_case` defaults to `"camel"` — so set it on the factory alongside the
headers, or the first call comes back as the vendor's 400 naming a field you did
not send. [The casing cascade](/tools/key-case-cascade) has the whole rule.
</Note>
