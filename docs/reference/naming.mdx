...
installed, and a host composes `mcp__<server>__<tool>` on top — the MCP entry
point names its server `charter`, so the result reads
`mcp__charter__gcalendar__events_list`.

The host builds `<server>` from the key the server was *configured under*, and
the whole composed name has to fit the 64 characters OpenAI allows a function
name. One server holding every pack peaks at 61; a server named after the pack
it serves spends eight more on saying the pack a second time and puts three
names over. Which is why `--pack` [takes a list](/using/mcp#add-it-to-your-client).
...
The row a tool gets in an audit artifact.
...
report_key(gdrive.comments_list)   # "gdrive__comments_list"
report_key(notion.comments_list)   # "notion__comments_list"
```

What [`egress_map`](/reference/observability#egress_map) and
[`conflict_map`](/reference/observability#conflict_map) key their dicts by. The
same qualification as above wherever the tool declares a pack, and the bare name
where it does not, because a report is not a tool surface: nothing dispatches on
these keys, each row carries its `pack` and `name` beside the data, and refusing
a pack-less tool would put the artifact out of reach of a tool built by hand.

Both maps raise [`DeclarationError`](/reference/errors#declarationerror) rather
than let two tools share a row. That guard and this key replace keying on the
bare name, under which sixteen colliding names across the fifteen shipped packs
turned one assembled surface into 529 rows for 549 tools, naming none of the
twenty it dropped.
