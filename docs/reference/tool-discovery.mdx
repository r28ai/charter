...
The fifteen shipped packs come to about 458,000 tokens of schema, while a trimmed
...
# (82, 1)        eighty-two tools, one definition: ToolSearch

session.search("+stripe refund")
# {"loaded": ["stripe__refunds_create", "stripe__refunds_list", ...], "note": "..."}

len(to_openai_tools(session))
# 6
```

Eighty-two tools cost 893 tokens on the first turn instead of 36,710.
...
`mode` resolves every tool's view at that [`Mode`](/reference/markers#mode)
instead of the one it was built with, by calling
[`with_mode`](/reference/tool#tool-with_mode) on each. `None`, the default,
leaves every tool exactly as it was given, by identity.

```python
session = ToolSession(reports.TOOLS, mode=plan_of(request.user))
```

That is the line that replaces a `TOOLSETS = {"free": [...], "pro": [...]}` dict
and the code choosing between its entries. The pack declares which fields belong
to which surface once, and the deployment names a surface per conversation.

The session mode is in force *alongside* a mode the tool was built with, not
instead of it. The author's `mode=` says which operation a tool is — `create`
replaces and `update` merges — and that is not a deployment decision; the
session's label says which surface the conversation is. So a session label only
ever **adds** the fields carrying it, on every tool, whatever the pack declared.
You do not have to read a pack to predict what keying it to a tier will do.

**It resolves once, here.** Credentials resolve per call, through a ContextVar,
because the subject can change between two calls of one conversation. A schema
cannot: it was serialised into a prompt the model is still reading, and
parameters that change under a model already shown them is a tool it will call
wrong. The session is the per-conversation object, so this is where the
per-conversation answer belongs. A surface that has to change means a new
session.

Everything downstream sees the resolved tools: the partition that decides what is
deferred, the `ToolSearch` catalogue, what a load hands back, what `dispatch`
validates against, and the [`egress_map()`](/reference/observability) a reviewer
runs over `session.tools`. Resolving costs one schema build per tool per mode,
paid at construction like the rest of the session, and tools from one pack share
the store their factory owns, so a surface's worth of tools at one mode build
their shared types once between them.

A field the mode withheld is not merely absent from the prompt. It is absent from
the type `dispatch` validates against, which forbids extras, so a tier the
deployment did not buy is refused rather than discouraged.

`mode` is a label or `None`, and an empty string is refused rather than read as
`None` — `None` is the *widest* view, so a plan lookup that came back empty would
otherwise be served every tier. A label that withholds a field the API requires is
refused too, since the request would go out without it. Both raise
[`DeclarationError`](/reference/errors#declarationerror) where the session is built.
