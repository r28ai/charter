...
are not the ones you would guess. In the Linear pack, `comment_create` costs 911
...
Seventeen tools land between 46,870 and 48,231. The eighteenth is `project_update`
...
search = linear.search_issues.derived(
    name="search_issues",
    drop={"filter"},
)
```
...
```
linear.search_issues    47,026 tokens  ->  396    (119x)
```
...
```
gdocs.documents_batch_update    8,183 tokens  ->  1,750    (79% smaller)
```
...
Sending `linear.search_issues` unnarrowed to Fireworks returns:
...
```
OpenAI    563,701 bytes    77 $defs per tool, 79 distinct across the whole payload
MCP       563,602 bytes    77 $defs per tool
```
...
here is reachable from that tool's own properties: `linear.teams_list` 77 of 77,
...
```
as shipped    563,701 bytes    77 $defs per tool
projected       4,160 bytes     1 $def  per tool
```

136x, because the saving lands once per tool rather than once. Progressive
