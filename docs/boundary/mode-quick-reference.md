...
```python
gmail(name="messages_send", mode="A", ...)   # the pack author, per tool
tool.with_mode("pro")                        # a deployer, per tool
ToolSession(tools, mode="pro")               # a deployer, per conversation
```

The session's mode joins the tool's own; both are in force, so a session label
only ever adds the fields carrying it. No mode reaches a `response_only` or
`disabled` field, at any label. See
[the mode system](/boundary/mode-system#custom-modes).
