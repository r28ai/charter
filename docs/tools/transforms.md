...
```mermaid
graph TD
    A[API specification] -->|exact schema in your code| B[Source schema + Format markers]
    B -->|create_llm_schema| C[LLM-friendly schema]
    C -->|LLM provides semantic input| D[Tool input]
    D -->|auto-transformer| E[Transform detection]
    E -->|apply_transform| F[Wire format conversion]
    F -->|call_api| G[API call]
```
