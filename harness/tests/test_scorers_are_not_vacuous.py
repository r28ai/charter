...
def test_the_variants_reach_the_phase_three_corpus():
    assert len(SCENARIOS) >= MIN_INSTANCES, (
        f"{len(SCENARIOS)} instances; phase 3 needs at least {MIN_INSTANCES}"
    )
    assert len({s.id for s in SCENARIOS}) == len(SCENARIOS)
    for s in SCENARIOS:
        if s.variant:
            assert s.id == f"{s.template}@{s.variant}"
            assert set(s.params) == set(type(s).defaults), (
                f"{s.id} params are not exactly the template's knobs"
            )
...
def test_judge_accepts_the_example(scenario: Scenario):
    ...
    assert verdict.correct, (
        f"scorer for {scenario.id} rejects its own correct example: {verdict.explanation}"
    )
...
def test_judge_rejects_every_wrong_observation(scenario: Scenario):
    ...
    assert len(wrongs) >= MIN_WRONG, (
        f"{scenario.id} supplies {len(wrongs)} wrong observation(s); at least {MIN_WRONG} are required"
    )
    vacuous = []
...
def test_wrong_observations_are_actually_different(scenario: Scenario):
    ...
    for reason, wrong in scenario.wrong_observations(expected, observed):
        assert wrong != observed, (
            f"{scenario.id}: wrong observation {reason!r} equals the correct one"
        )
...
def test_judge_is_pure_and_explains_failures(scenario: Scenario):
    ...
    for _, wrong in scenario.wrong_observations(expected, observed):
        ...
        assert verdict.explanation.strip(), (
            f"{scenario.id}: an INCORRECT verdict carries no explanation"
        )
...
def test_prompt_names_no_tool(scenario: Scenario):
    ...
    for forbidden in ("_api", "_graphql", "ainvoke", "tool"):
        assert forbidden not in prompt.casefold(), (
            f"{scenario.id} prompt leaks tooling vocabulary: {forbidden!r}"
        )
