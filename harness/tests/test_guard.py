...
def test_raw_arm_only_reaches_endpoints_the_pack_declares():
    ...
    assert (
        decide(
            "gmail_api",
            {"method": "GET", "path": "https://gmail.googleapis.com/gmail/v1/users/me/threads?q=x"},
            ctx,
        )
        is None
    )
    # sending: never
    assert decide(
        "gmail_api", {"method": "POST", "path": "gmail/v1/users/me/messages/send", "body": {}}, ctx
    )
    ...
    assert decide(
        "gmail_api", {"method": "POST", "path": "gmail/v1/users/me/messages/abc/trash"}, ctx
    )
...
def test_raw_arm_slack_search_is_withheld_like_the_charter_tool():
    ...
    assert (
        decide(
            "slack_api",
            {"method": "GET", "path": "conversations.history", "query": {"channel": "C1"}},
            ctx,
        )
        is None
    )
    assert decide(
        "slack_api", {"method": "GET", "path": "search.messages", "query": {"query": "x"}}, ctx
    )
    assert (
        decide(
            "slack_api",
    ...
    assert decide(
        "slack_api",
        {"method": "POST", "path": "chat.postMessage", "body": {"channel": "C777", "text": "x"}},
        ctx,
    )
...
def test_raw_arm_github_writes_are_sandboxed_by_path():
    ...
    assert (
        decide(
            ...
            {
                "method": "POST",
                "path": "repos/harness-owner/harness-42/issues",
                "body": {"title": "x"},
            },
            ctx,
        )
    ...
    assert decide(
        "github_api",
        {"method": "POST", "path": "repos/harness-owner/charter/issues", "body": {"title": "x"}},
        ctx,
    )
    assert decide(
        "github_api",
        {"method": "PATCH", "path": "repos/other/harness-42/issues/1", "body": {}},
        ctx,
    )
    # repo creation is not a Charter tool, so the raw arm cannot do it either
    assert decide(
        "github_api", {"method": "POST", "path": "user/repos", "body": {"name": "harness-new"}}, ctx
    )
...
def test_raw_arm_calendar_deletes_only_touch_seeded_events():
    ...
    assert (
        decide(
            "gcalendar_api",
            {"method": "DELETE", "path": "calendar/v3/calendars/primary/events/ev1"},
            ctx,
        )
        is None
    )
    assert decide(
        "gcalendar_api",
        {"method": "DELETE", "path": "calendar/v3/calendars/primary/events/dentist"},
        ctx,
    )
...
def test_raw_arm_graphql_documents_are_not_inspected():
    ...
    assert (
        decide("linear_graphql", {"query": 'mutation { issueDelete(id: "x") { success } }'}, ctx)
        is None
    )
...
@pytest.mark.parametrize(
    "pack", ["gmail", "gcalendar", "gsheets", "gdocs", "stripe", "github", "slack", "firecrawl"]
)
def test_every_charter_endpoint_is_reachable_on_the_raw_arm(pack):
    ...
    ctx = _ctx(
        "raw",
        [pack],
        owned={"slack.channel": ["C1"], "gcalendar.event": ["X"], "gmail.thread": ["X"]},
    )
    for ep in endpoints_for(pack):
        ...
        assert (
            decide(raw_tool_name(pack), {"method": ep.method, "path": path, "body": body}, ctx)
            is None
        ), (
            ep,
            path,
...
def test_the_same_holds_on_the_raw_arm():
    ...
    assert (
        decide("gcalendar_api", {"method": "DELETE", "path": f"{events}/agent-made"}, ctx) is None
    )
