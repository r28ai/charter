...
async def test_a_reaction_is_added_read_and_removed(needs, world, trash, run_tag, scope_skip):
    ...
    try:
        got = await slack.reactions_get.ainvoke({"channel": channel, "timestamp": ts, "full": True})
...
async def test_a_thread_reads_back_in_order(needs, world, trash, run_tag):
    ...
    reply = await _post(world, trash, channel, f"live reply {run_tag}", thread_ts=parent)
...
async def test_a_channel_is_created_and_somebody_is_invited(
    ...
    human = next(m for m in people["members"] if not m.get("is_bot") and m["name"] != "slackbot")
...
async def test_a_direct_message_channel_can_be_opened(needs, world, scope_skip):
    ...
    try:
        opened = await slack.conversations_open.ainvoke({"users": human["id"], "return_im": True})
    except CharterError as exc:
