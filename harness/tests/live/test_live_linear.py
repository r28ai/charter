...
async def _issue(world, trash, run_tag: str, *, title: str, **fields) -> dict:
    ...
    made = await linear.issue_create.ainvoke(
        {"variables": {"input": {"teamId": await _team_id(world), "title": title, **fields}}}
    )
    issue = made["issue"]
...
async def test_the_issue_filters_narrow_rather_than_erroring(
    needs, world, trash, run_tag, settings
):
