...
async def _product(world, trash, run_tag: str, *, title: str) -> dict:
    ...
    made = await shopify.product_create.ainvoke(
        {
            "variables": {
                "product": {"title": title, "tags": [f"live-{run_tag}"], "status": "ACTIVE"}
            }
        }
    )
...
async def test_a_variant_carries_its_sku_inside_the_inventory_item(needs, world, trash, run_tag):
...
async def test_stock_moves_by_a_delta_and_a_reused_key_moves_nothing(needs, world, trash, run_tag):
    ...
    fulfilled = await shopify.fulfillment_create.ainvoke(
        {
            "variables": {
                "fulfillment": {
                    "lineItemsByFulfillmentOrder": [{"fulfillmentOrderId": open_orders[0]["id"]}],
