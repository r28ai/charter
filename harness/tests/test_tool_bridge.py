...
def test_raw_arm_tools_pass_inspect_validation(pack):
    ...
    for tooldef in RawArm().tools([pack], wiring):
        ...
        assert tooldef.description.count("\n- ") >= 1, "the endpoint list must be derived, not empty"
...
def test_nested_models_keep_their_fields():
    ...
    email = next((d for n, d in defs.items() if {"to", "subject", "body"} <= set(d.get("properties", {}))), None)
...
def test_the_linear_filter_comparators_are_strings():
    ...
    params = charter_tooldef(next(t for t in linear.TOOLS if t.name == "issues_list")).parameters.model_dump(exclude_none=True)
    defs = params["$defs"]
    ...
    for comparator in ("key", "name"):
        ...
        assert {o.get("type") for o in eq["anyOf"]} == {"string", "null"}, f"team.{comparator}.eq is {eq}"
