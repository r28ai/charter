    ...
    if isinstance(result, dict):
        for key in keys:
            if value := result.get(key):
                return str(value)
    raise RuntimeError(f"no {' / '.join(keys)} in response; cannot clean up (got {type(result).__name__})")
...
async def gsheets_write(w: World) -> str:
    created = await call("gsheets", "spreadsheets_create", {"spreadsheet": {"properties": {"title": STAMP}}})
    ...
    try:
        ...
        read = await call("gsheets", "spreadsheets_values_get", {"spreadsheetId": sheet_id, "range": "A1:B1"})
...
async def slack_write(w: World) -> str:
    ...
    posted = await call("slack", "chat_post_message", {"channel": channel, "text": f"{STAMP} (preflight)"})
...
async def slack_reactions(w: World) -> str:
    ...
    channel = await w.slack.channel_id()
    posted = await call("slack", "chat_post_message", {"channel": channel, "text": f"{STAMP} (reaction probe)"})
    ts = ident(posted, "ts", "messageTs")
    try:
        await call("slack", "reactions_add", {"channel": channel, "timestamp": ts, "name": "white_check_mark"})
        await call("slack", "reactions_remove", {"channel": channel, "timestamp": ts, "name": "white_check_mark"})
...
async def stripe_write(w: World) -> str:
    ...
    created = await call("stripe", "customers_create", {"name": STAMP, "description": "charter preflight"})
    customer = ident(created, "id")
    try:
        await call("stripe", "customers_update", {"customer": customer, "description": "charter preflight (updated)"})
...
async def gdocs_write(w: World) -> str:
    ...
    try:
        await call(
            ...
            {
                "documentId": doc_id,
                "body": {"requests": [{"insertText": {"location": {"index": 1}, "text": f"{STAMP}\n"}}]},
            },
        )
...
async def linear_write(w: World) -> str:
    ...
    created = await call(
        "linear",
        "issue_create",
        {"variables": {"input": {"teamId": team_id, "title": STAMP, "description": "charter preflight"}}},
    )
    ...
    try:
        await call("linear", "issue_update", {"variables": {"id": issue_id, "input": {"title": f"{STAMP} (updated)"}}})
...
async def shopify_write(w: World) -> str:
    ...
    product = await call("shopify", "product_create", {"variables": {"product": {"title": STAMP, "tags": ["charter-preflight"]}}})
    product_id = ident(product.get("product", product) if isinstance(product, dict) else product, "id")
    try:
        customer = await call(
            "shopify",
            "customer_create",
            ...
            {"variables": {"input": {"email": f"{STAMP}@example.com", "firstName": "Charter", "tags": ["charter-preflight"]}}},
        )
        customer_id = ident(customer.get("customer", customer) if isinstance(customer, dict) else customer, "id")
        try:
...
async def firecrawl_scrape(w: World) -> str:
    page = await call("firecrawl", "scrape", {"url": "https://example.com", "formats": ["markdown"]})
...
PROBES: tuple[Probe, ...] = (
    ...
    Probe("gsheets", "create+write+read", ("spreadsheets_create", "spreadsheets_values_update", "spreadsheets_values_get"), gsheets_write),
    Probe("gdocs", "create+update+read", ("documents_create", "documents_batch_update", "documents_get"), gdocs_write),
...
async def main() -> int:
    ...
    parser.add_argument("--skip-billable", action="store_true", help=f"skip probes that spend credits: {', '.join(sorted(BILLABLE))}")
    ...
    for probe in PROBES:
        ...
        if probe.pack not in available:
            ...
            print(f"  {SKIP:4} {probe.capability:18} {touched:52} not configured; set {missing} in {default_env_file()}")
    ...
    if failures:
        ...
        print("Fix these before spending inference budget - each one is a scenario that would die mid-campaign.")
        return 1
    if unconfigured:
