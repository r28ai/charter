...
SYSTEM_LABELS = {"INBOX", "SENT", "DRAFT", "SPAM", "TRASH", "UNREAD", "STARRED", "IMPORTANT", "CHAT"}
...
async def main() -> int:
    ...
    parser.add_argument("--delete", action="store_true", help="actually remove them (default: scan only)")
    ...
    total = 0
    try:
        for name, fn in (("gmail", gmail), ("drive", drive), ("linear", linear), ("github", github)):
            try:
