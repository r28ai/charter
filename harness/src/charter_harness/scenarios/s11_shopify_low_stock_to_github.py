...
class ShopifyLowStockToGitHub(Scenario):
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        for title, inventory in self.params["products"]:
            p = await world.shopify.product_create(
                title=f"{title} ({ns})", tags=[ns, "harness"], inventory=inventory
            )
    ...
    def wrong_observations(
        ...
        return [
            (
                "a well-stocked product is listed",
                {"issues": [dict(issue, body=issue["body"] + "- Enamel mug (hx): 12 units\n")]},
            ),
            (
                "a low-stock product is missing",
                {"issues": [dict(issue, body="- Canvas tote (hx): 2 units\n")]},
            ),
