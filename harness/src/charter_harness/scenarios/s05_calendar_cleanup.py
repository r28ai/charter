...
class CalendarCleanup(Scenario):
    ...
    variants = {
        "one_of_three": {"events": [("Sprint planning", 10, False), ("[CANCELLED] 1:1 with Sam", 13, True), ("Release go/no-go", 16, False)]},
        "three_of_five": {
            "events": [
                ("[CANCELLED] Vendor demo", 9, True),
                ("Sprint planning", 10, False),
        ...
        "cancelled_last": {"events": [("Standup", 9, False), ("Design review", 11, False), ("[CANCELLED] Offsite prep", 14, True), ("[CANCELLED] Happy hour", 17, True)]},
        "all_but_one": {"events": [("[CANCELLED] A", 9, True), ("[CANCELLED] B", 10, True), ("Keep me", 11, False), ("[CANCELLED] C", 12, True)]},
        "decoy_title": {
            "events": [
                ("[CANCELLED] Vendor demo", 9, True),
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        for title, hour, cancelled in self.params["events"]:
            ...
            event = await world.calendar.event_insert(
                summary=f"{title} ({ns})", start=iso(start), end=iso(start + timedelta(minutes=30)), timezone=tz
            )
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        for e in expected["events"]:
            ...
            state[e["id"]] = "deleted" if event is None or event.get("status") == "cancelled" else "present"
        return {"state": state}
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        for e in expected["events"]:
            ...
            if e["cancelled"]:
                conditions.append((got == "deleted", f"cancelled event {e['title']!r} is still on the calendar"))
            else:
