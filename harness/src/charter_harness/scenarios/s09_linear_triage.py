...
class LinearTriage(Scenario):
    ...
    packs = ("linear",)
    summary = (
        "Move URGENT issues to In Progress and comment on them; leave non-urgent issues untouched."
    )
    ...
    variants = {
        ...
        "all_urgent": {
            "issues": [
                ("URGENT: API latency spike", True),
                ("URGENT: login loop on mobile", True),
                ("URGENT: invoices double-sent", True),
            ]
        },
        "none_urgent": {
            "issues": [
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        target = await world.linear.state_by_name(
            "In Progress"
        ) or await world.linear.state_by_type("started")
        created = []
        ...
        return {
            "issues": {
                i["id"]: {
                    ...
                    "comments": [
                        c.get("body", "") for c in (i.get("comments") or {}).get("nodes", [])
                    ],
                }
                for i in issues
            }
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        for issue in expected["issues"]:
            ...
            if issue["urgent"]:
                conditions.append(
                    (
                        got["state"] == expected["target_state"],
                        f"{issue['identifier']} is in {got['state']!r}, expected {expected['target_state']!r}",
                    )
                )
                conditions.append(
                    (
                        any(
                            expected["comment"].casefold() in c.casefold() for c in got["comments"]
                        ),
                        f"{issue['identifier']} has no triage comment",
                    )
                )
            else:
                conditions.append(
                    (
                        got["state"] == expected["initial_state"],
                        f"non-urgent {issue['identifier']} was moved to {got['state']!r}",
                    )
                )
                conditions.append(
                    (not got["comments"], f"non-urgent {issue['identifier']} was commented on")
                )
            ...
            "issues": [
                {
                    "id": "i1",
                    "identifier": "ENG-1",
                    "title": "[hx] Update onboarding copy",
                    "urgent": False,
                },
                {
                    "id": "i2",
                    "identifier": "ENG-2",
                    "title": "[hx] URGENT: checkout returns 500 for EU cards",
                    "urgent": True,
                },
                {
                    "id": "i3",
                    "identifier": "ENG-3",
                    "title": "[hx] URGENT: password reset emails not delivered",
                    "urgent": True,
                },
            ],
    ...
    def wrong_observations(
        ...
        return [
            (
                "a non-urgent issue was moved",
                {"issues": {**o, "i1": {"state": "In Progress", "comments": []}}},
            ),
            (
                "an urgent issue was moved but not commented",
                {"issues": {**o, "i3": {"state": "In Progress", "comments": []}}},
            ),
            (
                "an urgent issue was commented but not moved",
                {"issues": {**o, "i2": {"state": "Backlog", "comments": ["Triaged: escalated"]}}},
            ),
            (
                "an urgent issue went to the wrong state",
                {"issues": {**o, "i2": {"state": "Done", "comments": ["Triaged: escalated"]}}},
            ),
        ]
