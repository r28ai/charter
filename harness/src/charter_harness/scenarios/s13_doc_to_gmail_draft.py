...
DISTRACTOR_SUBJECTS = ["Quick question about the dashboard", "Re: our call last week", "Feature idea"]


class DocToGmailDraft(Scenario):
    id = "doc_to_gmail_draft"
    packs = ("gdocs", "gmail")
    summary = "Create a Gmail draft reply on a labelled thread using the text of a template document."
    ...
    variants = {
        ...
        "shipping_with_distractors": {"template": SHIPPING_TEMPLATE, "key_phrase": SHIPPING_PHRASE, "distractors": 2},
    }
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        doc = await world.docs.document_create(f"{ns} billing reply template", text=self.params["template"])
        return {
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        conditions = [
            ...
            (len(on_thread) == 1, f"expected exactly one draft on the thread, found {len(on_thread)} (of {len(observed['drafts'])} drafts)"),
        ]
        if len(on_thread) == 1:
            d = on_thread[0]
            conditions.append((expected["sender"].casefold() in d["to"].casefold(), f"draft is addressed to {d['to']!r}, not the sender"))
            conditions.append((expected["key_phrase"].casefold() in " ".join(d["body"].split()).casefold(), "draft body does not carry the template text"))
    ...
    def example(self) -> tuple[Expected, Observed]:
        ...
        observed = {
            "drafts": [{"id": "r1", "thread_id": "t1", "to": "Ada Lovelace <ada.hx@harness.invalid>", "subject": "Re: [hx] Invoice balance looks wrong", "body": self.params["template"]}],
            "sent_in_thread": 0,
        }
    ...
    def wrong_observations(self, expected: Expected, observed: Observed) -> list[tuple[str, Observed]]:
        d = observed["drafts"][0]
        return [
            ("the draft is not on the thread", {**observed, "drafts": [dict(d, thread_id="other")]}),
            ("the draft is addressed to someone else", {**observed, "drafts": [dict(d, to="me@harness.invalid")]}),
            ("the draft body is not the template", {**observed, "drafts": [dict(d, body="Hi, looking into it.")]}),
