...
ISSUES = ["Rotate the staging TLS certificate", "Backfill missing invoice PDFs", "Upgrade Postgres to 17"]
MORE_ISSUES = ["Remove the legacy webhook signer", "Add rate limits to the export API", "Fix flaky checkout e2e test"]
DISTRACTORS = ["Plan the Q4 offsite", "Renew the design tool licences", "Archive stale feature flags"]
...
class LinearToSlackDigest(Scenario):
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        for title in self.params["issues"]:
            issue = await world.linear.issue_create(title=f"[{ns}] {title}", description="Seeded by the harness.")
            created.append({"id": issue["id"], "identifier": issue["identifier"], "title": f"[{ns}] {title}"})
        distractors = []
        for i in range(self.params["distractors"]):
            title = DISTRACTORS[i % len(DISTRACTORS)]
            issue = await world.linear.issue_create(title=f"{title} ({ns})", description="Seeded by the harness; not tagged.")
            distractors.append({"id": issue["id"], "identifier": issue["identifier"], "title": f"{title} ({ns})"})
        ...
        marker = await world.slack.post(f"[{ns}] digest requested")  # a timestamp to read history from
        return {
            "tag": f"[{ns}]",
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        conditions = [(i["identifier"].upper() in text, f"{i['identifier']} is missing from the digest") for i in expected["issues"]]
        conditions += [
            (d["identifier"].upper() not in text, f"untagged issue {d['identifier']} was listed") for d in expected.get("distractors", [])
        ]
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected: Expected = {
            ...
            "team_key": "ENG",
            "issues": [
                {"id": "a", "identifier": "ENG-40", "title": "[hx] Rotate the staging TLS certificate"},
