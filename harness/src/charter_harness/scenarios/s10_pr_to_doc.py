...
class PullRequestToDoc(Scenario):
    ...
    variants = {
        ...
        "deep_paths": {
            "files": [
                ("services/billing/app/domain/invoice/total.py", "def total(lines):\n    return sum(l.amount for l in lines)\n"),
                ("services/billing/app/domain/invoice/__init__.py", "from .total import total\n"),
                ("services/billing/tests/domain/test_total.py", "def test_total():\n    assert True\n"),
            ]
        },
        "similar_names": {
            "files": [
        ...
        "six": {"files": FILES + MORE_FILES + [("scripts/backfill_totals.py", "print('backfill')\n")]},
    }
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        for rel, content in self.params["files"]:
            path = f"{ns}/{rel}"
            await gh.content_put(path=path, content=content, message=f"{ns}: add {rel}", branch=branch)
            paths.append(path)
        pr = await gh.pull_create(title=f"[{ns}] Server-side invoice totals", head=branch, base=base, body="Seeded by the harness.")
        return {
            "repo": gh.full_name,
            "branch": branch,
            "pr_number": pr["number"],
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        for f in files:
            ...
            if doc:
                docs.append({"id": f["id"], "title": doc.get("title", ""), "text": world.docs.text_of(doc)})
        return {"docs": docs}
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected = {
            ...
            "paths": ["hx/src/billing/invoice.py", "hx/docs/billing.md", "hx/tests/test_invoice.py"],
    ...
    def wrong_observations(self, expected: Expected, observed: Observed) -> list[tuple[str, Observed]]:
        doc = observed["docs"][0]
        return [
            ("a changed file is missing", {"docs": [dict(doc, text=doc["text"].replace("- hx/docs/billing.md\n", ""))]}),
            ("the PR title is missing", {"docs": [dict(doc, text="Files changed:\n- hx/src/billing/invoice.py\n- hx/docs/billing.md\n- hx/tests/test_invoice.py\n")]}),
