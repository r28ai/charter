...
class SlackThreadToGitHub(Scenario):
    ...
    packs = ("slack", "github")
    summary = (
        "Turn a Slack bug-report thread into a GitHub issue and reply in-thread with the issue URL."
    )
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        issues = await world.github.issues_search_in_sandbox(
            expected["issue_title"], state="open", expect=1
        )
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        if len(issues) != 1:
            return Verdict.fail(
                f"expected exactly one open issue titled {expected['issue_title']!r}, found {len(issues)}"
            )
        issue = issues[0]
        body = " ".join(issue["body"].split()).casefold()
        conditions = [
            (
                " ".join(r.split()).casefold()[:60] in body,
                f"reply {r[:40]!r}... is missing from the issue body",
            )
            for r in expected["replies"]
        ]
    ...
    def wrong_observations(
        ...
        return [
            ...
            (
                "a reply is missing from the issue body",
                {**observed, "issues": [dict(issue, body="\n\n".join(REPLIES[:2]))]},
            ),
            (
                "the reply links a different issue",
                {
                    **observed,
                    "thread": REPLIES + ["Filed as https://github.com/o/harness-sandbox/issues/8"],
                },
            ),
    ...
    async def teardown(self, world: World, ns: str, expected: Expected) -> None:
        for issue in await world.github.issues_search_in_sandbox(
            expected["issue_title"], state="open"
        ):
