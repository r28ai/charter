...
class ShopifyCustomerToStripe(Scenario):
    ...
    defaults = {"first_name": "Barbara", "last_name": "Liskov", "handle": "barbara", "lookalike": False}
    variants = {
        "hyphenated": {"first_name": "Marie-Claire", "last_name": "Dubois", "handle": "marie-claire"},
        "accented": {"first_name": "José", "last_name": "Álvarez", "handle": "jose"},
        "apostrophe": {"first_name": "Siobhán", "last_name": "O'Neill", "handle": "siobhan"},
        "long_name": {"first_name": "Anna Maria", "last_name": "von der Leyen-Schmidt", "handle": "anna.maria"},
        "with_lookalike": {"lookalike": True},
        "accented_with_lookalike": {"first_name": "José", "last_name": "Álvarez", "handle": "jose", "lookalike": True},
    }
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        customer = await world.shopify.customer_create(email=email, first_name=first, last_name=last, tags=[ns, "harness"])
        ...
        if self.params["lookalike"]:
            ...
            other = await world.shopify.customer_create(email=other_email, first_name=first, last_name=last, tags=["harness"])
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        observed: Observed = {
            "customers": [
                {"id": c["id"], "email": c.get("email"), "name": c.get("name"), "metadata": c.get("metadata") or {}}
                for c in customers
            ]
        }
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        if observed.get("lookalike_customers"):
            return Verdict.fail("a Stripe customer was created for the untagged customer with the same name")
        if len(customers) != 1:
            return Verdict.fail(f"expected exactly one Stripe customer with email {expected['email']}, found {len(customers)}")
        c = customers[0]
        shopify_id = str(c["metadata"].get("shopify_id", ""))
        return check(
            [
                (norm(c["email"]) == norm(expected["email"]), "email differs"),
                (norm(c["name"]) == norm(expected["name"]), f"name is {c['name']!r}, expected {expected['name']!r}"),
                (expected["shopify_numeric_id"] in shopify_id, f"metadata.shopify_id is {shopify_id!r}, expected {expected['shopify_numeric_id']}"),
            ]
        )
    ...
    def example(self) -> tuple[Expected, Observed]:
        ...
        observed: Observed = {"customers": [{"id": "cus_1", "email": "Barbara.hx@harness.invalid", "name": "Barbara Liskov", "metadata": {"shopify_id": "8123"}}]}
        if self.params["lookalike"]:
            expected["lookalike"] = {"shopify_id": "gid://shopify/Customer/8124", "email": "barbara.zzxh@harness.invalid"}
    ...
    def wrong_observations(self, expected: Expected, observed: Observed) -> list[tuple[str, Observed]]:
        ...
        if expected.get("lookalike"):
            wrongs.append(("the untagged lookalike was copied instead", {**observed, "customers": [], "lookalike_customers": ["cus_9"]}))
        return wrongs

    async def teardown(self, world: World, ns: str, expected: Expected) -> None:
        emails = [expected["email"]] + ([expected["lookalike"]["email"]] if expected.get("lookalike") else [])
        for addr in emails:
