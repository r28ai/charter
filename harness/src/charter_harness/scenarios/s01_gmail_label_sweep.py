...
class GmailLabelSweep(Scenario):
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        with_a = [
            m["id"] for m in await world.gmail.messages_list(label_ids=[expected["label_a"]["id"]])
        ]
        with_b: list[str] = []
        if label_b:
            with_b = [m["id"] for m in await world.gmail.messages_list(label_ids=[label_b["id"]])]
        observed: Observed = {
            "label_b_exists": label_b is not None,
            "with_a": sorted(with_a),
            "with_b": sorted(with_b),
        }
        if expected.get("label_other"):
            observed["with_other"] = sorted(
                m["id"]
                for m in await world.gmail.messages_list(label_ids=[expected["label_other"]["id"]])
            )
        return observed
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        conditions = [
            ...
            (
                not (distractors & set(observed["with_b"])),
                "a message that never had the old label received the new one",
            ),
        ]
        if distractors:
            conditions.append(
                (
                    distractors <= set(observed.get("with_other") or []),
                    "a message outside the old label lost its own label",
                )
            )
    ...
    def wrong_observations(
        ...
        if expected.get("distractor_ids"):
            ...
            wrongs.append(
                (
                    "a distractor received the new label",
                    {**observed, "with_b": observed["with_b"] + [d[0]]},
                )
            )
