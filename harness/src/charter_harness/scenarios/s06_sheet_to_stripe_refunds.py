...
class SheetToStripeRefunds(Scenario):
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        for c in expected["customers"]:
            ...
            conditions.append(
                (
                    total == c["refund"],
                    f"{c['email']}: refunded {total} cents, expected {c['refund']}",
                )
            )
            conditions.append(
                (len(items) <= 1, f"{c['email']}: {len(items)} refunds issued instead of one")
            )
    ...
    def example(self) -> tuple[Expected, Observed]:
        ...
        observed = {
            "refunds": {
                "pi_1": [{"amount": 1500, "status": "succeeded"}],
                "pi_2": [{"amount": 2500, "status": "succeeded"}],
            }
        }
    ...
    def wrong_observations(
        ...
        return [
            (
                "a partial refund was issued for the full charge",
                {"refunds": {**r, "pi_1": [{"amount": 4000, "status": "succeeded"}]}},
            ),
            ("one customer was not refunded", {"refunds": {**r, "pi_2": []}}),
            (
                "a refund was issued twice",
                {
                    "refunds": {
                        **r,
                        "pi_1": [
