...
class FirecrawlToLinear(Scenario):
    ...
    variants = {
        "postmortem": {"heading": "Incident 2026-09-03 Postmortem", "body": POSTMORTEM_BODY, "phrase": POSTMORTEM_PHRASE},
        "roadmap": {"heading": "Q4 Roadmap: Payments & Billing", "body": ROADMAP_BODY, "phrase": ROADMAP_PHRASE},
        "decoy_h2": {"heading": "Deployment Runbook v4", "body": DECOY_H2_BODY, "phrase": PHRASE},
        "code_block": {"heading": "Deployment Runbook v3", "body": CODE_BLOCK_BODY, "phrase": PHRASE},
        "long_heading": {
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        await gh.content_put(path=path, content=f"# {heading}\n\n{self.params['body']}", message=f"{ns}: add runbook", branch=base)
        return {
            "tag": f"[{ns}]",
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        return {"issues": [{"identifier": i["identifier"], "title": i["title"], "description": i.get("description") or ""} for i in issues]}
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        return check(
            [
                (title == wanted, f"title is {issues[0]['title']!r}, expected {expected['tag']} {expected['heading']!r}"),
                (norm(phrase) in norm(issues[0]["description"]), "description does not carry the page content"),
            ]
        )
    ...
    def example(self) -> tuple[Expected, Observed]:
        ...
        observed = {"issues": [{"identifier": "ENG-50", "title": f"[hx] {heading}", "description": f"# {heading}\n\n" + self.params["body"]}]}
        return expected, observed

    def wrong_observations(self, expected: Expected, observed: Observed) -> list[tuple[str, Observed]]:
        i = observed["issues"][0]
        return [
            ("the title differs from the H1", {"issues": [dict(i, title="[hx] Deployment Runbook")]}),
    ...
    async def teardown(self, world: World, ns: str, expected: Expected) -> None:
        ...
        await world.github.content_delete(path=expected["path"], branch=expected["branch"], message=f"{ns}: remove runbook")
