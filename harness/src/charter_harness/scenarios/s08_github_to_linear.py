...
class GitHubToLinear(Scenario):
    id = "github_to_linear"
    packs = ("github", "linear")
    summary = "Create a Linear issue per labelled GitHub issue and comment the Linear identifier back."
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        linear_issues = await world.linear.issues_tagged_eventually(expected["tag"], at_least=len(expected["issues"]))
        ...
        comments = await eventually(read_comments, lambda c: all(c[str(n)] for n in numbers), timeout=20.0)
        return {
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        for issue in expected["issues"]:
            ...
            conditions.append((len(ids) == 1, f"expected one Linear issue titled {wanted!r}, found {len(ids)}"))
            bodies = " ".join(observed["comments"].get(str(issue["number"]), [])).upper()
            conditions.append(
                (any(i.upper() in bodies for i in ids), f"GitHub #{issue['number']} has no comment naming its Linear issue")
            )
        conditions.append((len(observed["linear"]) == len(expected["issues"]), f"{len(observed['linear'])} Linear issues carry the tag, expected {len(expected['issues'])}"))
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected = {
            ...
            "label": "hx",
            "issues": [{"number": 1, "title": "Login page flashes white before render", "url": "u1"}, {"number": 2, "title": "CSV export drops the header row", "url": "u2"}],
            "team_key": "ENG",
            "tag": "[hx]",
        }
        observed = {
    ...
    def wrong_observations(self, expected: Expected, observed: Observed) -> list[tuple[str, Observed]]:
        return [
            ("a GitHub issue has no link-back comment", {**observed, "comments": {**observed["comments"], "2": []}}),
            ("a Linear issue is missing", {**observed, "linear": observed["linear"][:1]}),
            ("a comment names the wrong Linear issue", {**observed, "comments": {**observed["comments"], "2": ["Tracked in ENG-10"]}}),
            ("a Linear title was paraphrased", {**observed, "linear": [observed["linear"][0], {"identifier": "ENG-11", "title": "[hx] CSV export missing header"}]}),
        ]
