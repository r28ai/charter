...
class InboxToSheet(Scenario):
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        await world.sheets.values_update(
            sheet["spreadsheetId"], "Sheet1!A1:B1", [["From", "Subject"]]
        )
        return {
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        return check(
            [
                ...
                (
                    len(rows) == len(expected["rows"]),
                    f"expected {len(expected['rows'])} rows, found {len(rows)}",
                ),
            ]
        )
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected = {
            ...
            "rows": [
                {
                    "email": "ada.hx@harness.invalid",
                    "subject": "[hx] Invoice overdue for order 4471",
                },
                {
                    "email": "grace.hx@harness.invalid",
                    "subject": "[hx] Question about the enterprise plan",
                },
            ],
    ...
    def wrong_observations(
        ...
        return [
            ...
            (
                "an extra row was added",
                {"values": values + [["nobody@harness.invalid", "[hx] Not a real message"]]},
            ),
            (
                "an unlabelled message was rowed",
                {"values": values + [["edsger.hx@harness.invalid", "[hx] Weekly newsletter"]]},
            ),
            (
                "a subject was altered",
                {"values": values[:-1] + [[values[-1][0], "Enterprise plan question"]]},
            ),
        ]
