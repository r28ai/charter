...
class StripeToSheet(Scenario):
    ...
    summary = (
        "Find the Stripe customers matching an email pattern and list them (email, id) in a sheet."
    )
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        await world.sheets.values_update(
            sheet["spreadsheetId"], "Sheet1!A1:B1", [["Email", "Customer ID"]]
        )
        return {
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        got = {
            (
                str(r[0]).strip().casefold() if len(r) > 0 else "",
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected = {
            "spreadsheet_id": "s",
            "customers": [
                {"id": "cus_A", "email": "ada.hx@harness.invalid"},
                {"id": "cus_B", "email": "grace.hx@harness.invalid"},
            ],
        ...
        observed = {
            "values": [
                ["Email", "Customer ID"],
                ["ada.hx@harness.invalid", "cus_A"],
                ["Grace.hx@harness.invalid", "cus_B"],
            ]
        }
    ...
    def wrong_observations(
        ...
        return [
            (
                "the distractor customer was included",
                {"values": v + [["dennis.zzxh0@harness.invalid", "cus_Z"]]},
            ),
