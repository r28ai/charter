...
class SheetToCalendar(Scenario):
    ...
    summary = (
        "Create one calendar event per spreadsheet row, at the stated local time and duration."
    )
    ...
    variants = {
        ...
        "long_blocks": {
            "sessions": [
                ("Workshop", 9, 0, 180),
                ("Deep work", 13, 0, 120),
                ("Office hours", 16, 0, 60),
            ]
        },
        "early_late": {
            "sessions": [
                ("Breakfast briefing", 7, 30, 30),
                ("Midday check-in", 12, 0, 15),
                ("Evening review", 18, 45, 45),
            ]
        },
    }
    ...
    async def seed(self, world: World, ns: str) -> Expected:
        ...
        await world.sheets.values_update(
            ...
            [["Title", "Start", "Minutes"]]
            + [[r["title"], r["start_local"], r["minutes"]] for r in rows],
        )
        return {
            ...
            "rows": rows,
            "window": {
                "min": utc_iso(day - timedelta(days=1)),
                "max": utc_iso(day + timedelta(days=2)),
            },
        }
    ...
    async def observe(self, world: World, ns: str, expected: Expected) -> Observed:
        ...
        events = await world.calendar.events_list_eventually(
            q=ns,
            time_min=expected["window"]["min"],
            time_max=expected["window"]["max"],
            at_least=len(expected["rows"]),
        )
        return {
            "events": [
                {
    ...
    def judge(self, expected: Expected, observed: Observed) -> Verdict:
        ...
        for row in expected["rows"]:
            ...
            conditions.append(
                (
                    len(matches) == 1,
                    f"expected exactly one event titled {row['title']!r}, found {len(matches)}",
                )
            )
            if len(matches) == 1:
                e = matches[0]
                conditions.append(
                    (
                        _same_instant(e["start"], row["start"]),
                        f"{row['title']!r} starts at {e['start']}, expected {row['start']}",
                    )
                )
                conditions.append(
                    (
                        _same_instant(e["end"], row["end"]),
                        f"{row['title']!r} ends at {e['end']}, expected {row['end']}",
                    )
                )
        conditions.append(
            (
                len(observed["events"]) == len(expected["rows"]),
                f"expected {len(expected['rows'])} events, found {len(observed['events'])}",
            )
        )
    ...
    def example(self) -> tuple[Expected, Observed]:
        expected = {
            ...
            "rows": [
                ...
                {
                    "title": "hx Retro",
                    "start_local": "2026-09-14 15:00",
                    "minutes": 30,
                    "start": "2026-09-14T15:00:00+02:00",
                    "end": "2026-09-14T15:30:00+02:00",
                },
            ],
    ...
    def wrong_observations(
        ...
        return [
            (
                "an event starts one hour off (time zone mistake)",
                {"events": [off_by_hour] + events[1:]},
            ),
