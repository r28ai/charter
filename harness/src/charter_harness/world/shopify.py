...
class Shopify:
    ...
    async def gql(self, query: str, variables: dict[str, Any] | None = None) -> dict[str, Any]:
        data = await self.http.json(
            "POST", self.graphql_path, json={"query": query, "variables": variables or {}}
        )
        if data.get("errors"):
            raise WorldError(
                f"Shopify GraphQL error: {data['errors']}", status=200, body=str(data)[:2000]
            )
        return data["data"]
    ...
    async def product_create(
        self, *, title: str, tags: list[str], inventory: int, price: str = "10.00"
    ) -> dict[str, Any]:
        data = await self.gql(
            """
    ...
    async def await_products_tagged(
        ...
        *,
        ...
        def settled(products: list[dict[str, Any]]) -> bool:
            ...
            return wanted <= set(seen) and all(
                seen[i].get("totalInventory") == units for i, units in expected.items()
            )

        got = await eventually(
            lambda: self.products_tagged(tag), settled, timeout=timeout, interval=3.0
        )
        missing = wanted - {p["id"] for p in got}
        if missing:
            raise WorldError(
                f"{len(missing)} Shopify product(s) tagged {tag!r} not searchable after {timeout:.0f}s"
            )
        stale = {
            p["title"]: (p.get("totalInventory"), expected[p["id"]])
            for p in got
            if p["id"] in expected and p.get("totalInventory") != expected[p["id"]]
        }
        if stale:
            raise WorldError(
                f"Shopify inventory not settled after {timeout:.0f}s (seen, set): {stale}"
            )
        return got
    ...
    async def customer_create(
        self, *, email: str, first_name: str, last_name: str, tags: list[str]
    ) -> dict[str, Any]:
        data = await self.gql(
            """
    ...
    async def await_customers_tagged(
        self, tag: str, ids: list[str], *, timeout: float = 90.0
    ) -> list[dict[str, Any]]:
        wanted = set(ids)
        got = await eventually(
            lambda: self.customers_tagged(tag),
            lambda cs: wanted <= {c["id"] for c in cs},
            timeout=timeout,
            interval=3.0,
        )
        ...
        if missing:
            raise WorldError(
                f"{len(missing)} Shopify customer(s) tagged {tag!r} not searchable after {timeout:.0f}s"
            )
        return got
