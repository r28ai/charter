...
class Linear:
    ...
    async def gql(self, query: str, variables: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if data.get("errors"):
            raise WorldError(
                f"Linear GraphQL error: {data['errors']}", status=200, body=str(data)[:2000]
            )
        return data["data"]
    ...
    async def await_tagged(
        self, tag: str, ids: Sequence[str], *, timeout: float = 90.0
    ) -> list[dict[str, Any]]:
        ...
        issues = await eventually(
            lambda: self.issues_tagged(tag),
            lambda got: wanted <= {i["id"] for i in got},
            timeout=timeout,
            interval=2.0,
        )
        ...
        if missing:
            raise WorldError(
                f"{len(missing)} Linear issue(s) tagged {tag!r} not visible to the title filter after {timeout:.0f}s"
            )
        return issues
    ...
    async def issues_tagged_eventually(
        self, tag: str, *, at_least: int, timeout: float = 45.0
    ) -> list[dict[str, Any]]:
    ...
    async def issue_get(self, issue_id: str) -> dict[str, Any] | None:
        try:
            data = await self.gql(
                "query($id: String!) { issue(id: $id) { " + _ISSUE_FIELDS + " } }", {"id": issue_id}
            )
        except WorldError:
            return None
    ...
    async def issue_delete(self, issue_id: str) -> None:
        try:
            await self.gql(
                "mutation($id: String!) { issueDelete(id: $id) { success } }", {"id": issue_id}
            )
        except WorldError:
            pass
