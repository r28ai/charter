...
class GitHub:
    ...
    async def issue_create(self, *, title: str, body: str = "", labels: Sequence[str] = ()) -> dict[str, Any]:
        return await self.http.json(
            "POST", self._r("issues"), json={"title": title, "body": body, "labels": list(labels)}
        )

    async def issues_list(self, *, labels: Sequence[str] = (), state: str = "all") -> list[dict[str, Any]]:
    ...
    async def await_issues_listed(self, *, label: str, numbers: Sequence[int], timeout: float = 45.0) -> None:
    ...
    async def issue_comments(self, number: int) -> list[dict[str, Any]]:
        return await self.http.json("GET", self._r(f"issues/{number}/comments"), params={"per_page": 100})
    ...
    async def issues_search_in_sandbox(self, text: str, *, state: str = "all", expect: int = 0) -> list[dict[str, Any]]:
        ...
        async def read() -> list[dict[str, Any]]:
            ...
            return [i for i in items if needle in (i.get("title", "") + "\n" + (i.get("body") or "")).lower()]
    ...
    async def content_put(self, *, path: str, content: str, message: str, branch: str) -> dict[str, Any]:
        ...
        for attempt in range(5):
            existing_sha = None
            try:
                existing = await self.http.json("GET", self._r(f"contents/{path}"), params={"ref": branch})
    ...
    async def branch_create(self, name: str, *, from_branch: str | None = None) -> str:
        ...
        sha = ref["object"]["sha"]
        try:
            await self.http.json("POST", self._r("git/refs"), json={"ref": f"refs/heads/{name}", "sha": sha})
        except WorldError as exc:
    ...
    async def pull_create(self, *, title: str, head: str, base: str, body: str = "") -> dict[str, Any]:
        return await self.http.json(
            "POST", self._r("pulls"), json={"title": title, "head": head, "base": base, "body": body}
        )

    async def pull_files(self, number: int) -> list[dict[str, Any]]:
        return await self.http.json("GET", self._r(f"pulls/{number}/files"), params={"per_page": 100})
