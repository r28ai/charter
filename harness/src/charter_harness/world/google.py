...
class Gmail:
    ...
    async def await_labelled(
        self, label_id: str, ids: Sequence[str], *, timeout: float = 90.0
    ) -> list[dict[str, Any]]:
        ...
        got = await eventually(
            lambda: self.messages_list(label_ids=[label_id]),
            lambda ms: wanted <= {m["id"] for m in ms},
            timeout=timeout,
            interval=3.0,
        )
        ...
        if missing:
            raise WorldError(
                f"{len(missing)} Gmail message(s) not listed under label {label_id} after {timeout:.0f}s"
            )
        return got
    ...
    async def message_get(self, message_id: str, *, fmt: str = "metadata") -> dict[str, Any]:
        ...
        return await self.http.json(
            "GET", f"gmail/v1/users/me/messages/{message_id}", params=params
        )
    ...
    async def draft_get(self, draft_id: str) -> dict[str, Any] | None:
        ...
        try:
            return await self.http.json(
                "GET", f"gmail/v1/users/me/drafts/{draft_id}", params={"format": "full"}
            )
        except WorldError as exc:
            if exc.status == 404:
                return None
...
class Calendar:
    ...
    async def events_list(
        ...
        *,
        ...
        data = await self.http.json(
            "GET", f"calendar/v3/calendars/{calendar_id}/events", params=params
        )
    ...
    async def await_events(
        ...
        if missing:
            raise WorldError(
                f"{len(missing)} Calendar event(s) not found by q={q!r} after {timeout:.0f}s"
            )
        return got
    ...
    async def event_get(
        ...
        try:
            return await self.http.json(
                "GET", f"calendar/v3/calendars/{calendar_id}/events/{event_id}"
            )
        except WorldError as exc:
    ...
    async def event_delete(self, event_id: str, *, calendar_id: str = "primary") -> None:
        try:
            await self.http.request(
                "DELETE", f"calendar/v3/calendars/{calendar_id}/events/{event_id}"
            )
        except WorldError as exc:
...
class Sheets:
    ...
    async def spreadsheet_create(
        self, title: str, *, sheet_title: str = "Sheet1"
    ) -> dict[str, Any]:
        return await self.http.json(
            "POST",
            "v4/spreadsheets",
...
class Drive:
    ...
    async def files_list_eventually(
        self, *, name_contains: str, at_least: int = 1, timeout: float = 45.0
    ) -> list[dict[str, Any]]:
        """For files the model created: Drive search can lag a fresh document."""
        return await eventually(
            lambda: self.files_list(name_contains=name_contains),
            lambda fs: len(fs) >= at_least,
            timeout=timeout,
            interval=3.0,
        )
