...
class Slack:
    ...
    async def call(self, method: str, *, params: dict[str, Any] | None = None, json: Any = None) -> dict[str, Any]:
        verb = "POST" if json is not None else "GET"
        data = await self.http.json(verb, method, params=params, json=json)
        if not data.get("ok"):
            raise WorldError(f"Slack {method} failed: {data.get('error')}", status=200, body=str(data)[:2000])
        return data
    ...
    async def channel_id(self) -> str:
        if self._channel_id is None:
            cursor = None
            while True:
                params: dict[str, Any] = {"limit": 200, "types": "public_channel,private_channel", "exclude_archived": True}
                if cursor:
    ...
    async def replies(self, thread_ts: str) -> list[dict[str, Any]]:
        data = await self.call(
            "conversations.replies", params={"channel": await self.channel_id(), "ts": thread_ts, "limit": 200}
        )
        return data.get("messages", [])
