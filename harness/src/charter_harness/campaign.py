...
def preflight(models: list[str], api_key: str) -> None:
    ...
    for model in models:
        ...
        if response.status_code >= 400:
            raise SystemExit(f"model {model_id} is not usable on Fireworks: {response.status_code} {response.text[:300]}")
...
def run(
    *,
    ...
    if not chosen:
        raise SystemExit(f"no runnable scenario matches {scenarios!r} with the configured providers {sorted(settings.available())}")
    ...
    print(f"campaign {name}: {'complete' if success else 'INCOMPLETE'} — {len(logs)} log(s) in {log_dir}")
    return log_dir
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_run.add_argument("--models", default="nemotron", help=f"comma-separated; short names: {', '.join(MODELS)}")
    p_run.add_argument("--scenarios", default="all", help="all | id,id,... | pack name")
    p_run.add_argument("--epochs", type=int, default=3)
    p_run.add_argument("--max-samples", type=int, default=2, help="samples in flight per task (quota-bound)")
    ...
    p_run.add_argument("--variants", action="store_true", help="phase 3: include every parametric variant of the selected templates")
    ...
    p_list.add_argument("--all-providers", action="store_true", help="ignore which providers are configured")
    ...
    if args.command == "list":
        ...
        print(f"{len(chosen)} scenario(s)" + ("" if args.all_providers else f"; configured providers: {', '.join(sorted(settings.available()))}"))
        return 0
