...
def make_cleanup(world: World, scenarios: dict[str, Scenario], keep: bool):
    async def cleanup(state: TaskState) -> None:
        ...
        if not ctx.seeded:
            ...
            logger.warning(
                "seed for %s failed; anything tagged %s is left for a manual sweep",
                ctx.scenario,
                ctx.ns,
            )
            return
...
def build_task(
    *,
    ...
    chosen = select(
        scenarios, available=settings.available() if only_available else None, variants=variants
    )
    if not chosen:
        raise ValueError(
