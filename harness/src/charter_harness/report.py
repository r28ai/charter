    ...
    for sample in log.samples or []:
        ...
        runs.append(
            Run(
                ...
                template=str(
                    (sample.metadata or {}).get("template") or str(sample.id).split("@")[0]
                ),
                epoch=sample.epoch,
...
def _markdown(
    ...
    if templates:
        lines.extend(
            _breakdown_table("Per template — successes / runs, variants rolled up", templates, keys)
        )
    ...
    for key in keys:
        for run in cells[key].runs:
            ...
            lines.append(
                f"- **{run.scenario}** · {run.model} · {run.arm} · epoch {run.epoch}: {reason}"
            )
    if not any_failure:
        lines.append("_none_")
    lines.append("")
    ...
    if harness_errors:
        ...
        for run in harness_errors:
            lines.append(
                f"- **{run.scenario}** · {run.model} · {run.arm} · epoch {run.epoch}: {run.explanation}"
            )
        lines.append("")
        ...
        json.dumps(
            {
                ...
                "per_scenario": {
                    scenario: {
                        f"{m}|{a}": {"successes": k, "runs": n} for (m, a), (k, n) in v.items()
                    }
