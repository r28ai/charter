...
def _rest_params(pack: str) -> ToolParams:
    ...
    return ToolParams(
        properties={
            ...
            "path": JSONSchema(type="string", description="Path relative to the base URL, parameters filled in."),
            "query": JSONSchema(
                anyOf=[JSONSchema(type="object", additionalProperties=True), JSONSchema(type="null")],
...
class RawArm:
    ...
    def tools(self, packs: Sequence[str], wiring: Wiring) -> list[ToolDef]:
        ...
        for pack in packs:
            defs.append(_graphql_tool(pack, wiring) if pack in GRAPHQL_PACKS else _rest_tool(pack, wiring))
